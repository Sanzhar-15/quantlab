...
function buildClientScript(sheetForClient: number): string {
	...
	return [
		...
		'    activeInput = null;',
		'    activeCell = null;',
		'    // V3.4.0.X MEDIUM-2 closure (2026-05-24, single-lane Codex):',
		'    // broadcast typing=false on BOTH commit AND cancel paths.',
		'    // Pre-closure the broadcast lived ONLY in the cancel branch',
		'    // below; the commit branch did `return` before reaching it,',
		'    // so remote peers stayed visually "typing" indefinitely',
		'    // (until another presence write or panel dispose).  Now the',
		'    // broadcast fires HERE -- after the active{Input,Cell} reset',
		'    // but before either branch -- so both transitions out of',
		'    // edit mode clear the remote-peer cursor indicator within',
		'    // ~1s (pollRemote cadence).',
		'    vscode.postMessage({',
		'      type: \'presenceUpdate\',',
		'      state: {',
		'        sheet: SHEET,',
		...
		'      }',
		'    });',
		'    // V3.6.0.11 D9 (2026-05-26): emit typing_stroke on each',
		'    // text-change input event.  Host resets the 30s presence-',
		'    // typing watchdog IFF the flag is set.  Closes R-V3.5-7',
		'    // "long formula entry hits 30s" false-negative case.  The',
		'    // input event fires on typing/paste/deletion (not on focus',
		'    // or programmatic value sets), so the initial focus does',
		'    // not emit a stroke -- the prior presenceUpdate(typing:true)',
		'    // already armed the watchdog.  Fire-and-forget; no payload',
		'    // (the envelope itself is the signal).  Cleanup is implicit:',
		'    // endEdit removes the input from DOM + the closure is GC\\\'d.',
		'    input.addEventListener(\'input\', function () {',
		'      if (activeInput !== input) { return; }',
		'      vscode.postMessage({ type: \'typing_stroke\' });',
		'    });',
		...
		'        : \'\';',
		'      // 6.4-3d Step 5: mirror the server renderRows diagnostic tooltip',
		'      // so it SURVIVES virtualization repaints (renderRowsClient',
		'      // overwrites tbody.innerHTML on every scroll). htmlEscape-safe.',
		'      var titleAttr = (typeof e.diagnostic === \'string\' && e.diagnostic.length > 0)',
		'        ? \' title="\' + htmlEscape(e.diagnostic) + \'"\'',
		'        : \'\';',
