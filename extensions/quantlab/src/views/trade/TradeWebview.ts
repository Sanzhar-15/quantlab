...
export class TradeWebview {
	...
	static buildHtml(webview: vscode.Webview, extensionUri: vscode.Uri): string {
		const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'webview', 'trade.js'));
		const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'dist', 'webview', 'trade-style.css'));
		...
		const nonce = TradeWebview.getNonce();
...
<head>
	...
	<title>Quantlab Trade</title>
</head>
<body>
	<div id="trade-root"></div>
