...
export class NativeRequestService extends AbstractRequestService implements IRequestService {
	...
	private readonly requestChannelClient: RequestChannelClient;
	...
	constructor(
		...
		// Use IPC channel to main process for CORS-free requests
		this.requestChannelClient = new RequestChannelClient(mainProcessService.getChannel('request'));
	}
	...
	async request(options: IRequestOptions, token: CancellationToken): Promise<IRequestContext> {
		...
		// Route through main process to bypass CORS restrictions
		return this.logAndRequest(options, () => this.requestChannelClient.request(options, token));
	}
