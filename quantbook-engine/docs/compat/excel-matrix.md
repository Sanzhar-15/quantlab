...
| DB / VDB | ❌ | 0 | 4.10 polish | Remaining depreciation methods: DB (fixed-declining-balance with monthly fraction), VDB (variable-declining-balance with period range + switch-to-straight-line). Deferred from W5-181. IronCalc references at `.references/ironcalc/base/src/functions/financial.rs::fn_db` (+ VDB if present). |
