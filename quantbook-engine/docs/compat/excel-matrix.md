...
| DDB / DB / VDB | ❌ | 0 | 4.10 polish | Period-iteration depreciation methods with optional `factor` arg (DDB) / monthly fraction (DB) / cumulative range (VDB). Deferred to a follow-on W5-181+ cycle from W5-180. IronCalc references at `.references/ironcalc/base/src/functions/financial.rs::fn_ddb / fn_db`. |
