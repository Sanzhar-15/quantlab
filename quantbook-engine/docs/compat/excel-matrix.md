...
| SYD | ✅ | 10 | 4.10 polish | W5-180: sum-of-years digits depreciation. ScalarFn. Formula: `(cost − salvage) · (life − per + 1) · 2 / (life · (life + 1))`. **Asymmetric degenerate-case canon with SLN**: SYD `life = 0` → `#NUM!` (NOT `#DIV/0!`) per Microsoft + IronCalc; `per <= 0` or `per > life` → `#NUM!`. `per = life` boundary is valid (the rule is `>`, not `>=`). Pinned by `syd_sum_over_periods_equals_total_depreciation` invariant test: Σ SYD over k=1..life = (cost − salvage). |
| DDB | ✅ | 15 | 4.10 polish | W5-181: double-declining-balance depreciation. ScalarFn, 4-5 args (`factor` defaults to 2). Closed-form despite the name (no period iteration). Formula: `rate = min(factor/life, 1)`; `value = cost·(1−rate)^(period−1)` if rate<1 else (cost if period==1 else 0); `new_value = cost·(1−rate)^period`; `result = max(value − max(salvage, new_value), 0)`. The salvage floor stops depreciation before over-depreciating (Excel canon). Validation: `period>life`, `cost<0`, `salvage<0`, `period<=0`, `factor<=0` all → `#NUM!`. **Engine-convention divergence vs IronCalc**: IronCalc rejects Boolean for `factor` only (via `get_number_no_bools`); our `arg_num` allows Boolean uniformly across all args for consistency with the rest of the financial family. |
| DB | ✅ | 17 | 4.10 polish | W5-182: fixed-declining-balance depreciation. ScalarFn, 4-5 args (`month` defaults to 12, truncated to integer). **Period-iterating** (each period depends on accumulated book value from prior periods) — unlike DDB which is closed-form. Excel-specific 3-decimal rate rounding: `rate = round((1 − (salvage/cost)^(1/life)) · 1000) / 1000` deliberately introduces a small per-period error that the last-partial-period absorbs. First period uses `month/12` fraction; last period (when `period = life + 1` and `month ≠ 12`) uses complementary `(12 − month)/12` fraction. Validation: `month == 12 && period > life`, `period > life + 1`, `month <= 0`, `month > 12`, `period <= 0`, `cost < 0` all → `#NUM!`. `cost == 0` short-circuits to `0` (avoiding `(salvage/0)` NaN). |
| VDB | ❌ | 0 | 4.10 polish | Remaining depreciation method: variable-declining-balance with period range + switch-to-straight-line. Most complex of the depreciation batch (multiple optional args + cumulative range + DDB-vs-SLN crossover logic). |
