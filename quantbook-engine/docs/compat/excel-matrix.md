...
| SUMIF | ✅ | 17 | 4.3 V2 / w147 | W5-53 + W5-61: supports number/text/bool/blank criteria + comparators `> < >= <= <> =`. **Wildcards `?` / `*` shipped W5-61** (escape with `~`); apply only to Eq/Ne text criteria, only against text cells (not coerced numbers). 2-arg + 3-arg forms; sum_range default to range. Error in sum_range cell propagates (Excel canon). **SHAPE ANCHORING (w147, TA2/COR-02 — closed):** when `sum_range` differs in shape/size from `range`, Quantbook now reads a rectangle of `range`'s shape anchored at `sum_range`'s top-left, IGNORING `sum_range`'s declared extent — Excel-correct (`SUMIF(A2:A10,">5",B2)` reads B2:B10; out-of-extent cells pad Blank → 0). Implemented at the eval-site dispatch (`anchor_resize_value_arg`) via the registry `ShapeAnchorSpec`; the kernel flat-zip is unchanged. **Residuals (exotic):** a `StructuredRef` value arg is NOT re-anchored — same-table columns always share row-count so the flat-zip is already correct, but a CROSS-table structured value arg with a different row-count still zip-truncates; a LET/LAMBDA array-local value arg is NOT re-anchored either (no grid coords → still zip-truncates); a scalar / constant-name value arg → `#VALUE!`. A single-cell defined name (`NamedTarget::Cell`, e.g. xlsx-imported) IS anchored (transparent `ScalarNameRef` unwrap, w147). |
...
| AVERAGEIF | ✅ | 6 | 4.3 V2 / w147 | W5-55: same predicate suite as SUMIF; #DIV/0! on no matches; optional separate average_range. **SHAPE ANCHORING (w147, TA2/COR-02 — closed):** same as SUMIF — when `average_range` differs in shape/size from `range`, Quantbook now reads a rectangle of `range`'s shape anchored at `average_range`'s top-left (out-of-extent cells pad Blank → skipped in the average, NOT counted as 0). Same dispatch mechanism + same residuals as SUMIF (StructuredRef / array-local value args not re-anchored). |
...
| TRUE / FALSE | ✅ | — | 4.1 | Parsed directly to `Expr::Bool` at `crates/ql-formula-syntax/src/parser.rs:290-298`. `TRUE()` / `FALSE()` function-call form intentionally absent. **W5-D-PM12-1 (megaudit Codex LOW-1 closure): status flipped from ⚠️ to ✅ — explicit Bool token landed.** |
...
| SUBTOTAL | ⚠️ | 31 | 4.10 V1-260 sealer (W5-D-12) | W5-D-12: conditional aggregate dispatcher. Range-aware; routes function_num ∈ {1..=11, 101..=111} to AVERAGE/COUNT/COUNTA/MAX/MIN/PRODUCT/STDEV.S/STDEV.P/SUM/VAR.S/VAR.P. **Hidden-row skip (Wave G2)** and **nested-SUBTOTAL skip (2026-06-29)** both implemented: 101..=111 skip hidden rows; nested SUBTOTAL cells in arg RANGES are skipped for ALL function_nums (the ql-exec materializer blanks AST-root SUBTOTAL cells before dispatch — `=SUBTOTAL(..)+0` has root `+` so is NOT skipped — matching IronCalc). **v1 divergences (documented)**: (1) a single-cell DIRECT ref arg to a subtotal (`SUBTOTAL(9, A1)` where A1=`SUBTOTAL(..)`) is NOT skipped (it materializes as a scalar, not a range) — exotic; range-arg parity is full; (2) inherited from dispatched aggregates: text in data → #VALUE! (vs IronCalc/Excel skip); Boolean in data → coerced 1/0 (vs skip); Blank scalar → skipped (vs IronCalc push-as-0); PRODUCT of empty → 0 (vs 1.0 multiplicative identity). function_num truncates toward zero. NaN function_num → #NUM! (error-code consistency, not panic-safety). Boolean function_num coerces TRUE→1/FALSE→0 (IronCalc match, Excel-divergent). Text function_num always → #VALUE! (engine W5-58 strict, vs IronCalc lenient parse). **Binder admission**: scalar args (`SUBTOTAL(9, 1, 2)`), named-range args (`SUBTOTAL(9, SalesRange)`), AND literal range args (`SUBTOTAL(9, A1:A10)`) all bind + evaluate (the W2-literal-range lift landed; pinned by `w2_literal_range_subtotal_full_path` + the `owning_session_subtotal_nested_skip_*` session tests). IronCalc reference: `subtotal.rs` line-by-line — dispatch shape, hidden-row skip, AND (range-arg) nested-skip now match. Still differing: the inherited data-coercion semantics, and IronCalc additionally distinguishes table-FILTERED rows from manually-hidden rows (the engine has no filtered/hidden distinction — it skips hidden rows for 101..=111 only). **CLOSES Phase 4.10 V1-260 at 260 registered fns.** ⚠️ = partial: remaining divergences are the inherited-aggregate data coercions + the exotic single-cell nested residual. |
...
| DAYS / DAYS360 / NETWORKDAYS / NETWORKDAYS.INTL | ⚠️ | (see DATE row) | 4.5 V2 (W5-75) | W5-D-13.1: DAYS, DAYS360, NETWORKDAYS registered. **W5-74 (w144): NETWORKDAYS holidays arg now supported** (GAP-F-09 closed — range-and-context-aware tier; holidays in `[start,end]` excluded, blanks skipped, text→`#VALUE!`). **NETWORKDAYS.INTL not registered** (weekend-mask variant — defer). DAYS360 was previously also blocked at lex time (4-letter prefix); W5-D-13.1 lexer Ident-fallback closure makes it reachable from source text. |
| WORKDAY / WORKDAY.INTL | ⚠️ | (see DATE row) | 4.5 V2 (W5-75) | W5-D-13.1: WORKDAY registered. **W5-74 (w144): WORKDAY holidays arg now supported** (GAP-F-10 closed — same tier; a holiday is skipped like a weekend; `WORKDAY(s,0,..)` returns `s`). **WORKDAY.INTL not registered** (weekend-mask variant — defer). |
