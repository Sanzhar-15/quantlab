...
**✅ 6.1A + 6.1B inc.1/1b + inc.2a/2b SHIPPED 2026-05-26.** Contract: `docs/api/session-api.md` v2 (API6-01/02/03; Codex gpt-5.5 xhigh REVISE → all 13 findings resolved; review `docs/api/codex-6-1a-review.md`). Contract crate: `ql-session` (`38108a5c8ef`) — complete type-level `EngineSession` trait + DTOs + EngineError + operation/lifecycle + table ops. **Owning `WorkbookSession` core path SHIPPED** (`7335a5a1bfa`, preceded by session-owned-PlanCache refactor `83b1b33bac2`): `crates/ql-exec/src/session.rs` `impl EngineSession` — REAL: lifecycle + `{epoch,op_count}` 24-byte version token + single-cell mutations + add/rename sheet + set_name + recalc (Busy/op-registry, CellDiagnostic events) + read (snapshot/cell/list_sheets) + cancel/operation_status/poll_events; error mapping via free fns (orphan rules forbid `From<RuntimeError> for EngineError`). 8 session tests + 644 ql-exec lib green, clippy clean. Remaining trait methods surfaced `Capability/not_implemented_in_v1_core` (No-Fallbacks). Doc fix: ql-oplog::OpLog DOES expose a Loro VV (`oplog_vv`) — earlier "no VV" claim corrected; `{epoch,op_count}` is the deliberate v1 token. **NEXT = 6.1B inc.2c** (per `docs/api/workbook-session-impl-plan.md` §0): validate_formula → query_range (needs a `CellValue::Blank` decision) → snapshot_delta → delete/restore/move_sheet (MED-3) → tables → batch/txn/undo/redo/persistence → functions/bulk → Node smoke migration → 6.1C audit.
