...
**✅ PHASE 5 COMPLETE (2026-05-26)** — the 5.8 Phase 5 Megaudit (the "Megaudit after 5.7" checkpoint) ran 5-way at source `1465b1db4c4` and returned **PASS-WITH-FINDINGS**: ZERO reachable HIGH, risk register 39/39 confirmed (0 falsely-closed), exit criteria met for the reachable surface. Exit Criterion #2 amended below to scope collaborative table-merge as deferred (no collaborative table producer exists). Canonical record: `docs/phase5/phase-5-exit-packet.md` + `docs/phase5/megaudit-5-8/closures.md`. Confirmed-but-latent closures are post-exit polish (next session) — they do NOT block Phase 6 entry.

**Status (2026-05-24): Phase 5 V1 + D-1 + 5.3 + 5.5 V2 V2 + 5.5 V2 V3 V1 (steps 1-6) + 5.5 V2 V4 V1 (12/13 Tier items; K4 chunking deferred to V2 V4 V2) + 5.7 V1 + 5.7 V2 (V2.1 -> V2.7 + V2.8 megaudit + V2.9) + 5.7 V3.1 (a-e multi-window IDE demo + 2-lane audit + 6 actionable closures) + 5.7 V3.2 (a + a.1 + b + c + d + e cell-grid UI vertical slice + 2-lane audit + 5 actionable closures + exit packet) + 5.7 V3.3.0 (0.1-0.7 + 0.X 2-lane megaudit + 10 actionable closures + plan archive) + 5.7 V3.4 (0.1-0.7 + 0.X 2-lane megaudit + 9 closures + plan archive at `2706d555149`) ALL SHIPPED + AUDITED. ✅ **5.7 V3.5 (WorkbookSnapshot + sheet ops + format extension + partial-invalidate undo + mid-edit-render guard, ALL 11 SUB-STEPS SHIPPED + AUDITED 2026-05-24)** -- 0.1 decision lock (5 D-decisions; engine `48be1878db5`); 0.2 WorkbookSnapshot napi D3 (engine `bbd129ed629` + IDE `299fed5eda9`); 0.3 D4 SHEET-OPS SUITE COMPLETE (a renameSheet engine `74aa5039e04` + IDE `5ad934d087f`; b deleteSheet + Op::RemoveSheet tombstone engine `9be3672ca01` + IDE `91a6b3870bd`; c moveSheet + Op::MoveSheet display-order overlay engine `7ec7877a018` + IDE `e3642ef3458`); V3.5 deep-audit closures engine `fa7541f1e8d`; 0.4a sheet management commands engine `1be56176fd2` + IDE `8fecc25c03f`; 0.4b cell-grid render() workbookSnapshot migration engine `75963628c54` + IDE `0d4659e62d6`; 0.5 CellState format extension D1 engine `175258f4dd0` + IDE `0b6a6481a14`; 0.6 partial-invalidate undo D2 engine `1bd21296f31`; 0.7 mid-edit-render guard D5 (R-V3.4-3 KNOWN-GAP CLOSED) engine `bf443957d3b` + IDE `650e7ea6c3e`; 0.9 docs § 4.1.z5 engine `1792a470e40`.  **✅ 0.X cumulative megaudit (2026-05-24)** -- Codex Lane A `docs/audits/2026-05-24-phase-5-7-v3-5-0-x-codex.md` FAIL (4 HIGH + 1 LOW including A-HIGH-1 native-binding repro) + Opus Lane B `...-opus.md` PASS-WITH-FINDINGS (2 HIGH + 4 MED + 3 LOW + 1 INFO; Opus-M1 mocha-count rejected as false positive); cross-lane convergent HIGH (Codex A-HIGH-1 == Opus H2 partial-invalidate wrong-cell after remote interleaving); **5 distinct HIGHs + 1 valid MED + 1 LOW closed in-cycle** (Opus-H1 SetCellFormat tombstone guard widened to ALL 4 cell-keyed ops + new CacheEffect::RemoveSheet + new CollabSession.removed_sheets field; Opus-M2 setPresenceTyping(false) in show(); A-HIGH-2 workbookSnapshot reads from rebuild_workbook formula_at; A-HIGH-3 4 commands call refreshAll; A-HIGH-4 new _pendingRenderAfterTyping dirty flag with idle-tick defense-in-depth; convergent HIGH-1/H2 new pure_local_frontier gate on CollabSession; A-LOW-1 TS comment hygiene).  Remaining LOWs deferred to V3.5.1+ backlog (Opus-L1/L2/L3/I1 + Opus-M3 design opinion).  **R-V3.4-3 KNOWN-GAP fully CLOSED at V3.5.0.X** (V3.5.0.7 was incomplete -- A-HIGH-4 closure restores the deferred-render guarantee).  **Cumulative V3.5 ship + closure**: +34 ql-collab tests (81 -> 115: V3.5.0.5 +12 + V3.5.0.6 +13 + V3.5.0.X main +5 + V3.5.0.X follow-up audit +4) + +92 IDE mocha (254 -> 346) + ql-collab-ws unchanged at 42.  Rule 4 arc terminus HELD at 6 throughout V3.5 + V3.5.0.X (3 new types in closures all positive Send+Sync per-field walks: `CacheEffect::RemoveSheet { id: u16 }` + `CollabSession.removed_sheets: HashSet<u16>` + `CollabSession.pure_local_frontier: bool`; FormatId composition of primitives; FormatIdJson napi struct positive walk; CacheEffect::SetCellFormat private DTO; 0 new triggers).  **V3.5 PHASE COMPLETE**.  V3.5.0.4c sheet-tabs UI scaffold OPTIONAL/DEFERRED to V3.5.1+ (multi-sheet UX adequately covered by 0.4a Command Palette + 0.4b reactive title; V3.6+ multi-tab redesign may consolidate).  Canonical records: `docs/phase5/v1-exit-packet.md` (V1) + `docs/phase5/d-1-exit-packet.md` + `docs/phase5/5-3-exit-packet.md` + `docs/phase5/v2-v3-exit-packet.md` + `docs/phase5/v2-v4-v1-exit-packet.md` + `docs/phase5/5-7-v1-exit-packet.md` + **`docs/phase5/5-7-v2-exit-packet.md`** + **`docs/phase5/5-7-v3-2-exit-packet.md`** + `docs/architecture/ide-consumer-contract.md § 4.1.z5` (V3.5 surface) + `§ 4.1.z5 audit-closures` sub-section (V3.5.0.X cumulative closure record).  Remaining: V3.6+ (session-wide RegisterFormat FormatTable cache + format-aware buildHtml rendering + per-cell op-index for O(ops-for-this-cell) invalidate_cell + Loro UndoManager on_pop callback + incremental WorkbookSnapshot deltas + #REF! substitution for cross-sheet refs + Op::RestoreSheet + sheet-tabs multi-tab redesign + IDE-facing PutFormula napi for end-to-end repaired-formula integration tests) + V3.4.1+ (advanced undo polish + presence polish; per-peer color hashing; per-workbook PeerId stash if attribution feature surfaces) + V3.5.1+ (sheet-tabs UI scaffold if user signal surfaces; Opus-L1/L2/L3/I1 + M3 design polish) + 5.8 Phase 5 megaudit (~4-6d, UNBLOCKED) + V2 V4 V2 K4 chunking (~2-3d). **V3.3.1+** (incremental rendering polish) deferred indefinitely. **V2 audit ladder: 12 V2.1-V2.7 cycles + V2.8 megaudit (3 lanes) + V2.8 closures + V2.9 = 14 cycles. V3.1 audit ladder: V3.1.a-e = 5 sub-steps + V3.1.e 2-lane parallel audit + 6 actionable closures. V3.2 audit ladder: 4 ship cycles + V3.2.d 2-lane parallel audit + 5 in-cycle code closures = 5 cycles. V3.3.0 audit ladder: 7 sub-steps (0.1-0.7) shipped + 0.X 2-lane megaudit + 10 actionable closures + plan archive. V3.5 audit ladder: 11 sub-steps (0.1-0.9 + 0.X) shipped + 0.X 2-lane megaudit + 5 HIGH + 1 MED + 1 LOW closed in-cycle + V3.5.0.X follow-up audit (Codex Lane A `docs/audits/2026-05-24-phase-5-7-v3-5-0-x-closure-codex.md`; 3 MED + 2 partial LOW closed in-cycle including CODEX-MED-3 cross-finding regression closure via new safeRender helper; CODEX-MED-1 out-of-range RemoveSheet parity DEFERRED to V3.5.1+ as contrived).  Cumulative findings closed across Phase 5.7: 18+ HIGH + 49+ MEDIUM. Rule 4 arc terminus = 6 (V2.7 + V2.8 megaudit + V3.1.e + V3.2.a + V3.2.a.1 + V3.2.b + V3.2.c + V3.2.d + V3.3.0.3 + V3.3.0.4 + V3.3.0.5 + V3.3.0.X + V3.4.0.X + V3.5.0.5 FormatId/FormatIdJson positive walks + V3.5.0.X CacheEffect::RemoveSheet + CollabSession.removed_sheets + pure_local_frontier positive walks: 0 new triggers).** Audit transcripts: **32 in `docs/audits/2026-05-22-phase-5-7-*` + `docs/audits/2026-05-23-phase-5-7-*` + `docs/audits/2026-05-24-phase-5-7-*`** (5 V1 + 13 V2.1-V2.7 + 3 V2.8 megaudit + 2 V3.1.e + 2 V3.2.d + 2 V3.3.0.X + 2 V3.4.0.X + 2 V3.5.0.X Codex + Opus + 1 V3.5.0.X follow-up audit Codex Lane A).

✅ **5.7 V3.6 PHASE TERMINATION CLEAN** (V3.6.0.1 LOCK + V3.6.0.2 D1 + V3.6.0.X AUDIT-CLOSURE + V3.6.0.3 D2 + V3.6.0.X AUDIT-OF-D2 + V3.6.0.4 D3 + V3.6.0.X AUDIT-OF-D3 + V3.6.0.5 D4 + V3.6.0.X AUDIT-OF-D4 + V3.6.0.6 D5 + V3.6.0.X AUDIT-OF-D5 + V3.6.0.7 D6 SPIKE + V3.6.0.8.1 D6 LOCK + V3.6.0.8.2 ENGINE PART 1 + V3.6.0.8.3 ENGINE PART 2 napi + V3.6.0.8.4 AUDIT-OF-D6 + 5 HIGH CLOSURES + BENCH + V3.6.0.8.5 MOCHA + V3.6.0.10 D8 Op::RestoreSheet + V3.6.0.10 DOCS MEGAUDIT + V3.6.0.11 D9 typing-stroke watchdog SHIPPED 2026-05-23 → 05-26)** -- 9 D-decisions per V3.5.0.X Opus § F V3.6 ENTRY READINESS packet.  Status: D1-D6 + D8 + D9 SHIPPED; only D7 (#REF! substitution at V3.6.0.9) remains conditional pending user signal.  D6 perf contract delivered with 46× headroom (1.08 ms at 100 new cells vs 246 ms full snapshot).  D9 typing-stroke watchdog closes R-V3.5-7 false-negative.  V3.6 phase-termination megaudit unblocked at V3.6.0.11 ship.  D1 (Loro UndoManager `set_on_push` + `set_on_pop` callback wiring, HIGH PRIORITY): closes V3.5.0.X conservative `pure_local_frontier` gate properly via Loro 1.12.0's verified API (`OnPop = Box<dyn Fn(UndoOrRedo, CounterSpan, UndoItemMeta) + Send + Sync>` at registry/loro-internal-1.12.0/src/undo.rs:199); REMOVES `pure_local_frontier` field in V3.6.0.2.  D2 (session-wide RegisterFormat FormatTable cache): new `format_table_cache: HashMap<FormatId, Arc<str>>` field + new `CacheEffect::RegisterFormat` variant + new `FormatDefJson` napi struct + `WorkbookSnapshotJson.formats` additive field; unblocks D4 format-aware rendering.  D3 (per-cell op-index): new `cell_op_index: HashMap<(SheetId, RowId, ColId), Vec<usize>>` field; closes V3.5.0.6 R-V3.5-2 perf gap (invalidate_cell O(N) -> O(ops-for-this-cell)).  D4 (format-aware buildHtml rendering): engine-side `format_value` reuse via Phase 4.6 D-1 FormatTable; new `rendered: Option<String>` field on CellSnapshotJson; new `dateSystem: String` field on WorkbookSnapshotJson ("Excel1900" | "Excel1904"; String chosen over wrapper struct for simplicity; napi-rs auto-camelCases at JS boundary); workbook.locale() passthrough + per-snapshot parsed-format cache + Pending-skip per V3.6.0.X audit-of-D4 closures; CSP-safe.  D5 (IDE-facing `appendPutFormula` napi, NEW): thin wrapper over existing `Op::PutFormula`; unblocks end-to-end repaired-formula integration tests (closes V3.5.0.X A-HIGH-2 IDE-level verification gap).  D6 (incremental WorkbookSnapshot deltas): DEFER pending V3.6.0.7 mid-cycle profiling spike; ship only if cost > 50ms/call.  D7 (#REF! substitution): DEFER until user signal.  D8 (Op::RestoreSheet): DEFER until user signal.  D9 (sheet-tabs UX + mid-edit-render guard typing-stroke watchdog): DEFER sheet-tabs to V3.6+ multi-tab redesign per Opus § G; ship typing-stroke watchdog as V3.6.0.10 only if surfaced.  V3.6 sub-step rollout (actual + planned): V3.6.0.1 docs-only LOCK ✅; V3.6.0.2-0.6 priority ship (D1-D5) ✅; V3.6.0.7 profiling spike ✅ (verdict: D6 SHIPS); V3.6.0.8.1-8.5 D6 arc COMPLETE ✅ (D6 perf contract delivered with 46× headroom); V3.6.0.9 D7 #REF! conditional (PENDING user signal); V3.6.0.10 D8 Op::RestoreSheet ✅ SHIPPED + V3.6.0.10 docs megaudit ✅ SHIPPED; V3.6.0.11 D9 typing-stroke watchdog conditional (PENDING user signal); V3.6.0.X parallel Codex+Opus phase-termination megaudit + closures (PENDING at V3.6.0.11 ship; ~6-8 sessions).  Total estimated V3.6 wall time: 14-20 sessions.  **18 risks documented in plan as R-V3.6-1..18 + R-V3.6-NEW (the V3.6.0.7-spike-discovered rebuild_workbook dominant-cost risk, CLOSED-AT-V3.6.0.8.1 via option (a) cache rebuilt+repaired Workbook + clone-and-apply-delta)**.  All non-conditional risks closed; conditionals tied to D7/D9 surfacing.  Plan at `.plans/_active.md`.  ✅ **V3.6.0.2 D1 SHIPPED** at engine `89f5a5f9e0b`: REMOVED `pure_local_frontier` field + dispatch gate; ADDED `pending_undo_cells: Arc<Mutex<Option<Vec<(u16, u32, u32)>>>>` field + on_push closure encoding cells as LoroValue::List<List<I64>> into UndoItemMeta::value; undo/redo use top_undo_meta/top_redo_meta + pre-stage cells for synthetic inverse op's on_push; 2 V3.5.0.X regression tests REWRITTEN to verify partial-invalidate FIRES correctly (not full-rebuild fallback).  ✅ **V3.6.0.X AUDIT-CLOSURE SHIPPED** at engine `(THIS commit)`: Codex Lane A `docs/audits/2026-05-24-phase-5-7-v3-6-0-2-codex.md` (FAIL: 1 HIGH + 1 LOW + 2 INFO); Opus Lane B inline audit (PASS-WITH-CONVERGENT-FINDING).  Convergent HIGH (CODEX-V3-6-0-2-HIGH-1): grouped-undo + merge-interval-undo lose cells because Loro's `push_with_merge` discards subsequent meta when merging spans -- only the FIRST push's meta survives.  Codex empirically verified with `codex_tmp_grouped_undo_cache_probe` (4 stale cells remained post-undo); Opus independently confirmed by reading loro-internal-1.12.0/src/undo.rs:389-396.  Closure: new `inside_group: bool` + `undo_merge_interval_ms: i64` fields on CollabSession; start/end_undo_group + set_undo_merge_interval maintain these; append_op gates partial-invalidate behind `!inside_group && undo_merge_interval_ms == 0` (conservative full-rebuild fallback matches V3.5.0.6 semantics for grouped undo).  Clean fix (`set_top_undo_meta` to union cells) BLOCKED: API exists on loro_internal but not exposed on public `loro::UndoManager`; deferred to V3.6+ pending upstream contribution.  +2 ql-collab regression tests: grouped_undo_clears_cache_for_all_cells_in_the_group + merge_interval_undo_clears_cache_for_all_cells_in_window.  CODEX-LOW-1 (coverage expansion) DEFERRED to V3.6.0.3+ backlog.  Rule 4 arc terminus HELD at 6 throughout V3.6.0.2 + V3.6.0.X (new fields all Copy+Send+Sync trivially; 0 new triggers).  Cumulative V3.6 cycle through V3.6.0.X audit: +2 ql-collab tests (115 -> 117), IDE mocha unchanged at 346.  Audit transcripts: 33 (was 32; +1 V3.6.0.2 Codex Lane A).  Cumulative findings closed across Phase 5.7: 19+ HIGH + 50+ MEDIUM (was 18+ HIGH + 49+ MEDIUM).  ✅ **V3.6.0.3 D2 SHIPPED** at engine `ffa598a1c15` + IDE `b159c899359`: session-wide RegisterFormat FormatTable cache (new `format_table_cache: HashMap<FormatId, Arc<str>>` field on CollabSession + new `CacheEffect::RegisterFormat { id, string }` variant + walker integration; `apply_cache_effect` signature extended with 3rd param; all 3 callers updated; `force_clear_snapshot_cache` clears the cache; `rebuild_snapshot_cache` atomic-swaps; `invalidate_cell` uses throwaway local cache since RegisterFormat is session-wide) + new `FormatDefJson { id: FormatIdJson, string: String }` napi struct + `WorkbookSnapshotJson.formats: Vec<FormatDefJson>` additive field (populated from `Workbook.formats().iter()` -- authoritative; merges Builtin 0..=163 + Custom; avoids surfacing cross-peer cache-vs-Workbook divergence under concurrent RegisterFormat same-id-different-string).  IDE: `WorkbookSnapshotJson` interface gains required `formats: FormatDefJson[]` + new `FormatDefJson` interface; +4 mocha pinning shape; +1 stale V3.5.0.2 shape test updated; +10 V3.5.0.4b fixture additions for the now-required field.  +2 ql-collab regression tests: `register_format_op_populates_session_format_table_cache` + `register_format_cache_rebuilds_from_log_via_rebuild_snapshot_cache`.  Tests: ql-collab **119/119** (117 baseline + 2 new); IDE mocha **350/350** (346 baseline + 4 new).  Rule 4 arc terminus HELD at 6 (1 new ql-collab field + 1 new napi struct; positive Send+Sync via composition: `HashMap<K,V>: Send+Sync` iff both; `FormatId: Copy+Send+Sync`; `Arc<str>: Send+Sync`; `FormatDefJson` over `FormatIdJson` V3.5.0.5 walk + `String`; 0 new triggers).  Cumulative V3.6 cycle (through V3.6.0.3 D2): +4 ql-collab (115 -> 119); +4 IDE mocha (346 -> 350); audit transcripts still 33.  ✅ **V3.6.0.X AUDIT-OF-D2 SHIPPED** at engine `8480f247aea` + IDE `21fe6a73ec6` (2026-05-23).  Parallel Codex Lane A + Opus Lane B audit of V3.6.0.3 D2 surface; both PASS-WITH-FINDINGS.  Codex transcript: `docs/audits/2026-05-23-phase-5-7-v3-6-0-3-codex.md` (0 HIGH + 3 MED + 4 LOW + 6 INFO).  Opus transcript: `docs/audits/2026-05-23-phase-5-7-v3-6-0-3-opus.md` (1 HIGH + 3 MED + 4 LOW + 4 INFO).  3 convergent findings closed in-cycle: **CONVERGENT-HIGH-1** (Opus HIGH-1 ≡ Codex MED-3): cache walker LWW (`insert`) -> first-write-wins (`entry().or_insert`) mirroring `FormatTable::register_at`'s IdCollision rejection at the cache layer; closes the V3.6.0.1 plan body § D2 line 133 locked decision violation.  **CONVERGENT-MED-1** (Codex MED-1 ≡ Opus MED-2): `workbook_snapshot.formats` sorted by FormatId via the engine's derived Ord (Builtin variants first per enum order, then Custom lexicographically by `(peer, counter)`) before returning -- pre-closure was raw HashMap iter which varied across consecutive calls due to per-instance random hasher.  **CONVERGENT-MED-2** (Codex MED-2 ≡ Opus MED-1+MED-3): `collect_cache_effects` drops Builtin variant `Op::RegisterFormat` payloads at the cache walker (matches "ONLY Custom" docstring claim); Workbook-side `FormatTable::register_at` still accepts in-range Builtins as Phase 4.6 D-1 backward-compat (deferred V3.7+).  Codex INFO-1 REFUTED suspected undo-of-Op::RegisterFormat staleness bug empirically (Probe 1: non-cell-keyed ops trigger full-rebuild fallback via the V3.6.0.X audit-closure gate).  +5 ql-collab regression tests (Opus H.1 + H.2 + H.3 + H.5 + H.7).  Opus LOW-1 (CacheBuckets refactor) DEFERRED to V3.6.0.4 D3 ship.  Opus LOW-3 (Builtin 0..=163 docstring drift; ~20 actually preloaded) folded into 4-site docstring sweep.  Cumulative V3.6 cycle (through V3.6.0.X audit-of-D2): +9 ql-collab (115 -> 124); IDE mocha 350/350 unchanged (sort change is deterministic across existing shape tests).  Audit transcripts: 35 (was 33; +2 V3.6.0.X audit-of-D2 Codex + Opus).  Cumulative findings closed across Phase 5.7: 20+ HIGH + 53+ MEDIUM (was 19+ HIGH + 50+ MEDIUM).  Rule 4 arc terminus HELD at 6 (no new fields/variants/structs in audit-of-D2; behavior-only closures: `entry/or_insert` + `sort_by_key` + Builtin early-return).  ✅ **V3.6.0.4 D3 SHIPPED** at engine `119df9b3826` (2026-05-23).  Per-cell + per-sheet op-log index closes V3.5.0.6 R-V3.5-2 perf gap.  Engine surface: new `cell_op_index: HashMap<(u16,u32,u32), Vec<usize>>` + `sheet_op_index: HashMap<u16, Vec<usize>>` fields on CollabSession; new `CacheBuckets<'a>` struct bundling 5 mutable refs (closes Opus LOW-1 from V3.6.0.X audit-of-D2); new `OpLog::get(index)` accessor on ql-oplog (random-access lookup); `apply_cache_effect` signature refactored to `(&mut CacheBuckets<'_>, CacheEffect, op_log_index: usize)`; new `rebuild_op_indices_only()` helper called by undo+redo BEFORE the per-cell invalidate loop (necessary because Loro's UndoManager retract COMPACTS the visible op list and shifts positional indices -- design discovery during V3.6.0.4 implementation; documented as R-V3.6-10).  `invalidate_cell` rewritten to walk via `cell_op_index ∪ sheet_op_index` instead of full log walk: O(ops-for-this-cell) typical 1-3 + O(removes-for-this-sheet) typical 0-1 vs pre-D3 O(N).  `force_clear_snapshot_cache` clears the new indices; new `cell_op_index_iter` + `sheet_op_index_iter` accessors.  +6 ql-collab regression tests (130/130 pass; +6 over V3.6.0.X audit-of-D2 124 baseline).  Rule 4 arc terminus HELD at 6 (2 new HashMap fields + 1 struct with 5 ref fields; all positive Send+Sync via composition; 0 new triggers).  This session counted 3 plan-implement-audit cycles (V3.6.0.X audit-of-D2 + closures + V3.6.0.4 D3 ship), 1 over the CLAUDE.md ≤2 guideline; user-directed exception via AskUserQuestion.  ✅ **V3.6.0.X AUDIT-OF-D3 SHIPPED** at engine (this commit) (2026-05-23).  Parallel Codex Lane A + Opus Lane B audit of V3.6.0.4 D3 surface; both PASS-WITH-FINDINGS.  Codex transcript: `docs/audits/2026-05-23-phase-5-7-v3-6-0-4-codex.md` (0 HIGH + 2 MED + 4 LOW + 5 INFO).  Opus transcript: `docs/audits/2026-05-23-phase-5-7-v3-6-0-4-opus.md` (1 HIGH + 2 MED + 4 LOW + 3 INFO).  4 in-cycle closures: **CONVERGENT-HIGH-1** (Opus HIGH-1 / Codex LOW-2): `invalidate_cell` atomic-swap (build-into-fresh; defer `self.last_snapshot.remove(target_key)` until AFTER all fallible `OpLog::get` calls succeed).  Pre-closure the unconditional `remove()` at entry + fallible mid-walk Err produced a torn-write hazard.  **CONVERGENT-MED-2** (Codex MED-2 / Opus MED-1): `rebuild_op_indices_only` rewritten to use `apply_cache_effect` (canonical walker) with throwaway snapshot/format_cache buckets + local tombstones tracker; gives tombstone-aware index parity with `rebuild_snapshot_cache`.  Also added `cell_op_index.retain(|(sheet,_,_),_| *sheet != id)` to RemoveSheet handler (mirrors `snapshot.retain`; prunes ghost index entries for cells on tombstoned sheets).  **Codex MED-1**: `poll_remote_with_limit` partial-error rebuild -- track first drain error in the loop; if `merged > 0` before propagating, run `rebuild_snapshot_cache` to keep cache + indices consistent with the committed log state.  **Codex LOW-1**: `OpLog::get` docstring corrected from "O(1)" to "O(log N) BTree lookup" per Loro 1.12.0's internal `LengthFinder`-over-`generic_btree::BTree` implementation; invalidate_cell docstring updated to reflect the honest O(ops-for-this-cell * log N) bound.  ide-consumer-contract.md sweep: V3.6.0.4 D3 SHIPPED inline + R-V3.5-2 marked CLOSED.  +4 ql-collab regression tests (134/134 pass; +4 over V3.6.0.4 D3 130 baseline).  Codex INFO-3 + INFO-5 noted for V3.7+ (long-term stable-op-ID migration design; OpLog::append stores JSON-only payload + would need wire-format extension).  Rule 4 arc terminus HELD at 6 (no new fields/structs in audit-of-D3; behavior-only closures).  Cumulative V3.6 cycle: +19 ql-collab tests (115 -> 134); IDE mocha 350/350 unchanged; audit transcripts: 37 (was 35; +2 V3.6.0.4 Codex + Opus).  Cumulative findings closed across Phase 5.7: 21+ HIGH + 55+ MEDIUM (was 20+ HIGH + 53+ MEDIUM).  This session counted **5 plan-implement-audit cycles** (V3.6.0.X audit-of-D2 + closures + V3.6.0.4 D3 ship + V3.6.0.X audit-of-D3 + closures), 3 over CLAUDE.md ≤2 guideline; user-directed exception via AskUserQuestion + "Proceed next" directives.  ✅ **V3.6.0.5 D4 SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-23).  Format-aware buildHtml rendering via engine-side `ql_functions::format::render`.  Engine napi: new `CellSnapshotJson.rendered: Option<String>` additive field (populated when cell has format + value: format::parse the format-string -> render(value, fmt, eval_context) where eval_context.date_system = workbook.date_system() + locale = EnUs + now_provider = System).  Silent fallback to `None` on: no format / no value / format-id lookup miss / format-string parse error / wire-decode error.  New `WorkbookSnapshotJson.dateSystem: String` additive field ("Excel1900" | "Excel1904"; mapped from `ql_types::DateSystem`).  IDE: `types.ts` interfaces extended (CellSnapshotJson.rendered?: string + WorkbookSnapshotJson.dateSystem union); QuantbookCellSnapshot.entries[*].rendered passthrough; `extractSheetSnapshot` conditional include (only when rendered defined -- preserves exact-shape tests); `cellGridHtml::renderRows` + client-side `renderRowsClient` use `e.rendered ?? formatCellValue(e.value)` (escapeHtml covers both paths; CSP-safe).  +4 mocha shape tests (354/354 pass; +4 over V3.6.0.X audit-of-D3 350 baseline).  ql-collab 134/134 unchanged (no engine collab-side changes).  Rule 4 arc terminus HELD at 6 (1 new String field + 1 new Option<String> field on existing napi structs; 0 new triggers).  **mocha-test scope limitation**: full format::render integration tests (Op::RegisterFormat + Op::SetCellFormat + Op::PutValue -> rendered output) are deferred to V3.6.0.X audit-of-D4 -- the IDE napi at V3.6.0.5 D4 has no appendSetCellFormat / appendRegisterFormat method, so end-to-end mocha can only pin SHAPE.  Engine-side Rust integration tests TBD at audit-of-D4 closure.  Cumulative V3.6 cycle: ql-collab 134/134 + mocha 350 -> 354; audit transcripts still 37.  This session counted **6 plan-implement-audit cycles** (V3.6.0.X audit-of-D2 + closures + V3.6.0.4 D3 ship + V3.6.0.X audit-of-D3 + closures + V3.6.0.5 D4 ship), 4 over CLAUDE.md ≤2 guideline; user-directed exception via repeated "Proceed next" + "Ship D4 fully this session" AskUserQuestion answer.  ✅ **V3.6.0.X AUDIT-OF-D4 SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-24).  Most serious audit verdict in V3.6 cycle: Codex Lane A FAIL (1 HIGH + 2 MED + 3 LOW + 4 INFO) + Opus Lane B FAIL (3 HIGH + 3 MED + 4 LOW + 3 INFO).  Transcripts: `docs/audits/2026-05-24-phase-5-7-v3-6-0-5-codex.md` + `docs/audits/2026-05-24-phase-5-7-v3-6-0-5-opus.md`.  Convergent + in-cycle closures: **CONVERGENT-HIGH-1** (both lanes empirically probed): new `Op::SetDateSystem` wire variant + `DateSystemWire` enum (mirrors LocaleWire pattern) + replay handler + `from_qbook` seeds the op when loaded workbook's date_system != default.  Pre-closure Excel1904 workbooks rendered with Excel1900 epoch (1462-day off-by-one).  R-V3.6-11 documented.  **OPUS-HIGH-2** (Codex missed): new IDE `data-raw-value` attribute carries parseable raw representation for click-to-edit; `beginEdit` reads it first (falls back to `data-original-text` for backward compat).  Pre-closure `parseCellRawInput` NaN'd on currency/percent/thousands/date format-rendered strings, breaking every formatted-cell edit.  R-V3.6-12 documented.  **CONVERGENT-HIGH-3** (Opus HIGH-3 / Codex MED-1): short-circuit pre-render when `wire_value.is_pending()`; preserves IDE "(pending)" fallback.  Pre-closure pending+format displayed as formatted zero (Value::Blank routed as 0.0).  R-V3.6-13 documented.  **CONVERGENT-MED-1**: thread `workbook.locale()` through EvalContext (prevents silent forward-compat regression at V3.6.1+).  **CONVERGENT-MED-2**: per-snapshot parsed-format cache (HashMap<FormatId, FormatString>; reuses parse result across cells sharing FormatId).  **CONVERGENT-MED-3**: ide-consumer-contract.md sweep (R-V3.5-4 marked CLOSED; D4 "deferred" updated to SHIPPED).  **OPUS-LOW-2 + LOW-3**: dropped `.to_string()` on lookup result + TS docstring `date_system` -> `dateSystem`.  +2 ql-collab regression tests (Op::SetDateSystem replay + unknown-wire error).  Rule 4 arc terminus HELD at 6 (new DateSystemWire enum + Op::SetDateSystem variant; both Send+Sync via primitive composition; 0 new triggers).  Cumulative V3.6 cycle: ql-collab 134 -> 136; IDE mocha 354/354 unchanged; audit transcripts: 39 (was 37; +2 V3.6.0.5 Codex + Opus).  Cumulative findings closed across Phase 5.7: 24+ HIGH + 58+ MEDIUM (was 21+ HIGH + 55+ MEDIUM).  This session counted **8 plan-implement-audit cycles** (audit-of-D2 + closures + D3 + audit-of-D3 + closures + D4 + audit-of-D4 + closures), 6 over CLAUDE.md ≤2 guideline; user-directed exception throughout via "Proceed next" chain + AskUserQuestion "Close all in-cycle".  **V3.6.0.6 D5 IDE-facing appendPutFormula napi** (Opus § F.5, ~1 session) is the next recommended sub-step.  Baselines tracked: ql-collab **136/136** + IDE mocha **354/354** + ql-collab-ws 40/40 + engine workspace all 30 suites pass.  ✅ **V3.6.0.6 D5 SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-24).  Engine napi: new `CollabSession::appendPutFormula(sheet: u16, row: f64, col: f64, text: String) -> Result<()>` mirroring `appendPutValue` validation discipline (validate_u32_index on row/col + sheet u16 at FFI boundary); wraps existing `Op::PutFormula { sheet, row, col, text }` (shipping since Phase 4.6.x); no new wire variant; no new CacheEffect (the cell-keyed cache walker already handles PutFormula via `CacheEffect::Cell`).  IDE: `types.ts` adds `CollabSessionInstance.appendPutFormula(...)` + `QuantbookCellSnapshot.entries[*].formula?: string`; `session.ts` adds `appendPutFormulaValidated(session, sheet, row, col, text)` typed wrapper (validates sheet/row/col BEFORE the FFI ToUint32-coercion boundary; matches `appendPutValueValidated` pattern; text not validated since wire accepts any string); `extractSheetSnapshot` passes through `cell.formula` (additive; conditional-key discipline like `rendered` preserves V3.5.0.4b shape-stability tests); `cellGridHtml::renderRows` server-side emits `data-raw-formula="${escapeHtml(e.formula)}"` when entry has formula text; client-side `renderRowsClient` mirrors; **`beginEdit` precedence chain updated to `data-raw-formula > data-raw-value > data-original-text`** (extends V3.6.0.X audit-of-D4 OPUS-HIGH-2 § G.2 -- cells with both formula AND value have two raw representations; formula is the more-authoritative source for editing since it's what the user typed).  +19 mocha tests (354 -> **373**): 6 appendPutFormulaValidated input validation (negative sheet, sheet>u16, fractional row, oversize col, non-string text, accepts valid); 4 engine napi (opCount, fractional row reject, negative row reject, out-of-u32 col reject); 3 workbookSnapshot surface (formula-only cell -> formula property; rename-chain repair end-to-end; round-trip via to_qbook/from_qbook); 2 extractSheetSnapshot (formula+value pass-through; formula-absent shape stability); 4 buildHtml emit (with-formula attribute, without-formula td scope check, HTML-escape XSS, beginEdit precedence chain).  **V3.5.0.X A-HIGH-2 IDE-level verification gap CLOSED end-to-end** (engine-side Rust regression already verified the repair; now IDE-driven mocha verifies the rename-chain repair surfaces through napi `workbookSnapshot`).  Tests: ql-collab **136/136** unchanged (no engine collab-side changes); ql-oplog **67/67** unchanged; ql-collab-ws **42/42** unchanged (30 lib + 10 transport + 2 doctests); engine workspace **99 test-result lines pass**; IDE mocha **373/373** (+19 V3.6.0.6 D5 over V3.6.0.X audit-of-D4 354 baseline).  Rule 4 arc terminus HELD at 6 (napi method only; no new structs/fields/variants; 0 new triggers).  This session counted **1 plan-implement-audit cycle** (V3.6.0.6 D5 ship), WITHIN the CLAUDE.md ≤2 cycle guideline -- recovery from the V3.6.0.5 D4 / audit-of-D4 / docs-audit session's 9+ cycle bloat.

✅ **V3.6.0.X AUDIT-OF-D5 SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-24).  Opus Lane B transcript at `docs/audits/2026-05-24-phase-5-7-v3-6-0-6-opus.md` (FAIL: 3 HIGH + 3 MED + 4 LOW + 4 INFO).  Codex Lane A BLOCKED by OrbStack Mac bridge outage during the audit window; deferred to V3.7+ retrospective if user signal surfaces.  4 in-cycle closures + 1 bonus:
- **OPUS-HIGH-1** (IDE write-path for formulas didn't exist): `dispatchIncomingMessage`'s `'putValue'` arm gains `req.rawInput.trimStart().startsWith('=')` prefix detection; if formula, routes to `appendPutFormulaValidated`.  Webview envelope UNCHANGED (still `{type:'putValue', rawInput}`); host does the classification.  V3.5.0.X A-HIGH-2 closure framing is now true at the user-driven flow level, not just the napi-call level.
- **OPUS-HIGH-2** (`extractSheetSnapshot` silently dropped formula-only cells): pass-through formula-only cells (`cell.formula !== undefined && cell.value === undefined`) with `value: { kind: 'pending' }`.  Matches V3.6.0.X audit-of-D4 CONVERGENT-HIGH-3 pending-render strategy.  Pre-existing V3.5.0.4b skip-test updated.
- **OPUS-HIGH-3** (`appendPutFormula(sheet: u16, ...)` silent coercion): new `validate_u16_index` helper mirroring `validate_u32_index`; both `appendPutValue` + `appendPutFormula` change `sheet: u16` -> `sheet: f64` + call `validate_u16_index`.  Closes the V1 H2 silent-coercion class (napi-rs 3.9.0 ToUint32 + `try_into<u16>` silently coerces NaN/Infinity/fractional/2^32 to in-range u16).  Pre-closure docstring at lib.rs:876-878 wrongly claimed the class didn't apply at u16; corrected.
- **OPUS-MED-2**: ide-consumer-contract.md § 4.1.z6 sweep (status + HEAD trail + Tests baseline + D5 row PENDING -> SHIPPED + new V3.6.0.6 D5 + audit-of-D5 sub-sections + Out-of-scope removal).
- **OPUS-LOW-2** (bonus): validator suite coupling to engine binary -- added `suiteSetup(shouldSkip())` so CI hosts without the engine skip cleanly instead of failing.

Deferred to V3.6.0.7+ backlog: Opus MED-1 (client/server data-raw-formula guard asymmetry; safe today because client only emits in editable mode); LOW-1 (text contract docs); LOW-3 (engine napi test regex weak); LOW-4 (multi-peer + undo CRDT regression at IDE level); INFO-1..4.  +8 mocha tests (373 -> **381**): 3 dispatcher routing (formula prefix detected; leading-whitespace formula; non-formula stays on value path); 2 formula-only-cell pass-through (renders pending; data-raw-formula attribute emits); 3 u16 sheet rejection on both appendPutValue and appendPutFormula (NaN/Infinity/fractional/2^32).  ql-collab 136/136 unchanged.  Rule 4 arc terminus HELD at 6 (new `validate_u16_index` is a free function; sheet param type change is signature-only).

This session counted **2 plan-implement-audit cycles** (V3.6.0.6 D5 ship + V3.6.0.X audit-of-D5 closures), at the CLAUDE.md ≤2 cycle ceiling.

✅ **V3.6.0.7 D6 PROFILING SPIKE SHIPPED** at engine (this commit) (2026-05-25).  Time-boxed criterion bench at `crates/ql-bindings-node/benches/workbook_snapshot.rs` measuring post-D2 + post-D4 `workbookSnapshot` cost.  12 workload combinations across 1k / 10k / 100k cells × 0/50 % format × 1/5 sheets; Builtin format ids 1 / 9 / 14 rotated across formatted cells (no `Op::RegisterFormat` needed since Builtins pre-registered; no `appendSetCellFormat` exposed at napi V3.6.0.6 -- bench bypasses via `CollabSession::append_op` direct).  Bench replicates the body of `crates/ql-bindings-node/src/lib.rs:1802 workbook_snapshot` in pure Rust (napi can't be invoked from Cargo bench harness); setup-time assertion (cell count) guards bench-vs-napi drift.  Hardware: Apple M4 Max / macOS 26.2 / rustc 1.83.0 / criterion 0.8.2.  **Decision combo `cells=100000 fmt=50% sheets=1` median = 251.38 ms ≈ 5× the 50 ms threshold**; even the no-format 100k case is 152.40 ms (3× over).  Threshold breakpoint lies between 10k (25 ms) and 50k (127 ms) cells.  Scaling linear-to-slightly-superlinear in cell count (10× cells → 10.0-10.2× time); rebuild_workbook + sheet/table/column repair walks dominate.  D4 cost is real but not dominant: ~99 ms overhead at 100k cells for ~50k format::render + ~50k `Op::SetCellFormat` replays (~2 μs / render with the post-D4 parsed_format_cache).  **Decision: D6 SHIPS in V3.6.0.8.**  Implementation lock: `workbookSnapshotDelta(last_seen_version)` napi + `WorkbookSnapshotDeltaJson { changed_cells, removed_cells, sheets_changed, version }` (additive; `workbookSnapshot` retained for full-rebuild fallback) + R-V3.6-5 freshness fallback + R-V3.6-NEW rebuild_workbook strategy (cache prior rebuilt Workbook + diff forward, OR read from V3.4.0.2 incremental cache + skip rebuild when no rename-repair changed).  Files: new `crates/ql-bindings-node/benches/workbook_snapshot.rs` (~240 LOC) + `crates/ql-bindings-node/Cargo.toml` [dev-dependencies] + [[bench]] entry + `docs/audits/2026-05-25-phase-5-7-v3-6-0-7-spike.md` (spike transcript) + sweep across `.plans/_active.md` + this file + `docs/architecture/ide-consumer-contract.md § 4.1.z6`.  No engine source code (session.rs / lib.rs / op.rs) modified.  Tests: ql-collab **136/136** + ql-oplog **67/67** + ql-collab-ws 40/40 + engine workspace clean build (release).  IDE mocha **384/384** (3-baseline-undercount discovery: handoff said 381 over same HEAD; live count is 384 -- documentation drift only, no test regression).  Rule 4 arc terminus HELD at 6 (no new fields/variants/structs).  This session counted **1 plan-implement-audit cycle** (V3.6.0.7 D6 spike SHIP), WITHIN the CLAUDE.md ≤2 cycle guideline.  Next recommended sub-step: V3.6.0.8 D6 incremental snapshot deltas implementation (~3 sessions) per the V3.6.0.7 verdict; D6 implementation must include the rebuild_workbook cost strategy.

✅ **V3.6.0.8.1 D6 DESIGN LOCK SHIPPED** at engine (this commit) (2026-05-25).  Docs-only lock; no engine source modified.  Pattern mirrors V3.6.0.1 (the V3.6 phase-entry lock that landed all 9 D-decisions before V3.6.0.2 implementation).  Resolves the open R-V3.6-NEW question from V3.6.0.7 spike.  **Option (a) cache rebuilt+repaired Workbook + clone-and-apply-delta CHOSEN**; option (b) read-from-`last_snapshot`-and-skip-`rebuild_workbook` REJECTED because `last_snapshot.formula` carries PRE-rename-repair text (would silently regress V3.5.0.X A-HIGH-2 contract -- IDE would surface stale formula references after sheet rename); option (c) hybrid revalidate-changed-cells DEFERRED to V3.7+.  Locked napi surface: new `workbookSnapshotDelta(lastSeenVersion: Buffer) -> WorkbookSnapshotDeltaJson` method + new `WorkbookSnapshotDeltaJson { changedCells, removedCells, sheetsChanged, sheetsRemoved, formatsAdded, version, fullRebuildRequired }` interface.  Existing `workbookSnapshot()` retained for full-rebuild fallback (when `fullRebuildRequired=true`).  Two-call IDE discipline: empty `lastSeenVersion` Buffer returns `fullRebuildRequired=true`; IDE then calls `workbookSnapshot()` and caches the `version` token for subsequent delta calls.  Locked internal state on `CollabSession`: 2 new fields -- `last_snapshot_workbook: Option<Arc<Workbook>>` (post-repair) + `last_snapshot_oplog_vv: Option<VersionVector>`.  Both positive Send+Sync compositionally (`Workbook` auto-derives via field composition per `crates/ql-storage/src/workbook.rs:295-299`; `Arc<T>: Send+Sync` iff T; Loro `VersionVector` via `BTreeMap<PeerID, Counter>` [historical narrative; V3.6.0.8.4 OPUS-MED-1 corrected the actual type to `FxHashMap<PeerID, Counter>` per loro-internal-1.12.0/src/version.rs:29]).  **Rule 4 arc terminus stays at 6** -- no Rule 4 trigger.  Locked algorithm: 8-step body -- cache-miss + decode-VV + staleness-check + same-VV fast-path + op-enumeration-since-cached-VV + rename-detection branch + cell-only fast-path (Arc::make_mut clone + apply per-op + skip repair) OR rename-full-rebuild branch + JSON-delta-build.  Cell-only fast-path is steady-state IDE-editing happy path; any `Op::RenameSheet | RenameTable | RenameColumn` in the delta triggers full rebuild_workbook + repair walks (because repair touches ALL formula cells in the workbook, not just the renamed sheet -- verified at `crates/ql-collab/src/repair.rs:227-345`).  Locked invalidation triggers: `merge_bytes` + `discard_pending_ops` + `undo` + `redo` all call new `force_clear_workbook_cache()` helper; `append_op` does NOT invalidate (it's the cell-only fast-path target).  5 new risks documented R-V3.6-14..18 (cached-Workbook-stale-after-Loro-retract; VV-vs-op_count-drift; multi-peer-concurrent-rename-cell-edit; Workbook-clone-cost-may-exceed-20ms-at-100k-fallback-to-Arc-COW; apply_op-private-to-ql-oplog-prefer-new-`apply_ops_in_range(from, to)`-public-helper-on-OpLog).  R-V3.6-NEW (from V3.6.0.7 spike) closed as CLOSED-AT-V3.6.0.8.1.  Sub-step rollout: V3.6.0.8.1 ✅ docs-only lock; V3.6.0.8.2 = engine PART 1 (new fields + `force_clear_workbook_cache` + invalidation wiring at 4 callsites + new `apply_ops_in_range` helper on `OpLog` + ql-collab regression tests); V3.6.0.8.3 = engine PART 2 (`workbook_snapshot_delta` napi method + cell-only fast-path + rename-full-rebuild branch + staleness check + bench delta vs full snapshot at 100k cells + napi shape tests); V3.6.0.8.4 = V3.6.0.X audit-of-D6 (parallel Codex Lane A + Opus Lane B + closures).  D6 total arc estimate: 4 cycles across ≥2 sessions.  Files modified at V3.6.0.8.1: `.plans/_active.md` (V3.6.0.8.1 SHIPPED sub-section + frontmatter + R-V3.6-14..18 + R-V3.6-NEW resolution); this file (this paragraph); `docs/architecture/ide-consumer-contract.md § 4.1.z6` (D6 row + V3.6.0.8.1 sub-section preview with napi shape + IDE consumer protocol).  No engine source touched.  No new tests.  No IDE / mocha changes.  Tests baselines unchanged: ql-collab **136/136** + ql-oplog **67/67** + ql-collab-ws 40/40 (+2 ignored doctests) + IDE mocha **384/384**.  This session counted **1 plan-implement-audit cycle** (V3.6.0.8.1 docs lock), WITHIN the CLAUDE.md ≤2 cycle ceiling.

✅ **V3.6.0.8.2 D6 ENGINE PART 1 SHIPPED** at engine (this commit) (2026-05-25).  Lands the engine-side foundation for the V3.6.0.8.3 napi `workbook_snapshot_delta` per the V3.6.0.8.1 design lock; no new napi surface yet.  Two new fields on `CollabSession` -- `last_snapshot_workbook: Option<Arc<Workbook>>` (post-repair) + `last_snapshot_oplog_vv: Option<loro::VersionVector>` -- with `pub fn` accessors (`last_snapshot_workbook` / `last_snapshot_oplog_vv`) + setter (`set_workbook_cache(Arc<Workbook>, VersionVector)`) + invalidator (`force_clear_workbook_cache()` -- production, not test-gated).  Both fields default `None` in both `new` + `from_snapshot` constructors (per V3.6.0.8.1 lock: `from_snapshot` does NOT pre-populate the cache because rebuild_workbook + repair walks cost more than waiting until a consumer needs the result).  Invalidation wired at 4 production callsites: (1) `merge_bytes` (after rebuild_snapshot_cache; remote ops may shift LWW winners), (2) `discard_pending_ops` (after rebuild_snapshot_cache; fork_at_vv dropped ops), (3) `undo` (AT TOP, BEFORE Loro's undo per R-V3.6-14 -- retract may have included rename op whose effect would need un-repair), (4) `redo` (AT TOP, mirror of undo).  `append_op` does NOT invalidate (cell-only fast-path target; explicitly verified by `v3_6_0_8_2_append_op_does_NOT_invalidate_workbook_cache` regression test).  New public free function in ql-oplog: `pub fn apply_ops_in_range(log, workbook, from_index, to_index, registry) -> Result<usize, ReplayError>` (re-exported via `ql_oplog::apply_ops_in_range`; iterates `log.iter().skip(from).take(to-from)` and invokes the existing private `apply_op` per op; preserves `replay_into`'s from-empty contract; out-of-range to_index silently stops at log end; empty/inverted range = O(1) no-op returning 0).  V3.6.0.8.3 napi `workbook_snapshot_delta` will use this for the cell-only fast-path delta replay onto the cached Workbook clone.  `force_clear_snapshot_cache` test seam extended to clear the workbook cache too (cohesive test-state discipline).  +13 ql-collab regression tests (149/149 pass; +13 over V3.6.0.X audit-of-D5 136 baseline): 8 workbook cache invariants (fresh/from_snapshot empty; set populates atomic; force_clear resets atomic; merge_bytes/discard/undo/redo invalidate; append_op preserves; Arc identity preserved through setter) + 4 apply_ops_in_range correctness (empty noop; full-range matches replay_into; forward-from-partial composes correctly; oversized to_index saturates at log end) + 1 cache discipline (Arc ptr_eq through setter).  No new napi (V3.6.0.8.3 scope); no IDE / mocha changes (mocha baseline 384/384 unchanged); engine workspace release build clean.  Rule 4 arc terminus HELD at 6 (per the V3.6.0.8.1 lock: Workbook auto-derives Send+Sync via field composition at `crates/ql-storage/src/workbook.rs:295-299`; Arc<T>: Send+Sync iff T; VersionVector via Loro's `BTreeMap<PeerID, Counter>` [historical narrative; V3.6.0.8.4 OPUS-MED-1 corrected the actual type to `FxHashMap<PeerID, Counter>`]; Option<X>: Send+Sync iff X; 0 new triggers).  R-V3.6-14 (cached Workbook stale after Loro retract) CLOSURE LANDED via the undo + redo top-of-method clear discipline.  R-V3.6-15 (VV vs op_count drift) debug-assert deferred to V3.6.0.8.3 -- the invariant is "set together / clear together" not "monotonic drift" at this layer.  R-V3.6-17 (Workbook clone cost) profiled at V3.6.0.8.3.  R-V3.6-18 (apply_op private) CLOSED via the new `apply_ops_in_range` public helper.  This session counted **2 plan-implement-audit cycles** (V3.6.0.8.1 docs lock + V3.6.0.8.2 engine PART 1), AT the CLAUDE.md ≤2 cycle ceiling.

✅ **V3.6.0.8.3 D6 ENGINE PART 2 napi SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-25; user-directed cycle-3 override of the CLAUDE.md ≤2 cycle ceiling).  Lands the napi surface for incremental WorkbookSnapshot deltas per the V3.6.0.8.1 design lock.  New `WorkbookSnapshotDeltaJson` napi struct (`changedCells: Vec<ChangedCellJson>` + `removedCells: Vec<RemovedCellJson>` + `sheetsChanged: Vec<SheetSnapshotJson>` + `sheetsRemoved: Vec<u32>` + `formatsAdded: Vec<FormatDefJson>` + `version: Buffer` + `fullRebuildRequired: bool`) + 2 helper structs `ChangedCellJson { sheet, cell: CellSnapshotJson }` + `RemovedCellJson { sheet, row, col }`.  New `#[napi(js_name = "workbookSnapshotDelta")] CollabSession::workbookSnapshotDelta(lastSeenVersion: Buffer)` method implementing the V3.6.0.8.1 8-step algorithm: cache-miss + decode-VV + staleness + same-VV-fast-path + op-enumeration + rename-detection + cell-only-fast-path (Arc-deref-clone + ql_oplog::apply_ops_in_range + skip repair walks) + JSON-delta-build.  Rename ops trigger fullRebuild fallback (locked at V3.6.0.8.1 -- the rename delta would degenerate to "all cells with formulas" anyway).  Existing `workbookSnapshot` napi now populates the cache via `set_workbook_cache(Arc::clone(&workbook), oplog_vv())` after `rebuild_workbook`; `workbook` wrapped in `Arc` BEFORE rendering so no double-clone (rendering reads through Arc::deref).  Added `last_snapshot_op_count: Option<usize>` 3rd cache field (revision to V3.6.0.8.2's 2-field cache invariants -- delta path derives the replay slice `[op_count, log.len())` without a VV->op_count walk; preserves the "all Some / all None" invariant); extended `set_workbook_cache` (pins op_count alongside VV) + `force_clear_workbook_cache` (clears all 3 fields) + new `last_snapshot_op_count()` accessor.  New `pub fn oplog_vv(&self)` + `pub fn log(&self) -> &OpLog` accessors on `CollabSession` so the napi can grab the VV at populate time + enumerate ops at delta time.  `loro::VersionVector` re-exported as `ql_oplog::VersionVector` for ergonomics (avoids a direct loro dep on ql-bindings-node).  New free fns `empty_delta` + `classify_delta_op` at file scope (helpers; `classify_delta_op` recurses into `Op::BatchCommit` mirroring V3.4.0.X HIGH-1 walker discipline).  IDE: types.ts adds `WorkbookSnapshotDeltaJson` + `ChangedCellJson` + `RemovedCellJson` interfaces + `CollabSessionInstance.workbookSnapshotDelta(lastSeenVersion: Buffer): WorkbookSnapshotDeltaJson` method.  +6 mocha shape tests (390/390 pass; +6 over V3.6.0.X audit-of-D5 live 384 baseline -- the 381 documented baseline was a 3-count undercount): empty Buffer + no-cache + same-VV fast-path + cell-only emits changes + rename triggers fullRebuild + undo invalidates cache.  Tests: ql-collab 149/149 unchanged (V3.6.0.8.2 baseline preserved; new op_count field naturally pinned by existing set_workbook_cache test calls) + ql-oplog 67/67 + IDE mocha **390/390** + engine workspace release build clean.  Rule 4 arc terminus HELD at 6 (all new napi struct fields + 1 new `last_snapshot_op_count: Option<usize>` cache field are positive Send+Sync composition; 0 new triggers).  V3.6.0.8.3 scope limitations: `removedCells` always empty (V3.7+); `sheetsChanged` always empty in cell-only path (cell ops don't change sheet metadata; full-rebuild paths return `fullRebuildRequired=true` instead); rename ops trigger fullRebuild rather than incremental rename delta.  Bench delta-vs-full at 100k cells DEFERRED to V3.6.0.8.4 audit-of-D6 (functional + regression test surface is complete at V3.6.0.8.3; perf verification + R-V3.6-15 full debug-assert + R-V3.6-17 clone-cost profile all naturally fit into V3.6.0.8.4's audit cycle).  This session counted **3 plan-implement-audit cycles** (V3.6.0.8.1 docs lock + V3.6.0.8.2 engine PART 1 + V3.6.0.8.3 napi), 1 over the CLAUDE.md ≤2 cycle ceiling; **user-directed exception** via AskUserQuestion at cycle 3 entry.

✅ **V3.6.0.8.4 D6 AUDIT-OF-D6 + CLOSURES + BENCH SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-25; user-directed cycle-4 override, 2 over CLAUDE.md ≤2 ceiling).  Closes V3.6.0.8 D6 arc + ships the D6 perf contract.  Codex Lane A FAIL (3 HIGH + 1 LOW + 1 INFO) + Opus Lane B PASS-WITH-FINDINGS (3 HIGH + 5 MED + 3 LOW + 1 INFO) audit transcripts at `docs/audits/2026-05-25-phase-5-7-v3-6-0-8-{codex,opus}.md`; in-cycle closure summary at `docs/audits/2026-05-25-phase-5-7-v3-6-0-8-closures.md`.  **5 HIGH closed in-cycle**: (1) CONVERGENT-HIGH-1 (Codex-HIGH-2 ≡ Opus-HIGH-1) — `classify_delta_op` allowlist for metadata ops (silent data loss on AddSheet / MoveSheet / CreateTable / DropTable / ResizeTable / SetName / SetLocale / SetReferenceMode / SetDateSystem; pre-closure these were caught by `_=>{}` catch-all + ALSO advanced the cache token so IDE got empty delta + new version while engine had new sheets); (2) Codex-HIGH-1 — `poll_remote_with_limit` was a 5th log-mutation site missed by V3.6.0.8.2 invalidation discipline (force_clear_workbook_cache after rebuild_snapshot_cache in the `merged > 0` block); (3) Codex-HIGH-3 — new `pub fn snapshot_cell(sheet, row, col) -> Option<CellState>` O(1) HashMap.get accessor on CollabSession replacing the O(cells_in_sheet) `snapshot_cells(sheet).find()` per delta cell; (4) Opus-HIGH-2 — new `version: Buffer` field on `WorkbookSnapshotJson` napi struct (populated from `cache_vv.encode()`) + TS-optional in `WorkbookSnapshotJson` interface (so fixture-literal unit tests aren't forced to include opaque placeholders; napi-side always populated); (5) Opus-HIGH-3 — **the primary perf bug**: `OpLog::iter().skip(N).take(M)` was deserializing ALL N skipped ops on every delta call; fix is `for i in start..end { log.get(i)? }` (K * O(log N) BTree random-access).  Applied to both `apply_ops_in_range` (crates/ql-oplog/src/replay.rs) + the napi delta enumeration (crates/ql-bindings-node/src/lib.rs).  **4 of 5 MED closed in-cycle**: MED-1 BTreeMap → FxHashMap docstring drift (3 sites; actual Loro 1.12.0 type is `FxHashMap<PeerID, Counter>` per loro-internal-1.12.0/src/version.rs:29); MED-2 `formats_added` sort by full `FormatId` (engine-side derived Ord: Builtin first, then Custom by (peer, counter)) instead of just `kind` string; MED-3 `Arc::make_mut` docstring drift (V3.6.0.8.1 lock said make_mut; V3.6.0.8.3 ships `(*cached_arc).clone()` always-clone -- correct because cached_arc capture has strong_count >= 2 so make_mut would clone anyway); MED-4 `Workbook::clone()` cost profile via new `bench_workbook_clone` micro-bench (1k=7 μs / 10k=69 μs / 100k=692 μs; well under 20 ms threshold; R-V3.6-17 CLOSED).  Deferred to V3.6.0.8.5+: Opus-MED-5 comprehensive mocha coverage (multi-peer / undo / redo / AddSheet-in-delta / SetCellFormat-in-delta / BatchCommit-in-delta / chained-delta regressions); Opus-LOW-1 empty-stack undo/redo over-invalidation; Opus-LOW-2 per-accessor 3-field debug_assert (the primary R-V3.6-15 monotonicity assert ships at V3.6.0.8.4 lib.rs:2461-2475); Opus-LOW-3 `set_workbook_cache` API tightening (caller-supplied VV → method-read both atomically).  **R-V3.6-15 CLOSED** (monotonicity `debug_assert_eq!(new_ops.len(), current_op_count - cached_op_count)` after op enumeration).  **R-V3.6-17 CLOSED** (clone cost measured well under threshold).  Bench `crates/ql-bindings-node/benches/workbook_snapshot.rs` extended with `bench_workbook_snapshot_delta` (1 / 10 / 100 new cells × 100k initial workload) + `bench_workbook_clone` (1k / 10k / 100k cells).  **D6 perf contract achieved with 46× headroom**: 100 new cells delta median = **1.08 ms** vs full snapshot baseline 246 ms (228× faster than full; 46× under the 50 ms threshold).  1 new cell delta = 1.01 ms; 10 new cells = 1.00 ms.  Pre-closure 100 new cells = 741 ms (3× SLOWER than full snapshot); the Opus-HIGH-3 OpLog::get fix + Codex-HIGH-3 snapshot_cell fix together delivered the 230× improvement.  Tests: ql-collab **149/149** unchanged + ql-oplog **67/67** unchanged + ql-collab-ws **42/42** unchanged + IDE mocha **390/390** (2 shape-pinning tests updated to include the new `version` field; net delta 0) + engine workspace release build clean.  Rule 4 arc terminus HELD at 6 (the new `version: Buffer` field is positive Send+Sync via napi-rs's `Vec<u8>` wrapper composition; no new triggers).  **D6 ARC COMPLETE.**

✅ **V3.6.0.8.5 D6 Opus-MED-5 mocha coverage SHIPPED** at IDE (this commit) (2026-05-25; user-directed cycle-5 override, 3 over CLAUDE.md ≤2 ceiling).  Closes the Opus-MED-5 finding deferred from V3.6.0.8.4 (comprehensive mocha coverage gap).  +10 mocha regression tests (400/400 pass; +10 over V3.6.0.8.4 390 baseline): merge_bytes invalidation (multi-peer); malformed Buffer fullRebuild; AddSheet-in-delta fullRebuild (CONVERGENT-HIGH-1 end-to-end); MoveSheet-in-delta fullRebuild; RemoveSheet-in-delta sheetsRemoved (cell-only-deltable, NOT fullRebuild); chained delta; redo invalidation (R-V3.6-14 mirror); appendPutFormula-in-delta surfaces formula; renameSheet cross-sheet rename-arm; `workbookSnapshot().version` round-trip via `workbookSnapshotDelta` same-VV fast-path (OPUS-HIGH-2 end-to-end).  Pure-mocha cycle; no engine source modified.  Tests: ql-collab 149/149 + ql-oplog 67/67 + ql-collab-ws 42/42 unchanged; IDE mocha **400/400** (+10).  Rule 4 arc terminus HELD at 6.  This session counted **5 plan-implement-audit cycles** (V3.6.0.8.1 docs lock + V3.6.0.8.2 engine PART 1 + V3.6.0.8.3 napi + V3.6.0.8.4 audit + closures + bench + V3.6.0.8.5 mocha), **3 over the CLAUDE.md ≤2 cycle ceiling**; all 3 overrides user-directed via AskUserQuestion at cycle 3 entry + cycle 4 entry + cycle 5 entry.

✅ **V3.6.0.10 D8 Op::RestoreSheet SHIPPED** at engine (this commit) + IDE (this commit) (2026-05-25; user-directed cycle-6 override, 4 over CLAUDE.md ≤2 ceiling).  Closes the V3.6.0.1 D8 conditional ("DEFER until user signal"; user directed via AskUserQuestion at cycle 6 entry).  New `Op::RestoreSheet { id: SheetId }` wire variant in ql-oplog (additive serde-tagged enum; no schema bump per V3.5.0.3b precedent).  New `Workbook::restore_sheet(id)` method (HashSet::remove on `removed_sheets`; out-of-range silently dropped; idempotent).  Reverses the V3.5.0.3b tombstone: cells written BEFORE the original `Op::RemoveSheet` are preserved by the V3.5.0.3b storage discipline and reappear; cells silently-no-op'd while tombstoned do NOT reappear.  Cross-peer convergence: HashSet::remove on absent is no-op; concurrent {RemoveSheet, RestoreSheet} resolved by Loro's causal-merge order.  New `Op::RestoreSheet` replay arm in ql-oplog/src/replay.rs.  New `CacheEffect::RestoreSheet { id }` on CollabSession's cache walker + `collect_cache_effects` arm + `apply_cache_effect` arm (removes id from `tombstones` HashSet; does NOT re-add pre-tombstone cells to `last_snapshot` — full rebuild paths handle that).  V3.6.0.8.4 `classify_delta_op` allowlist extended to force `fullRebuildRequired=true` on `Op::RestoreSheet` (cache dropped pre-tombstone cells; Workbook preserved them; fullRebuild is the cheapest path).  New `#[napi(js_name="restoreSheet")] fn restore_sheet(id: u32)` napi method (mirrors deleteSheet validation: id <= u16::MAX + pre-rebuild existence check; idempotent on already-restored).  IDE types.ts adds `restoreSheet(id): void` to CollabSessionInstance + session.ts adds typed wrapper.  +5 ql-collab regression tests (149 -> 154): untombstone in workbook + idempotent (double-restore + restore-on-never-removed) + out-of-range no-op + cache drops tombstone (post-restore cell writes land in cache) + remote-merge convergence (peer A removes + peer B restores + merge -> both peers converge to restored).  +6 IDE mocha tests (400 -> 406): napi contract (1 op, out-of-range bad_arg, u16 overflow bad_arg, restored sheet visible in workbookSnapshot.sheets with name preserved, idempotent) + workbookSnapshotDelta fullRebuild on RestoreSheet.  Rule 4 arc terminus HELD at 6 (new wire variant + new CacheEffect variant; both positive Send+Sync via primitive composition; 0 new triggers).  R-V3.6-9 documented (`#REF!` substitution + Op::RestoreSheet interaction): D7 not shipped so interaction is moot today; if D7 ships V3.6.0.9 later, RestoreSheet must precede the rename-repair pass on rebuild_workbook (existing replay order verified).  Tests: ql-collab **154/154** + ql-oplog **67/67** + ql-collab-ws 42/42 + IDE mocha **406/406** + engine workspace release build clean.  This session counted **6 plan-implement-audit cycles** (V3.6.0.8.1 through V3.6.0.8.5 + V3.6.0.10), **4 over the CLAUDE.md ≤2 cycle ceiling**; all 4 overrides user-directed via AskUserQuestion at cycle 3, 4, 5, 6 entry.

✅ **V3.6.0.10 docs megaudit + sweep SHIPPED** at engine `503832303e9` (2026-05-25/26; fresh-window session cycle 1 of 2, within CLAUDE.md ≤2 ceiling).  3-lane megaudit (Codex Lane A + Opus Lane B + claude-self Lane C) at V3.6.0.10 ship found 3 HIGH + 9 MED + 7 LOW + 5 INFO across docs/handoffs.  Transcripts: `docs/audits/2026-05-25-phase-5-7-v3-6-0-10-megaudit-docs-{codex,opus}.md`.  Full convergent sweep closed all HIGHs + 7/9 MEDs + 7 LOWs in-commit: ide-consumer-contract.md § 4.1.z6 status/HEAD/tests/decision-table/risk-register/transcripts/out-of-scope + 5 new sub-sections (V3.6.0.8.2/8.3/8.4/8.5/10 + 10 docs megaudit); .plans/_active.md frontmatter rewritten + direction normalized + V3.6.0.8.4/8.5 chronology fixed + D7/D8/D9 design subsections re-numbered (D7→V3.6.0.9, D8→V3.6.0.10 SHIPPED, D9→V3.6.0.11); MASTER-PLAN top-line summary normalized + 18-risks claim corrected (was 13); MEMORY.md line 2 compounded-handoff drift cleared (~14k → ~300 chars per CLAUDE.md "index not memory" rule); risk register R-V3.6-9/14/16/18 status-normalized + R-V3.6-15 debug_assert citation corrected (lib.rs:2454 → lib.rs:2604); Arc::make_mut docstring drift swept in 2 source files (session.rs:976 + lib.rs:2453) + 5 doc sites; BTreeMap → FxHashMap drift inline-corrected in 3 historical-narrative paragraphs.  Deferred to V3.6.1+: condense `.plans/_active.md` frontmatter status field wall-of-text further (LOW; readability only).  Pure doc sweep + 2 in-source docstring fixes; no engine behavior or IDE changes; tests baseline UNCHANGED (ql-collab 154 + ql-oplog 67 + IDE mocha 406 + workspace clean).  Rule 4 arc terminus HELD at 6.

✅ **V3.6.0.11 D9 typing-stroke watchdog SHIPPED** at IDE `aa9d0448bb1` (2026-05-26; fresh-window session cycle 2 of 2, within CLAUDE.md ≤2 ceiling).  Closes R-V3.5-7 "long formula entry hits 30s" false-negative from V3.5.0.X audit closures.  Pure IDE/TypeScript work; no engine changes (V3.6.0.1 D9 design lock: Rule 4 trigger NONE; arc terminus HELD at 6).  New `typing_stroke` envelope arm in `dispatchIncomingMessage` (cellGridLogic.ts) + new optional `onTypingStroke?: () => void` field on `DispatchDeps` interface.  New `pub resetTypingWatchdog()` method on `CellGridPanel` (cellGridPanel.ts) -- re-arms the `presenceTypingWatchdog` setTimeout for `PRESENCE_TYPING_WATCHDOG_MS` (30_000ms) IFF `_presenceRepaintInFlight === true`; no-op otherwise (defensive: stray typing_stroke from a webview-endEdit race must NOT promote false → true).  Panel wires `onTypingStroke: () => this.resetTypingWatchdog()` in `handleIncoming` dispatch call with `_disposed` guard mirroring `onLocalTyping`.  Webview `beginEdit` (cellGridHtml.ts) adds `input.addEventListener('input', ...)` that calls `vscode.postMessage({type:'typing_stroke'})` on every text-change input event (typing/paste/deletion -- NOT focus, since the 'input' event doesn't fire on focus or programmatic value sets); guards with `activeInput !== input` so stale closures from prior edits no-op; cleanup is implicit via input DOM removal + GC.  +7 mocha tests (406 → 413): single envelope fires once + sequence of 5 fires 5 times + no errorReply (fire-and-forget) + extra payload fields accepted + onTypingStroke optional doesn't crash + unknown type still warns + onTypingStroke does NOT fire onLocalTyping (separate concerns).  Panel-level watchdog re-arm behavior (real setTimeout interaction) NOT covered by mocha here (requires vscode mocking + fake timers; deferred to V3.6.0.X phase-termination megaudit).  Tests: ql-collab 154/154 + ql-oplog 67/67 + ql-collab-ws 42/42 + IDE mocha **413/413** + engine workspace release build clean (engine unchanged).  R-V3.5-7 CLOSED-AT-V3.6.0.11.  **V3.6 phase-termination megaudit now UNBLOCKED**: all V3.6 D-decisions SHIPPED except V3.6.0.9 D7 #REF! substitution (conditional pending user signal -- phase-termination audit can proceed against V3.6 as-shipped; D7 can ship post-megaudit if surfaced).

✅ **V3.6.0.X phase-termination 3-lane megaudit + closures SHIPPED** at engine `<this commit>` + IDE `<companion>` (2026-05-26).  Codex Lane A at engine `c1da1a92bd6` (FAIL; 1 HIGH + 1 LOW + 9 INFO; CODEX-PT-A1); Opus Lane B at engine `51149253383` (FAIL; 1 HIGH + 4 MED + 5 LOW + 5 INFO; OPUS-PT-B1 convergent with CODEX-PT-A1); claude-self Lane C synthesis at `docs/audits/2026-05-26-phase-5-7-v3-6-0-X-phase-termination-closures.md`.  **CONVERGENT-HIGH-1 closed in-cycle**: RestoreSheet preserved-cells gap.  Pre-closure `CacheEffect::RemoveSheet` pruned cells from `CollabSession.last_snapshot` + `CacheEffect::RestoreSheet` did not rehydrate -- 6 docstrings claimed "cells reappear via workbookSnapshot full rebuild" but the implementation contradicted at all 6 (op.rs Op::RestoreSheet, lib.rs napi restore_sheet x2 blocks, types.ts JSDoc x2 blocks, ide-consumer-contract.md § 4.1.z6 V3.6.0.10).  Closure: walker no longer prunes cells/cell_op_index on RemoveSheet (cache mirrors V3.5.0.3b Workbook storage-preservation discipline; pre-tombstone cells preserved + resurfaced automatically on Op::RestoreSheet).  Visibility filters: existing `is_sheet_removed` in `workbook_snapshot` + new `removed_sheet_ids` filter in `workbook_snapshot_delta` `changedCells` build + new `removed_sheets.contains` filter in `list_sheets_from_cache`.  V3.6.0.X audit-of-D3 CONVERGENT-MED-2 "ghost entry prune" semantic OBSOLETED (entries are not ghosts; they're real pointers the rebuild path respects).  OPUS-PT-B2 inline Arc::make_mut comment drift at lib.rs:2646-2652 swept (V3.6.0.8.4 OPUS-MED-3 closure was partially complete -- 4 sites swept; inline comment missed).  OPUS-PT-B3 5-site contract drift auto-closed (docstrings now match implementation post-fix).  OPUS-PT-B4 + B5 test gaps closed (+4 ql-collab regression tests + 2 IDE mocha tests).  CODEX-PT-F3 + OPUS-PT-B6 + B7 current_work.md tail drift swept.  R-V3.6-19 added to risk register (CLOSED-AT-V3.6.0.X-phase-termination).  Deferred to V3.6.1+/V3.7+ per Opus recommendation: OPUS-PT-B8 (producer/replay asymmetry on restoreSheet/deleteSheet napi; design observation) + B9 (`WorkbookSnapshotDeltaJson.removedCells` field always empty; V3.7+ feature) + B10 (no IDE workbookSnapshotDelta consumer in production code; D6 engine perf shipped, IDE wiring pending).  Tests final: ql-collab **158/158** (+4 over 154 V3.6.0.11 baseline) + ql-oplog **67/67** + ql-collab-ws **42/42** + IDE mocha **415/415** (+2 over 413 V3.6.0.11 baseline) + engine workspace release build clean.  Rule 4 arc terminus HELD at 6 (no new types; existing CacheEffect::RemoveSheet semantic narrowed).  **V3.6 PHASE TERMINATION CLEAN.**  Cycle 4 of 2 this session (2 over CLAUDE.md ≤2 ceiling; user-directed "proceed").
...
8. **5.8 Phase 5 Megaudit** — ✅ **EXECUTED 2026-05-26 (PASS-WITH-FINDINGS)**. 5-way (Codex Lane A empirical/convergence + Opus Lanes B/C/D invariants/interaction/contract + claude-self synthesis) at source `1465b1db4c4`. **ZERO reachable HIGH**; risk register **39/39 confirmed** (0 falsely-closed); all 4 exit criteria met for the reachable surface. Lane A (Codex) voted FAIL on three table-convergence HIGHs (concurrent CreateTable/RenameTable/RenameColumn abort `rebuild_workbook`), but these were proven **UNREACHABLE** — no collaborative table producer exists (the only table producers are single-writer `WorkbookRuntime` in `ql-exec`, not composed into `CollabSession`). Convergent theme: latent replay-conflict aborts / empty-deltas, each gated on a producer that does not exist yet (tables, format-id RegisterFormat, ClearFormula). Canonical record: `docs/phase5/megaudit-5-8/closures.md` (synthesis) + `docs/phase5/phase-5-exit-packet.md` + 4 lane transcripts `docs/phase5/megaudit-5-8/lane-{a,b,c,d}.md`. **Post-exit polish (confirmed-but-latent, next session):** `export_snapshot` napi tombstone filter (completes R-V3.6-19 — it missed `export_snapshot` + the raw `snapshot_cells` accessor); `CellValueJson` discriminated-union retype (TS); collab `merge_bytes` convergence + wire round-trip coverage gaps.
...
- `ql-collab` is real. ✅ (5.8 megaudit: 10.7k LOC, real Loro CRDT, 154 lib + 101 integration = 255 tests.)
- Cells, formulas, and sheets merge deterministically. ✅ (5.8 megaudit verified; CONVERGENT-HIGH-1 RemoveSheet/RestoreSheet closed.) **Names: ⚠️ verified one layer removed** — at the replay / WorkbookRuntime layer, NOT the collaborative `merge_bytes` path (no collab `SetName` producer-conflict path exists; session mega-audit Lane O6). Acceptable for v1 (no reachable collab divergence), but not full collab-path coverage. **Tables: collaborative merge DEFERRED** (criterion amended 2026-05-26 per the 5.8 megaudit) — concurrent table create/rename/rename-column hard-fails at replay (documented "V1 LIMITATION"), but this is UNREACHABLE: no collaborative table producer exists (tables are single-writer `WorkbookRuntime`-only, not wired into `CollabSession`). Collaborative table editing arrives with a collaborative table producer (Engine Phase 6+ / collab = Product Phase 9 = v1.5-deferred), which MUST land conflict-resolution (stable table/column IDs, non-aborting merge) + `removedCells` emission in the same change. Tracking: `docs/phase5/megaudit-5-8/closures.md` §3/§5/§6.
- Offline sync and conflict diagnostics work. ✅ (5.8 megaudit: 8 offline tests; structured `errorReply` diagnostics.)
- Single-writer op log is no longer confused with collaboration. ✅ (5.8 megaudit: single-writer `WorkbookRuntime` is architecturally separate from collaborative `CollabSession` — the megaudit's central table finding turns on this separation holding.)
...
**✅ DECISION-LOCKED 2026-05-26** (Codex-validated): authoritative locked decisions + sequence at `docs/phase6/decision-lock.md`. Lock = **wedge-first, staged**: 6.1A session-API contract → 6.1B owning `WorkbookSession` → 6.1C security audit → 6.4-0 function-metadata substrate → 6.4A MVP Python UDF + minimal `quantbook-py` authoring slice (proves the Month-6 kill gate) → 6.4B UDF hardening → 6.3 full bindings → 6.2 service (defer HTTP-vs-gRPC) → 6.5 SQL/connectors → 6.6 AI (sentinel; `=AI()` cell fn is v2-aligned) → 6.7 audit. Entry-readiness analysis: `docs/phase6/entry-plan.md`. Active plan: `.plans/_active.md`.

**✅ 6.1A + 6.1B inc.1/1b + inc.2a/2b/2c-1/2c-2 SHIPPED 2026-05-26.** Contract: `docs/api/session-api.md` v2 (API6-01/02/03; Codex gpt-5.5 xhigh REVISE → all 13 findings resolved; review `docs/api/codex-6-1a-review.md`). Contract crate: `ql-session` (`38108a5c8ef`) — complete type-level `EngineSession` trait + DTOs + EngineError + operation/lifecycle + table ops. **Owning `WorkbookSession` SHIPPED** in `crates/ql-exec/src/session.rs` `impl EngineSession` (`83b1b33bac2` PlanCache → `7335a5a1bfa` core → `993492b6f9b` validate_formula/query_range/`CellValue::Blank` → `2b5e7a13f5b` structure+table ops → `9f7a1645dbd` tombstone-read-consistency audit fix) — REAL: lifecycle + `{epoch,op_count}` 24-byte version token + single-cell mutations + add/rename/**delete/restore/move** sheet (fail-loud) + set_name + **table ops** (create/rename/rename_column/resize/drop) + recalc (Busy/op-registry, CellDiagnostic events) + read (snapshot/cell/list_sheets/query_range/validate_formula; tombstoned sheet = uniformly NotFound) + cancel/operation_status/poll_events; error mapping via free fns (orphan rules forbid `From<RuntimeError> for EngineError`). 17 session tests + 653 ql-exec lib green, clippy clean. Remaining trait methods surfaced `Capability/not_implemented_in_v1_core` (No-Fallbacks). Doc fix: ql-oplog::OpLog DOES expose a Loro VV (`oplog_vv`) — earlier "no VV" claim corrected; `{epoch,op_count}` is the deliberate v1 token. NEXT = 6.1B inc.2c-3 (snapshot_delta) → batch/txn/undo/redo/persistence → functions/bulk → Node smoke migration → 6.1C audit. **(Superseded — inc.2c-3 through inc.2c-8 all SHIPPED 2026-05-27; current NEXT = persistence; see the inc.2c-8 entry below + `docs/api/workbook-session-impl-plan.md` §0.)**

**✅ 6.1B inc.2 AUDIT (F3–F10) + inc.2c-3 snapshot_delta SHIPPED 2026-05-27.** Parallel Codex (gpt-5.5 xhigh) + Opus audit of `WorkbookSession` (synthesis `docs/audits/2026-05-27-inc2-session-audit/SYNTHESIS.md`; 10 distinct findings, all verified at source). **Audit-fix `879f3601747`**: F3 `with_runtime` FaultGuard (panic→Faulted, the in-engine half of the §8 boundary); F4 read-path coord bounds (query_range/cell/validate_formula → BadArgument, closes a u32 overflow); F5 tombstone op-log fidelity (double-delete = true no-op; restore-of-live = Conflict); F6 `require_live_sheet` on rename_sheet + create_table; F7 query_range include-options fail-loud (Capability); F8 Appendix-A honest mapping; F9 deleted-sheets-formula-readable doc; F10 corrected misleading tables.rs atomicity comment (the BatchCommit fix is TRACKED — pre-existing, near-zero reachability). **inc.2c-3 `20427b1c4c7`**: `snapshot_delta` via design (a) — a session change-log keyed by a monotonic **`state_seq`** that REPLACES `oplog.len()` as the token counter (advances on every committed mutation AND every recompute-with-changes + Blank-clear, so distinct states never share a token; closes Codex HIGH F1+F2-token-half). `RecomputeResult` gained `changed_cells`. §4.3 rules + fail-loud `invalid_version_token`; epoch-bump for delta-inexpressible states (move/restore sheet, table ops). **ql-exec lib 667/0, clippy clean, workspace `cargo check` green.** NEXT = batch/txn → undo/redo → persistence → functions/bulk → Node smoke → 6.1C audit. **(Superseded — see the inc.2c-8 entry below for current NEXT = persistence.)**

**✅ 6.1B inc.2c-4 `batch` SHIPPED 2026-05-27** (`58a55f4cfb8` + fix `9d471be3663`). `EngineSession::batch(ops, options)` via **option (a)**: validate-all up front → append ONE `Op::BatchCommit` before mutating → apply via a graph-maintaining runtime with the op-log DETACHED (new additive `WorkbookRuntime::with_session_state_no_oplog`, so workbook+calcgraph update but no second per-op append) → `record_changes` once. All four `SessionOp` variants (SetValue/SetFormula/Clear/SetFormat); validation-atomic; calcgraph stays live (proven by a batch→recalc_dirty test). **Built by a delegated Opus agent; a self-review caught + fixed a real latent replay-divergence** — same-cell value/formula ops in one batch (e.g. `[SetFormula A1, SetValue A1]`) built the log from pre-batch state but applied sequentially, and `Op::PutValue` replay does not clear a formula (`replay.rs:486-510`), so a replay would diverge; FIXED by rejecting >1 value/formula op per cell in a batch → `Conflict/conflicting_batch_ops` (`SetFormat` stays orthogonal). **ql-exec lib 681/0, clippy clean, workspace green.**

**✅ 6.1B inc.2c-5 transaction handle SHIPPED 2026-05-27.** The multi-call `EngineSession::begin_transaction`/`txn_add`/`commit_transaction`/`rollback_transaction`. Added the two `WorkbookSession` fields (`txns: HashMap<TransactionId, Vec<SessionOp>>`, `next_txn_id: u64`). The handle is a **pure `SessionOp` buffer holding no engine borrow**; `commit_transaction` drains it through the SAME `batch` machinery, inheriting validation-atomicity + graph consistency + one `Op::BatchCommit` + single `state_seq` tick + the same-cell value/formula conflict guard (`conflicting_batch_ops`). No lock between begin and commit → the buffer is validated at **commit time** (a sheet deleted after `txn_add` → `sheet_not_found` at commit). `begin`/`txn_add`/`commit` require `Ready`; `rollback` is ungated (cleanup); `close` drops all buffers. A **failed commit leaves the transaction open** (buffer restored — `batch` applied nothing). Unknown handle → fail-loud `NotFound`/`transaction_not_found`; id-space exhaustion → loud `Internal`/`transaction_id_exhausted` (`checked_add`, never wraps). 10 session tests. **Parallel Codex (gpt-5.5 xhigh) + Opus audit CLEAN** — no HIGH; both reviewers converged on 1 MED (gate `txn_add` on `ensure_ready` — APPLIED) + 2 LOW (`next_txn_id` `checked_add`, op-log-len test assertions — APPLIED). Synthesis `docs/audits/2026-05-27-inc2c5-txn-audit/SYNTHESIS.md`. **ql-exec lib 691/0, clippy clean, workspace green.** (superseded NEXT — see inc.2c-6/2c-7 below.)

**✅ 6.1B inc.2c-6 — F2 Blank-durability gap CLOSED (`Op::ClearValue`) SHIPPED 2026-05-27** (`d8d22a04248`). `set_value(Blank)` on a value-only cell previously appended NO op (`CellWireValue::from_value(Blank)==None`) → the clear was invisible to the op-log and lost on replay/save-load (a cell `5` then Blank-cleared replayed back to `5`). New additive `Op::ClearValue { sheet, row, col }` replays to `Workbook::put_at(.., Value::Blank)`; every value-clear path now logs it (`WorkbookRuntime::set_value`, `WorkbookSession::batch`, `WorkbookTransaction::commit`). `clear_formula` deliberately unchanged (preserves values — Excel convert-to-literal). ql-collab gains `CacheEffect::ClearValue` (mirrors ClearFormula); bindings `classify_delta_op` routes it cell-keyed. `OPLOG_SCHEMA_VERSION` NOT bumped (additive variant). Prerequisite for correct undo. Delegated-agent impl, reviewed at source + independently verified; ql-exec 692/0, ql-oplog/ql-collab green.

**✅ 6.1B inc.2c-7 — undo/redo SHIPPED 2026-05-27** (`4f8e9858d77`). `undo`/`redo`/`can_undo`/`can_redo` via a long-lived `loro::UndoManager` field (built in `from_workbook`, `set_merge_interval(0)` → 1 Loro commit = 1 undo unit; every command emits exactly one commit except `rename_table`/`rename_column` which are wrapped in a `grouped()` Loro undo group — the F10 PutFormula loop). `rematerialize` after a Loro undo/redo: replay the (compacted) op-log onto a CLONE of the construction-time **baseline workbook** → `rebuild_from_workbook` → `recompute_all` with the op-log DETACHED → `bump_epoch` (outstanding `snapshot_delta` token → EpochMismatch full rebuild). Empty stack → `consumed:false`. Builds on inc.2c-6: undo of a Blank-clear restores the prior value. **Parallel Codex+Opus audit:** both verified the two load-bearing claims at source (1-command-1-undo-unit; linear-replay rename fidelity needs no ql-collab repair passes); both flagged ONE real bug — a populated-`from_workbook` session + undo silently dropped pre-loaded content (replayed from an empty workbook) — **FIXED via the `baseline: Workbook` field** (Codex HIGH / Opus M1). F10 table-rename append-atomicity was left tracked at this point, then CLOSED in its own increment inc.2c-8 (`dca5695549e`) — NOT folded into persistence; 100-step undo cap documented. Synthesis `docs/audits/2026-05-27-inc2c67-undo-audit/SYNTHESIS.md`. **ql-exec lib 709/0, clippy clean, workspace build green.** (superseded NEXT — see inc.2c-8 below.)

**✅ 6.1B inc.2c-8 — F10 atomic table-rename SHIPPED 2026-05-27** (`dca5695549e`). `rename_table`/`rename_column` previously appended `Op::RenameTable`/`RenameColumn` + one `Op::PutFormula` per rewritten formula cell as SEPARATE `oplog.append` calls interleaved with the `put_formula` mutations → a mid-loop append failure left a partial log + partially-rewritten workbook (the tracked F10 non-atomicity / Codex HIGH from the inc.2 audit; near-zero reachability — PutFormula serializes a String, only a Loro-internal/OOM failure triggers it). Fix: collect the rename op + all PutFormula rewrites into ONE `Op::BatchCommit` appended BEFORE any mutation, then apply — mirroring `rename_sheet`. Replay applies the inner ops in order ([Rename*, PutFormula*]); `apply_rename_table`/`apply_rename_column` only re-key metadata and rely on the explicit PutFormula ops for text → complementary, replay reproduces the identical end state. The undo `grouped()` wrapper is now defense-in-depth (single-commit by construction). **Focused Codex (gpt-5.5 xhigh) audit: no HIGH/MED** — atomicity + replay fidelity + op-equivalence verified at source; 2 LOW (stale docs, rename_table e2e coverage) APPLIED. `rename_table_emits_ops`/`rename_column_emits_ops` updated to assert the single-BatchCommit shape + new e2e `rename_table_op_log_replay_reconstructs_rename`. **ql-exec lib 709/0 + e2e 21/0, ql-oplog 70/0, clippy clean, workspace build green.** **NEXT = 6.1B persistence** (open/import/save/export via ql_io → adds PersistenceError to Appendix A). **DESIGN FORK to lock first: `open` adopts loaded state — Option 1 (baseline = loaded workbook + FRESH op-log; simple, always-correct, loses op-log history on resave) vs Option 2 (baseline = empty + adopt loaded op-log; preserves history but needs `loaded_wb == replay(loaded_oplog)`).** The inc.2c-7 baseline field is wired for Option 1. Then functions/bulk → Node smoke (+ `.node` rebuild + B#1/S2-01 mocha) → 6.1C audit. **(Superseded — inc.2c-9 below SHIPPED `.qbook` open/save with Option 1; current NEXT = import/export → functions/bulk.)**

**✅ 6.1B inc.2c-9 — `.qbook` `open`/`save` SHIPPED 2026-05-27.** Persistence round-trip on `WorkbookSession`. **DESIGN FORK LOCKED — Option 1** (user-confirmed): `open` reconstructs the workbook from the `.qbook` envelope via `ql_io::load_workbook_with_oplog` and adopts it with `*self = from_workbook(loaded_wb)` — a FRESH empty op-log + fresh `UndoManager` + `baseline = loaded_wb` + re-minted epoch (so any pre-open `snapshot_delta` token → EpochMismatch full rebuild) + reset state_seq/change_log/ops/events/txns, registry preserved — then recomputes with the op-log DETACHED (`with_runtime_no_oplog`; saved computed values may be stale, mirroring `loader.rs::load_workbook_and_recompute`), folding failures into `CellDiagnostic` events. The loaded op-log is **fully validated** (the snapshot framing by `load_workbook_with_oplog` PLUS its per-op payloads by an explicit `iter()` loop — Codex HIGH: framing-only validation would silently accept a payload-corrupt sidecar then mask it on the next save) then **discarded** (Option 1 trade-off: history not carried; re-save writes only this session's edits). `save(&self)` writes `&self.workbook` + `&self.oplog` via `ql_io::save_workbook_with_oplog`; the workbook name is derived from the path file-stem (no document-name metadata in v1; `None` → loud `BadArgument`). New `map_persistence_err` fills Appendix A (`Persistence`/`qbook_error`|`session_oplog`|`qbook_unsupported_version`|`qbook_truncated_header`; foreign `#[non_exhaustive]` wildcard → loud `Internal`/`unmapped_persistence_error`). `open` legal in `New`/`Ready` (re-open replaces the document) via a new `ensure_openable` gate; `save` uses `ensure_readable`. Option 2 (adopt the loaded op-log) rejected — needs `loaded_wb == replay(loaded_oplog)`, re-opening the data-loss bug class inc.2c-7 closed; revisit for collab-v1.5. `import`/`export` (xlsx/csv) stay honest `Capability/not_implemented_in_v1_core` — a follow-up sub-increment. **Parallel Codex(gpt-5.5 xhigh)+Opus audit:** 1 HIGH (sidecar payload-validation gap — FIXED + regression test) + 2 LOW docs/test-coverage (module-doc drift fixed; `can_undo`-false-after-open + save→open→re-save→open stability tests added) + INFO (Appendix-A variant-name spelling tidied); both verified the post-open undo invariant + open-failure atomicity + recompute op-log isolation at source. Synthesis `docs/audits/2026-05-27-inc2c9-persist-audit/`. **ql-exec lib 719/0 + e2e 21/0, clippy clean, workspace build green.** **(Superseded — inc.2c-10 below SHIPPED xlsx import; current NEXT = csv import + xlsx/csv export → functions.)**

**✅ 6.1B inc.2c-10 — xlsx `import` + `ql-io-xlsx` dependency inversion SHIPPED 2026-05-27.** `WorkbookSession::import("xlsx")` is live. **Root blocker:** `import` lives in `ql-exec`; the xlsx importer lives in `ql-io-xlsx`, which depended on `ql-exec` (only for formula recompute) — so `ql-exec → ql-io-xlsx` would be a cycle (`ql-io-xlsx` was a leaf crate, never wired into a consumer). **Fix — dependency inversion (chose A2 over A1-parse-only to preserve `ql-io-xlsx`'s tested recompute/Strict/`formula_failures` behavior):** new `pub trait XlsxRecomputer` in `ql-io-xlsx`; `import_xlsx_bytes`/`import_xlsx_path` gained a `recomputer: Option<&dyn XlsxRecomputer>` param (BestEffort/Strict require it → loud `XlsxError::Engine` if `None`; Skip ignores it); `recompute_loaded_workbook` deleted; `ql-exec` moved to `ql-io-xlsx`'s `[dev-dependencies]` (dev-dep cycles are allowed). `ql-exec` gained `ql-io-xlsx` as a normal dep + public `EngineXlsxRecomputer` (`src/xlsx_recompute.rs`, the moved recompute logic over `WorkbookRuntime::recompute_all`). `import("xlsx")` = parse via `import_xlsx_bytes(.., Some(&EngineXlsxRecomputer))` (recomputes during import) → **Option-1 adoption** (`*self = from_workbook`, registry preserved, epoch re-minted; NO second recompute) → `push_xlsx_import_diagnostics`. `"csv"` → honest `not_implemented` (net-new — no csv importer); unknown format → loud `BadArgument`. `map_xlsx_err` fills Appendix A (Io/Zip/Calamine/XmlParse/MalformedOoxml/UnsupportedFeature → Persistence; Export/Engine → Internal; foreign `#[non_exhaustive]` wildcard → loud `unmapped_xlsx_error`). **~90 `ql-io-xlsx` test call-sites migrated** (delegated to a fresh Opus agent, reviewed at source; 3 `src` lib-test sites use `None` — verified behavior-preserving since they fail before recompute or use Skip — because the dev-dep cycle puts two `ql_io_xlsx` crate instances in the lib-test graph, an E0277 the integration tests don't hit). **Parallel Codex(gpt-5.5 xhigh)+Opus audit, every finding verified at source:** 1 HIGH (both reviewers — `push_xlsx_import_diagnostics` dropped `report.feature_inventory.counts`, the PRIMARY dropped-feature channel: CF/DV/merged-cells/comments/drawings/protection/hidden-sheets → silent fidelity loss; FIXED + regression test) + 1 MEDIUM (Codex — recompute outside the `FaultGuard`; assessed NON-BUG: `import` replaces `self` atomically after a local-workbook recompute, so no torn state — documented, a guard would wrongly fault a healthy session) + INFO (docs). Both confirmed: no cycle, Option-1 atomicity, `map_xlsx_err` exhaustive, the 3 `None` sites sound. **Tracked follow-up:** `ql-exec` now transitively pulls the xlsx WRITER's heavy deps (umya-spreadsheet + image/rav1e/exr/tiff) — feature-gate the writer behind a `write` feature + depend `default-features = false` to keep the core engine + WASM/bindings lean (`ql-exec/Cargo.toml` tracks it). Spec: `docs/api/xlsx-import-integration-plan.md`; synthesis `docs/audits/2026-05-27-inc2c10-xlsx-import-audit/`. **ql-exec lib 723/0 + e2e 21/0, clippy clean; ql-io-xlsx green (lib 60 + calamine_smoke 49); workspace build green.** **NEXT = `import("csv")` + `export` (xlsx/csv) → functions (6.4-0 metadata substrate then 6.4) + reserved bulk → Node smoke migration (+ `.node` rebuild + B#1/S2-01 mocha) → 6.1C audit → 6.4 Python UDFs.**

**[superseded note] ✅ 6.1A SHIPPED 2026-05-26** (docs-only, no code cycle): `docs/api/session-api.md` — the stable engine session contract (API6-01/02/03). Covers command surface (product commands ≠ collab op variants), versioned DTOs, `EngineError` taxonomy, operation-lifecycle + cancellation, sync/async + panic/validation boundary, event/diagnostic stream, function-metadata + graph-invalidation contract (the 6.4-0 prerequisite), per-binding obligations + golden-flow matrix. Bottom-up-extracted from the proven napi `CollabSession` surface + single-writer `WorkbookRuntime`; anchors verified at source (`calcgraph_session.rs:69-71` owning-session anticipation, `:149-164`/`:201-203` hardcoded whitelists = the metadata gap, `lib.rs:306-349` the `[kind]` error workaround). **✅ Codex-validated (gpt-5.5 xhigh, verdict REVISE → all 5 HIGH/5 MED/2 LOW/1 INFO resolved in `session-api.md` v2; review: `docs/api/codex-6-1a-review.md`).** Key locks added: single-writer `OpLog` mandatory → its Loro VV is the version token (resolves the deferred-collab token gap); cancellation scoped honestly (in-engine recalc = pre-start cancel only in v1; hard no-late-commit only for out-of-process UDF/SQL/AI; `Busy` lifecycle state); `batch(ops,options)` + opaque transaction handle (no RAII borrow across FFI); reserved bulk `write_range`/`publish_dataset`/`bind_range`/`refresh_source`/`materialize_query` for 6.4/6.5; `functions_used` reverse index + registration invalidation; malformed version token = fail-loud `invalid_version_token` (No-Fallbacks); error variant→code mapping appendix. **NEXT = 6.1B** (implement `WorkbookSession`; CODE phase → fresh session).
