...
**✅ 6.1B inc.2c-7 — undo/redo SHIPPED 2026-05-27** (`4f8e9858d77`). `undo`/`redo`/`can_undo`/`can_redo` via a long-lived `loro::UndoManager` field (built in `from_workbook`, `set_merge_interval(0)` → 1 Loro commit = 1 undo unit; every command emits exactly one commit except `rename_table`/`rename_column` which are wrapped in a `grouped()` Loro undo group — the F10 PutFormula loop). `rematerialize` after a Loro undo/redo: replay the (compacted) op-log onto a CLONE of the construction-time **baseline workbook** → `rebuild_from_workbook` → `recompute_all` with the op-log DETACHED → `bump_epoch` (outstanding `snapshot_delta` token → EpochMismatch full rebuild). Empty stack → `consumed:false`. Builds on inc.2c-6: undo of a Blank-clear restores the prior value. **Parallel Codex+Opus audit:** both verified the two load-bearing claims at source (1-command-1-undo-unit; linear-replay rename fidelity needs no ql-collab repair passes); both flagged ONE real bug — a populated-`from_workbook` session + undo silently dropped pre-loaded content (replayed from an empty workbook) — **FIXED via the `baseline: Workbook` field** (Codex HIGH / Opus M1). F10 table-rename append-atomicity left tracked (pre-existing, near-zero reachability, folds into persistence); 100-step undo cap documented. Synthesis `docs/audits/2026-05-27-inc2c67-undo-audit/SYNTHESIS.md`. **ql-exec lib 709/0, clippy clean, workspace build green.** (superseded NEXT — see inc.2c-8 below.)
