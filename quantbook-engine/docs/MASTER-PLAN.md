...
10. **4.10 Function Library Expansion Wave 2 - V1 260**  
    ...
    **Status (2026-05-17): SHIPPED at 260/260 registered fns** through HEAD `517bcab08b7` (W5-D-12.1 audit transcripts). Path: W5-D-1..W5-D-7 Wave 3 distributions (+38 → 238), W5-D-8 BIT* (+5 → 243), W5-D-9 base conversion (+6 → 249), W5-D-10 ERF/ERFC family (+4 → 253), W5-D-11 PERCENTILE/QUARTILE (+6 → 259), W5-D-12 SUBTOTAL (+1 → **260** — V1-260 hit). 12 ship+audit commit pairs across the closeout arc. Matrix coverage now 80% per `scripts/report-compat-coverage.sh` (210 ✅ implemented + 38 ⚠️ partial + 1 🔄 reserved + 62 ❌ missing across 311 rows). Acceptance: FN4-260-01 ✅ 260 registered (hit the "roughly 260" target exactly); FN4-260-02 ✅ every fn has matrix-backed tests + coverage-discipline EXPLICITLY_DEFERRED entry; FN4-260-03 ✅ unregistered fns produce `#NAME?` per existing engine convention. Audit-discipline rule (parallel Codex + Opus per batch) caught 5 HIGH + 10 MEDIUM + 20 LOW across the 5 V1-260 closeout batches alone; all closed in-commit per the `.1` convention. Known follow-ups: (a) literal range refs in `AggregateArg` context (`SUM(A1:A10)`, `SUBTOTAL(9, A1:A10)`) remain unbindable — pre-existing engine-wide gap at `plan.rs:706-731` and `plan.rs:380-383`, deferred to a future polish pass; (b) Batch E remainders LOOKUP vector form + LEFTB/RIGHTB/MIDB/LENB/FINDB/SEARCHB byte-length variants deferred (matrix marked ❌). Full audit trail in `docs/audits/2026-05-17-w5-d-*.md` (12 transcripts).
