...
> - **(c) M5 + §4b DTO field-parity sweep** — thread `schema_version` + `full_rebuild_reason` through napi→IDE, add `CellValue.blank` to the mirrors, reshape the napi error into `.code`/`.class`/`.details` data; opaque-handle/buffer-ownership discipline.
