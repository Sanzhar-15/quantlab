...
> - **(b) M2 refactor** (`start_recalc`/`await_recalc`) — opens with an AskUserQuestion on the split shape (synchronous recompute → pre-start-cancel window; lock released between). The §3.3 cancellation acceptance item.
