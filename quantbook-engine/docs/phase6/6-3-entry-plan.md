...
> - **(d) H1/L4 IDE allowlist FIRST (Codex MED — H1 blocks 6.3 entry, must precede any error-heavy method wiring into IDE paths)** — extend `QuantbookErrorCode` + the compile-enforced Record with the **16 missing codes** (incl. `panic` — pairs with 6.3-1a) on `feat/visualise-v1`.
