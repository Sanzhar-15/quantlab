...
| `RenameTable { A, B }` × concurrent edit referencing A | **Phase 5.3 step 4 + 5b ✅ shipped 2026-05-20:** post-merge `ql_collab::repair_table_rename_chain` rewrites the formula text from `A[col]` to `B[col]`. Caller-driven; use `CollabSession::rebuild_workbook` to chain `replay → sheet repair → table repair → column repair`. |
| `RenameTable { _, A }` × `RenameTable { _, B }` (same source, different targets) | **Phase 5.3 step 4 ✅ shipped:** advisory-skip on missing source. Last-in-causal-order wins (the second op's source is no longer present; advisory-skip preserves the first peer's rename). |
| `RenameTable { _, X }` × `RenameTable { _, X }` (different sources, same target) | **Phase 5.3 step 4 audit closure ✅ shipped:** **HARD-FAIL** at replay via `TableCreateRejected { source: Duplicate }`. V1 limitation (audit-locked revert of an unsound auto-disambig — V2 closure requires API change to thread disambig trace from replay to repair). Tracked at `docs/PHASE-4-V2-BACKLOG.md` Tier H. |
| `RenameColumn { T, A, AA }` × concurrent edit referencing `T[A]` | **Phase 5.3 step 5c + 5c audit closure ✅ shipped 2026-05-20:** post-merge `ql_collab::repair_column_rename_chain` rewrites `T[A]` → `T[AA]`. Rules are scoped per-table; refs to OTHER tables' columns of the same name are untouched. |
| `RenameColumn { T, _, X }` × `RenameColumn { T, _, X }` (same table + same target) | **HARD-FAIL** at replay via `TableColumnRejected`. V1 limitation (same root cause as cross-source table collision; same V2 closure path). |
| `RenameTable { T, Sales }` × `RenameColumn { T, A, AA }` (cross-kind concurrent) | **Phase 5.3 step 5c audit closure ✅ shipped:** `apply_rename_column` advisory-skips when its wire table was concurrently renamed (mirrors `apply_rename_table` step 4 pattern). `repair_column_rename_chain` resolves the column op's wire table name through the table-rename chain to the current canonical, so column repair rules match post-table-repair formula text. **V1 limitation**: in some causal orderings the column rename's intent is lost (advisory-skip); V2 closure: causality-aware tracking via Loro op-ids. |
| `DropTable "T"` × concurrent edit referencing T | Edit appends with old table-ref → bind fails post-merge → `#NAME?`. |
| `DropTable "T"` × concurrent `RenameTable { T, T2 }` | **Phase 5.3 step 5a audit closure ✅ shipped:** both causal orderings converge to "table absent". `DropTable` now advisory-skips on missing source (mirrors `apply_rename_table` step 4 pattern); pre-closure rename-then-drop hard-failed with `TableNotFound`. |
