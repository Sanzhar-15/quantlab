...
| 15 | **4.9.O** (W5-150, shipped) | Closing megaudit. Codex backend failed to produce findings on both dispatch attempts (process gap, not engine bug); audit fell back to focused self-review of 5 load-bearing files. **Found 1 HIGH:** `PlanCache` collision for `@<range>` formulas across cells in the same sheet (cache key omitted cell coords, but bind-time `@`-narrowing produces cell-specific plans). **Fixed in this commit:** added `cell_anchor: Option<(RowId, ColId)>` to `PlanCacheKey`; 4 call sites set it conditionally (when canonical text contains `@`) so non-`@` formulas keep cross-cell cache sharing. Regression test pinned. 3 LOW findings (Unknown-locale round-trip not test-pinned; scientific-notation locale swap not test-pinned; i32::MIN/MAX AxisSpec edge cases not tested) deferred to follow-up. **Verdict: SHIP-READY.** Full audit transcript in `docs/audits/2026-05-15-phase-4.9-closing-megaudit.md`. |
