...
| 3 | **4.9.C** (W5-139, shipped, parser-only) | Parser: accepts `Token::R1C1Ref` + emits intermediate `Expr::R1C1Ref { sheet, row_axis, col_axis }` (new variant, distinct from `Expr::CellRef`) for single refs and `RangeRef::R1C1Cells { sheet, start_row, start_col, end_row, end_col }` for ranges. Per-axis mixed-relativity check (row1 vs row2, col1 vs col2 independently) rejects with new `ParseError::R1C1MixedRelativity` (closes Codex MEDIUM + Sonnet H-4). Cross-form ranges (A1+R1C1 endpoints) reject via existing `ParseError::InvalidRange`. Sheet qualification (`Sheet1!R1C1`) works via the existing `apply_sheet_to_term` plumbing. **Binder lowering deferred** — `bind_with_context_v2` returns `BindError::UnsupportedVariant` on R1C1 forms today; absolute lowering with `BindSite::at_cell` ships in its own sub-phase. Calcgraph's 5 `RangeRef`/`Expr` exhaustive matches (fingerprint × 2, graph, stripes × 2) added loud `unreachable!()` arms — R1C1 forms must be lowered before reaching calcgraph (storage canon per § 4.4). Printer arms same pattern (`print_with` handles R1C1 in 4.9.D). **Refinement vs the design wording "Expr::CellRef with intermediate markers":** chose a new `Expr::R1C1Ref` variant rather than extending `CellAddr` with relative offsets — the existing 50+ `CellAddr` construction sites stay untouched, and the new variant cleanly isolates the parser-intermediate form. |
