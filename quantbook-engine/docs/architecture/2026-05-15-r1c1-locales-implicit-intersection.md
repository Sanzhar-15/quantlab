...
| 11 | **4.9.K** (W5-146, shipped, partial — canonical-storage wiring deferred) | `WorkbookRuntime::set_reference_mode(mode)` + `set_locale(locale)` APIs: pure-metadata setters that append `Op::SetReferenceMode` / `Op::SetLocale` (idempotent — setting to the current value emits no op, keeps the log compact). **Canonical-storage contract per § 4.4 deferred** to a follow-up: today `set_formula(text)` continues to call `lex(text)` (A1+EnUs); the full `lex_with(text, wb.reference_mode(), wb.locale())` → `parse_with(.., site)` → `print_with(.., A1, EnUs, None)` round-trip wiring lands in a separate sub-phase. The runtime APIs + op-log shape from 4.9.K's main body are in place; the `set_formula`-side canonicalization is the remaining work. |
