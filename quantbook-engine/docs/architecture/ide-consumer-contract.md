...
4. **Finer-grained "N pending changes" via `pending_op_count()` (V2 V4 V1 step 2 SHIPPED 2026-05-21)**: `has_pending_flush()` is just a boolean. For bounded-queue policies ("warn user when more than N changes pending" or "switch editor to read-only if backlog exceeds N"), use `session.pending_op_count() -> usize`. Returns log entries not yet flushed to the currently-attached transport. **Semantic note**: the count includes BOTH local appends AND merges from peers — it's "what the transport hasn't seen yet," not "your own unsynced edits." For "my edits only," the IDE tracks its own peer-id-filtered counter. Pairs naturally with `has_pending_flush()` (the boolean check is `pending_op_count() > 0`).
