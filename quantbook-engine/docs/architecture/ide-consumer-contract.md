...
`CellState` extended with `format: Option<FormatId>` field.  Per-cell `Op::SetCellFormat` cache integration ships at V3.5.0.5; **session-wide RegisterFormat FormatTable cache SHIPPED at V3.6.0.3 D2** (engine `ffa598a1c15` + IDE `b159c899359`) with `WorkbookSnapshotJson.formats: Vec<FormatDefJson>` additive field; **format-aware buildHtml rendering SHIPPED at V3.6.0.5 D4** (engine `5b3017c70d2` + IDE `d7cb01557d4`) with new `CellSnapshotJson.rendered: Option<String>` + `WorkbookSnapshotJson.dateSystem: String` additive fields; V3.6.0.X audit-of-D4 closures (engine THIS commit): `Op::SetDateSystem` wire variant + replay handler (Excel1904 round-trips correctly); per-snapshot parsed-format cache; Pending-skip-pre-render; locale passthrough; IDE `data-raw-value` attribute (preserves click-to-edit for currency/percent/thousands/date formats).  The V3.6.0.X audit-of-D2 closure (CONVERGENT-MED-1) sorts the formats field by FormatId for stable shape across snapshots; CONVERGENT-HIGH-1 closure mirrors `FormatTable::register_at` rejection at the cache walker (first-write-wins).
...
**IDE side**: V3.5.0.5 shipped passthrough only (extractSheetSnapshot dropped format).  V3.6.0.5 D4 ships full format-aware rendering: `extractSheetSnapshot` conditionally includes `rendered` (preserves V3.5.0.4b exact-shape tests for cells without format); `cellGridHtml::renderRows` uses `e.rendered ?? formatCellValue(e.value)`; CSP-safe escapeHtml on both paths; new `data-raw-value` attribute carries parseable raw representation for click-to-edit (V3.6.0.X audit-of-D4 OPUS-HIGH-2 closure prevents `parseCellRawInput` NaN on formatted strings).
...
- **Per-cell op-index for indexed-lookup `invalidate_cell`** SHIPPED at V3.6.0.4 D3 (engine `119df9b3826`).  New `cell_op_index: HashMap<(u16, u32, u32), Vec<usize>>` + `sheet_op_index: HashMap<u16, Vec<usize>>` fields on `CollabSession`.  Maintained incrementally in `append_op` via `apply_cache_effect` (+ new `CacheBuckets<'a>` bundling struct).  Rebuilt during `from_snapshot` + `merge_bytes` + `discard_pending_ops` + post-undo/redo `rebuild_op_indices_only`.  `invalidate_cell` walks `cell_op_index[(s,r,c)] ∪ sheet_op_index[s]` via the new `OpLog::get(idx)` random-access accessor instead of the full log.  Per-op cost is Loro `LoroList::get` BTree lookup (O(log N)).  Memory cost: ~24 bytes per indexed op-cell pair; bounded by visible op history.  See V3.6.0.X audit-of-D3 transcripts for the design discovery (R-V3.6-10: positional indices need refresh after Loro UndoManager retracts).
...
**Status**: **V3.6 PHASE TERMINATION CLEAN.**  V3.6.0.8 D6 ARC COMPLETE + V3.6.0.10 D8 Op::RestoreSheet SHIPPED + V3.6.0.11 D9 typing-stroke watchdog SHIPPED + V3.6.0.X phase-termination 3-lane megaudit (Codex + Opus + claude-self) + 1 convergent HIGH closed + MED/LOW sweeps.  D6 perf contract delivered: 100 new cells delta = 1.08 ms vs 246 ms full snapshot (228× faster than full; 46× under the 50 ms threshold).  R-V3.5-7 + R-V3.6-15 + R-V3.6-17 + R-V3.6-19 CLOSED.  Opus-MED-5 (comprehensive mocha) CLOSED at V3.6.0.8.5.  D8 ships `Op::RestoreSheet { id }` un-tombstone (V3.6.0.10).  D9 ships `typing_stroke` envelope arm + `resetTypingWatchdog` panel method + webview `'input'` listener (V3.6.0.11).  Phase-termination CONVERGENT-HIGH-1 (RestoreSheet preserved-cells gap; 6-site contract drift) closed via walker drop-no-prune semantic mirroring V3.5.0.3b Workbook storage discipline.  Only D7 (#REF! substitution at V3.6.0.9) remains conditional pending user signal; not blocking phase exit.  Spike verdict (V3.6.0.7): 251 ms median at 100k cells / 50 % format / 1 sheet (5× the 50 ms threshold).  Lock verdict (V3.6.0.8.1): option (a) cache rebuilt+repaired Workbook + clone-and-apply-delta CHOSEN.  Engine foundation (V3.6.0.8.2): 2 new fields + 4 invalidation callsites + new public `apply_ops_in_range` helper on ql-oplog.  Napi surface (V3.6.0.8.3): `workbookSnapshotDelta` + `WorkbookSnapshotDeltaJson` + `version: Buffer` field on `WorkbookSnapshotJson`.
...
**Tests baseline**: ql-collab **158/158** (V3.6.0.X phase-termination +4 over V3.6.0.11 154 -- CONVERGENT-HIGH-1 regression suite: snapshot-cache resurfaces preserved cells + rebuild path under remove-then-restore + tombstoned-sheet cells remain hidden until restore + post-restore writes stack atop preserved cells) + ql-oplog **67/67** + ql-collab-ws **42/42** (40 lib+transport + 2 ignored doctests) + IDE mocha **415/415** (V3.6.0.X phase-termination +2 over V3.6.0.11 413 -- end-to-end restoreSheet cell preservation through napi `workbookSnapshot`) + engine workspace release build clean.  V3.6.0.7 + V3.6.0.8.4 add criterion benches (`cargo bench -p ql-bindings-node --bench workbook_snapshot`).
...
|---|---|---|
| (a) Cache rebuilt+repaired Workbook + clone-and-apply-delta | **CHOSEN** | Skips full `rebuild_workbook` + repair walks on the cell-only steady-state path.  Cache is post-repair so contract is preserved. |
| (b) Read directly from `last_snapshot` cache + skip `rebuild_workbook` | REJECTED | `last_snapshot.formula` carries PRE-rename-repair text per `crates/ql-collab/src/session.rs:1916-1922`.  Skipping repair silently regresses the V3.5.0.X A-HIGH-2 contract -- IDE would surface stale `Sheet1!A1` references after `Sheet1 -> Renamed`. |
| (c) Hybrid revalidate-only-changed-formula-cells | DEFERRED to V3.7+ | Adds complexity; option (a) should deliver if its assumptions hold. |

**Locked napi surface**:

```typescript
interface CollabSessionInstance {
...
interface WorkbookSnapshotDeltaJson {
  changedCells: Array<{ sheet: number; cell: CellSnapshotJson }>;
  removedCells: Array<{ sheet: number; row: number; col: number }>;
  sheetsChanged: SheetSnapshotJson[];
  sheetsRemoved: number[];           // sheet ids (RemoveSheet tombstones since lastSeen)
  formatsAdded: FormatDefJson[];     // RegisterFormat ops since lastSeen
  version: Buffer;                   // opaque VV token; cache for next call
  fullRebuildRequired: boolean;      // true → call workbookSnapshot() instead
}
```
...
**Locked invalidation triggers** (calls new `force_clear_workbook_cache(&mut self)` helper):

| `CollabSession` method | Reason |
|---|---|
| `merge_bytes` | Remote ops re-order via Loro CRDT causal merge; LWW winners may shift. |
| `discard_pending_ops` | Log replaced via `fork_at_vv`; prior ops may vanish. |
| `undo` / `redo` | Loro UndoManager retract compacts the visible log (R-V3.6-10); cached Workbook reflects pre-retract state. |
| **NOT** `append_op` | Local cell-keyed appends are exactly the case the cell-only fast-path handles; cache stays valid. |

Mirrors the V3.4.0.2 `force_clear_snapshot_cache` callsite set + V3.6.0.4 `cell_op_index` invalidation pattern.

**Locked algorithm**:

```
fn workbook_snapshot_delta(&self, last_seen: Vec<u8>) -> Result<WorkbookSnapshotDeltaJson>:
  if self.last_snapshot_workbook.is_none() OR last_seen.is_empty():
    return { fullRebuildRequired: true, ..empty }
  let caller_vv = VersionVector::decode(&last_seen).map_err(...)?;
  if &caller_vv != self.last_snapshot_oplog_vv.as_ref().unwrap():
    return { fullRebuildRequired: true, ..empty }  // staleness fallback
  let current_vv = self.log.oplog_vv();
  if current_vv == caller_vv:
    return { version: current_vv.encode(), ..empty }  // same-VV fast-path
  let ops = ops_since(&caller_vv);
  let has_rename = ops.any(|op| matches!(op, RenameSheet | RenameTable | RenameColumn));
  if has_rename:
  ...
  build JSON delta from changed_cells + removed_cells + sheets_changed + ...
  return { version: current_vv.encode(), fullRebuildRequired: false, ... }
```

**Sub-step rollout for V3.6.0.8 D6 arc**:
...
**Producer/replay validation asymmetry (OPUS-PT-B8, documented 2026-05-26)**: the napi PRODUCERS `restoreSheet` + `deleteSheet` are STRICT — they reject `id > u16::MAX` AND `id >= workbook.sheet_count()` with `[bad_argument]` (`lib.rs` `restore_sheet`/`delete_sheet`, pre-rebuild existence check).  The REPLAY side is PERMISSIVE — `Workbook::restore_sheet` (`ql-storage/src/workbook.rs:781`) guards `id < sheets.len()` and otherwise no-ops; `Op::RestoreSheet`/`Op::RemoveSheet` replay silently drops out-of-range ids (`op.rs` "Out-of-range ids: silently dropped" note).  This is INTENTIONAL, not a bug: the producer guard gives the local IDE caller a clear error instead of emitting an op the permissive replay would silently swallow — the divergence the V3.5.1+ backlog item CLOSURE-CODEX-MED-1 (cache-vs-replay parity for out-of-range RemoveSheet) describes is only reachable via manually-constructed logs, never via these napi producers.  IDE consumers should treat `[bad_argument]` from `restoreSheet`/`deleteSheet` as a real caller error (bad sheet id), NOT retry it.  (Already-tombstoned/already-restored is NOT `[bad_argument]` — that is CRDT idempotency, a replay no-op.)
...
- **R-V3.6-19** (NEW) RestoreSheet napi cell-recovery gap.  CLOSED-AT-V3.6.0.X-phase-termination.  3-lane megaudit (Codex CODEX-PT-A1 + Opus OPUS-PT-B1 convergent + claude-self Lane C synthesis) found that 6 docstrings promised "cells reappear via workbookSnapshot full rebuild" but the cache walker dropped cells at `CacheEffect::RemoveSheet` and never rehydrated them at `CacheEffect::RestoreSheet`.  CLOSED: walker no longer prunes on RemoveSheet (cache mirrors V3.5.0.3b Workbook storage-preservation discipline); pre-tombstone cells preserved + resurfaced automatically on RestoreSheet.  Visibility via 3 filters (`workbook_snapshot.is_sheet_removed`, new `removed_sheet_ids` filter in `workbook_snapshot_delta`, new `removed_sheets.contains` filter in `list_sheets_from_cache`).  +4 ql-collab tests + 2 IDE mocha tests.  See `docs/audits/2026-05-26-phase-5-7-v3-6-0-X-phase-termination-{codex,opus,closures}.md`.
...
| V3.6.0.11 D9 typing-stroke watchdog | (ship-only sub-step; coverage by V3.6.0.X phase-termination audit below) | inline at `.plans/_active.md` + this section + docs/MASTER-PLAN.md |
| **V3.6.0.X phase-termination** | Codex Lane A: `2026-05-26-phase-5-7-v3-6-0-X-phase-termination-codex.md` (FAIL; 1 HIGH + 1 LOW + 9 INFO; convergent CODEX-PT-A1 with Opus) | Opus Lane B: `2026-05-26-phase-5-7-v3-6-0-X-phase-termination-opus.md` (FAIL; 1 HIGH + 4 MED + 5 LOW + 5 INFO; convergent OPUS-PT-B1 with Codex). 3-lane synthesis + closures: `2026-05-26-phase-5-7-v3-6-0-X-phase-termination-closures.md`.  CONVERGENT-HIGH-1 (RestoreSheet preserved-cells gap) closed via cache-walker drop-no-prune semantic mirroring V3.5.0.3b Workbook storage discipline; +4 ql-collab + 2 IDE mocha regression tests; R-V3.6-19 added.  V3.6 PHASE TERMINATION CLEAN. |
