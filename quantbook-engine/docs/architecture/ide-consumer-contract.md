...
- **Auto-flush (✅ Phase 5.5 V2 V2 SHIPPED 2026-05-21):** `CollabSession::set_auto_flush_policy(AutoFlushPolicy::OnAppend)` makes every mutator (`append_op`, `merge_bytes`, presence writes, `undo` / `redo` when an item was consumed, `sweep_presence`) auto-invoke `flush_to_transport` after a successful state change. IDE callers can drop their scheduled-flush-tick loops. Default is `AutoFlushPolicy::Disabled` (V2 V1 explicit-drive behavior preserved — set explicitly to opt in). **Partial-state contract**: if auto-flush fails (closed transport, I/O error), the surrounding mutator returns `Err(CollabSessionError::Transport(_))` BUT the local op is already committed (mutate-then-flush ordering). Callers MUST treat local state as authoritative + either retry the flush or detach the transport. The explicit `flush_to_transport` API remains available regardless of policy (mix-and-match supported). **Receive-side exclusion**: `poll_remote` / `poll_remote_with_limit` do NOT auto-flush by design (audit-locked Phase 5.5 V2 V2 Codex M1 + Opus M1, 2026-05-21) — when remote bytes are drained, the caller decides when to fan onward via explicit `flush_to_transport`. **Fanout caveat (3+ peers)**: in a hub topology (A↔hub↔B) where each peer's writes should reach the other, the HUB must enable `OnAppend` AND call `flush_to_transport` after each `poll_remote` batch (poll-side does not auto-flush). Symmetric `OnAppend` on both endpoints of a paired transport is safe in V2 V2 only because of the poll-side exclusion; the constraint relaxes once V2 V3 lands per-transport version-vector idempotency guards.
