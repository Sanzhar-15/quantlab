...
- **R-V3.4-3 Presence + 2-window collab race** -- **KNOWN-GAP at V3.4.0.X** (reclassified from DEFERRED at V3.4.0.5b via Opus M2 finding).  The V3.4.0.5b deferral covered the SCROLL handler clobber (`activeInput` guard) + the POLL-MERGED rebuild (full `webview.html = ...` regenerates presence decoration naturally), but MISSED a third race class: a host-driven `webview.html` rebuild during the user's mid-edit `<input>` lifetime destroys the in-progress input.  Reproducible in two-window collab smoke (window 1 mid-edit + window 2 commit -> window 1's pollRemote-merged tick fires `render()` -> webview.html reassign destroys the `<input>`).  V3.4.0.5c follow-up MUST add a host-level guard (boolean OR a `typing:true`-counter tracking active edits) checked in `tickPollRemote` before calling `render()` -- skip the render and re-queue for the next tick if mid-edit.  Tracked in V3.4.1+ backlog; not V3.4.0.X-blocking (no live-smoke complaint surfaced; user-action gap pending).
...
**R-V3.4-3 RECLASSIFIED**: was DEFERRED at V3.4.0.5b; promoted to KNOWN-GAP at V3.4.0.X via Opus M2.  The race is reproducible in two-window collab smoke (window 1 mid-edit + window 2 commit -> window 1's `webview.html` rebuild destroys mid-edit `<input>`).  V3.4.0.5c follow-up MUST add a host-level `presenceRepaintInFlight` boolean (or equivalent guard) to `tickPollRemote`'s render gate.  Not blocking V3.4.0.X close; tracked in V3.4.1+ backlog.
