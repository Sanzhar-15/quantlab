...
| 2 | D2-1 CellValueJson union | LOW | **VALIDATED + SCOPED 2026-05-26 (not shipped — deferred):** retyping `extensions/quantlab/src/quantbook/types.ts` `CellValueJson` to a real discriminated union (`{kind:'number';number:number} \| {kind:'boolean';boolean:boolean} \| {kind:'text';text:string} \| {kind:'error';error:string} \| {kind:'pending'}`) compiles CLEAN against production `src/` (the live consumer `extractSheetSnapshot` already narrows on `kind`). Only **20 sites in `test/quantbook-roundtrip.test.ts`** need updating (13 `.value!.number` accesses on the bare union → add a `cellNumber(v): number` narrow-or-throw test helper; 6 absence assertions at L4360-4362/4446-4448 → cast-to-record or drop, now structurally enforced; 1 incomplete `{kind:'number'}` literal at L5346 → add payload). Rust stays a `#[napi(object)]` struct (napi constraint). Reverted to keep the tree green; a fresh session applies the recipe in ~20 min. Keep the defensive runtime guards. | small (TS, test fan-out) |
