...
- **Closure (ship `ff76fc99d49` + audit-closure `0bd592a4aa8`):** added `CollabSession::pending_op_count() -> usize`. Implementation uses **VV math** — for each peer in `current_vv`, take the per-peer counter delta against `last_flushed_vv` (or 0 if absent), and sum positive deltas. Strictly monotonic under all current mutators including undo (the undo's inverse op is a new VV entry even though it retracts a visible LoroList entry). Sibling invariant: `pending_op_count() > 0` iff `has_pending_flush() == true`. 8 tests pin the contract (fresh boundary, grows with appends, resets after flush, reset on attach, unchanged on failed flush, includes peer-merge ops, GROWS under undo, from_snapshot reports imported count).
...
### K3. EchoServer tokio::sync::Mutex migration risk — ✅ SHIPPED V2 V4 V1 step 4 (2026-05-21) + step 4 closure race fix

- **Source:** V2 V3 step 5 megaudit Opus-B M5; expanded by V2 V4 V1 step 4 audit closure (Codex M1) which found a real multi-thread Drop race the initial comment-only closure didn't cover.
- **Problem:** `EchoServer::accept_task` pushes per-conn handles to `Arc<Mutex<Vec<JoinHandle>>>`. Initial framing: `std::sync::Mutex` is benign with `accept_task.abort()` because the lock-and-push is sync. Step 4 audit (Codex M1) revealed this only protects SINGLE-thread interleaving; on a multi-thread tokio runtime, Drop on a different worker thread can drain `conn_tasks` AFTER the accept task spawned but BEFORE it pushed → per-conn task leaks past test end.
- **Closure (full):** step 4 ship added the LOAD-BEARING comment; step 4 audit closure refactored to `ServerState { closing: bool, tasks: Vec<JoinHandle> }` shared between EchoServer + TextFrameServer. Drop sets `closing = true` under the lock + drains; accept loop checks the flag AFTER taking the lock and aborts orphan handles immediately if `closing` is true. Both paths atomic under the same mutex. Closes BOTH the single-thread (original framing) AND multi-thread (Codex M1) races.
