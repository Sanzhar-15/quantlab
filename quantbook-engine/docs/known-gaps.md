...
**Date last touched:** 2026-05-12 (Engine Phase 3.10 megaudit close — W5-43)  
...
| GAP-G-01 | Phase 0 `Graph` edges and stripe entries are append-only. When `CalcgraphSession` re-binds a formula whose deps changed (`=SUM(A:A)` → `=SUM(B:B)`, or `=A1+B1` → `=C1+D1`), the OLD entries remain in the graph: stripe stays registered, `formula_to_range_deps` keeps the old range, AND (Phase 3.4) the old forward `add_edge(formula, dep)` edges stay too. Subsequent writes hit the stale entries + pass the precision check, producing false-positive dirty marks. **Phase 3.10 megaudit (W5-43) escalation: NOT performance-only.** H3 found: stale `B1 → A1` after `B1 = A1` rebound to `B1 = 1` plus new `A1 = B1` creates a FALSE `#CIRC!` cycle in Tarjan. H4: rebound range-dep formulas wrongly dirty on writes to the OLD range. Real correctness impact. | `ql-exec/src/calcgraph_session.rs::extract_and_register_deps` re-bind path; `ql-calcgraph/src/graph.rs::register_range_dependency` + `add_edge` have no remove counterparts | Engine | **Engine Phase 4** (delta-edge graph storage OR per-formula edge/stripe revocation API). Megaudit-deferred (correctness fix needs architectural choice — not safe to half-bake at Phase 3.10 close). |
| GAP-G-03 | Phase 3.10 megaudit H1: range-dep formulas don't create scheduler edges. `register_range_dependency` populates stripe + `formula_to_range_deps` but doesn't `add_edge(formula, dep_formula)` for formula cells inside the range. Tarjan can't see range-induced ordering or cycles. Example: `A1 = SUM(Sales)` where Sales covers A1 isn't detected as a self-cycle; `A1 = SUM(B1:B1)`, `B1 = A2 + 1` can evaluate A1 before B1. | `ql-exec/src/calcgraph_session.rs::extract_and_register_deps` (range branch only registers stripe, no graph edges) | Engine | Engine Phase 4.7 (array formulas + FormulaRegion binder) — needs lazy per-recompute range→formula edge expansion OR pre-computed region-binder edges |
