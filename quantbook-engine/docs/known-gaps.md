...
| GAP-T-01 | Time-zone offset acquisition for `NOW()` / `TODAY()` local-time policy not implemented. Phase 4.5.A.0 EvalContext design includes a `NowProvider` trait with `utc_offset_seconds()`. The native implementation needs either a small `iana_time_zone` crate dep (~50 LOC, single-purpose) or a libc FFI `localtime_r` call. Decision deferred to 4.5.A implementation — prototype both and pick the cheaper. WASM target uses a host callback (Phase 6.3). | Filed W5-68 (Codex Phase 4.5 review HIGH 4 fix); no code yet | Engine | Engine Phase 4.5.A (immediate) |
