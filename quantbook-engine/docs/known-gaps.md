...
**Date last touched:** 2026-05-13 (Engine Phase 4 entry — W5-49: GAP-G-01 + GAP-G-03 architectural decision)  
...
| GAP-G-01 | Phase 0 `Graph` edges and stripe entries are append-only. When `CalcgraphSession` re-binds a formula whose deps changed (`=SUM(A:A)` → `=SUM(B:B)`, or `=A1+B1` → `=C1+D1`), the OLD entries remain in the graph: stripe stays registered, `formula_to_range_deps` keeps the old range, AND (Phase 3.4) the old forward `add_edge(formula, dep)` edges stay too. Subsequent writes hit the stale entries + pass the precision check, producing false-positive dirty marks. **Phase 3.10 megaudit (W5-43) escalation: NOT performance-only.** H3 found: stale `B1 → A1` after `B1 = A1` rebound to `B1 = 1` plus new `A1 = B1` creates a FALSE `#CIRC!` cycle in Tarjan. H4: rebound range-dep formulas wrongly dirty on writes to the OLD range. Real correctness impact. **DECISION 2026-05-13 (W5-49):** Option A — per-formula revocation API for `Graph` + `StripeIndex` (clear_outgoing, clear_range_deps_for_formula, clear_for_formula with reverse-lookup `formula_to_stripe_keys`); wired into `extract_and_register_deps` at the same point as `remove_formula_deps`. See `docs/architecture/2026-05-13-graph-storage-decision.md` (Codex-reviewed). Implementation queued for next session. | `ql-exec/src/calcgraph_session.rs::extract_and_register_deps` re-bind path; `ql-calcgraph/src/graph.rs::register_range_dependency` + `add_edge` have no remove counterparts | Engine | **Engine Phase 4 pre-V2 (Option A)** — implement BEFORE Phase 4.3 V2 (SUMIF/COUNTIF/lookup). |
| GAP-G-03 | Phase 3.10 megaudit H1: range-dep formulas don't create scheduler edges. `register_range_dependency` populates stripe + `formula_to_range_deps` but doesn't `add_edge(formula, dep_formula)` for formula cells inside the range. Tarjan can't see range-induced ordering or cycles. Example: `A1 = SUM(Sales)` where Sales covers A1 isn't detected as a self-cycle; `A1 = SUM(B1:B1)`, `B1 = A2 + 1` can evaluate A1 before B1. **DECISION 2026-05-13 (W5-49):** small Option C — Tarjan supplemental adjacency at recompute time (build `supplemental: HashMap<NodeId, Vec<NodeId>>` of temp F→G edges between dirty formula nodes; pass to new `topo::schedule_with_supplemental`). NOT FormulaRegion binder for now; Option B deferred to Phase 4.7. Implementation order: A before C (C reads `formula_to_range_deps` which A makes current). See `docs/architecture/2026-05-13-graph-storage-decision.md`. | `ql-exec/src/calcgraph_session.rs::extract_and_register_deps` (range branch only registers stripe, no graph edges); `ql-calcgraph/src/topo.rs` (Tarjan walks `graph.outgoing(v)` only) | Engine | **Engine Phase 4 pre-V2 (small Option C, after Option A)** — implement BEFORE Phase 4.3 V2. Phase 4.7 still owns the FormulaRegion binder for array formulas + SIMD region dispatch (GAP-G-02). |
