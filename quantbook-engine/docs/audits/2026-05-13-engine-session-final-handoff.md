...
9. **(IMMEDIATELY NEXT) — Phase 4.5.A part 2** — Add `Workbook::date_system: DateSystem` field + `.qbook` schema v2→v3 migration + `WorkbookEnv::eval_context()` override + NOW/TODAY upgrade to use `ymd_to_serial` (replaces the approximate 25569 flat offset).
