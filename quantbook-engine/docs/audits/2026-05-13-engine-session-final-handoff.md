...
11. **Phase 4.5.B wave 2** ✅ SHIPPED W5-73 — DATEVALUE (ISO + US slash + 2-digit year), TIMEVALUE (24h + 12h), WEEKDAY (all 9 return_type variants), EOMONTH (with phantom-Feb-1900 = 60), EDATE (day-clamp). 13 of 18 V1 fns shipped cumulatively. Registry 116 → 121.
12. **Phase 4.5.B wave 3** ✅ SHIPPED W5-74 — DAYS / NETWORKDAYS / WORKDAY / YEARFRAC. **V1 WAVE CLOSES 18/18.** 4 V1 divergences filed (GAP-F-08..11). Registry 121 → 125.
13. **Phase 4.5.C V2 wave 4/6** ✅ SHIPPED W5-75 — DATEDIF / DAYS360 / WEEKNUM / ISOWEEKNUM. NETWORKDAYS.INTL + WORKDAY.INTL inherit tier-4 dependency (deferred).
14. **Phase 4.5 mega-audit closure** ✅ SHIPPED W5-76 — Codex + Sonnet parallel mega-audit on the W5-68→W5-75 arc caught **3 HIGH (1 shared) + 5 unique MEDIUM** issues, all closed. **HIGH-1** YEARFRAC basis 0 vs DAYS360 FALSE inconsistency — refactored to share `days360_count` US/NASD helper; basis 4 kept Excel-quirk non-adjusted form (`YEARFRAC(2024-01-01, 2024-12-31, 4) == 1.0`) and divergence documented. **HIGH-2** WEEKDAY return-type 12-16 anchor off-by-one — `n - 11` → `n - 10`, exhaustive tests added. **HIGH-3** ISOWEEKNUM `#NUM!` at Excel1900 serial 1 / Excel1904 serial 0 — algorithm rewritten in unix_days space; documented divergence from `WEEKDAY` for Excel1900 serials 1..=60. Plus MEDIUMs: serial-0 policy unified (WEEKDAY/WORKDAY/NETWORKDAYS/YEARFRAC reject Excel1900 serial 0); DATEVALUE 2-digit-year comment corrected; EDATE phantom-day comment de-duplicated; WASM clock guard added (`#NUM!` instead of panic); `Workbook::set_date_system` docstring warns about `WorkbookEnv` cache; GAP-T-01 reclassified from 4.5.A → 4.10 polish. **7 gates green** (1630 workspace tests passed; +30 W5-76 tests; fmt + clippy `-D warnings` + build-flags + lock-pins + multiversion clones + cargo audit). **Engine Phase 4.5 V1+V2 wave FULLY CLOSED.**
15. **(IMMEDIATELY NEXT — choice)** Phase 4.5.D format parser (~2-3 sessions, highest user-visible impact) OR Phase 4.5.E TEXT() + locale stubs (~0.5 session) OR move to Phase 4.6 cross-sheet refs.
