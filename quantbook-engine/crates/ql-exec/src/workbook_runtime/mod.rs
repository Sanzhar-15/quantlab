...
// **Tier D1 Step 1 (2026-05-18 — Phase 5 prep, monolith split):**
// error + result types extracted to `error.rs` per
// `docs/architecture/workbook-runtime-split-design.md`. Re-exported
// here so callers continue to import `RuntimeError` /
// `RecomputeFailure` / `RecomputeResult` from `crate::workbook_runtime`
// unchanged.
mod error;
...
impl<'a> WorkbookRuntime<'a> {
    ...
    pub fn rename_sheet(
        ...
        let new_name_arc: std::sync::Arc<str> = std::sync::Arc::from(new_name.as_str());
...
mod tests {
    ...
    // ===================================================================
    // W5-146 (Phase 4.9.K) — `set_reference_mode` / `set_locale` runtime
    // API tests.
    // ===================================================================
