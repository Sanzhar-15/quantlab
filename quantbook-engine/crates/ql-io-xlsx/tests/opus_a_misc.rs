...
fn sheet_with_only_blank_overlay_cells_followed_by_normal_sheet() {
    ...
    println!("Empty.overlay (5,3) = {:?}", sheet0.format_overlay().get(5, 3));
    println!("Normal.read(0,0) = {:?}", sheet1.read(0, 0));
    println!("Normal.overlay (0,0) = {:?}", sheet1.format_overlay().get(0, 0));
    let _ = std::fs::remove_file(&tmp);
}
...
fn strict_update_original_does_not_leave_partial_output() {
    ...
    let mut buf: Vec<u8> = Vec::new();
    {
        ...
        let parts: &[(&str, &str)] = &[
            ...
            ("xl/drawings/drawing1.xml", r#"<?xml version="1.0"?><wsDr/>"#),
        ];
        for (n, c) in parts {
    ...
    println!("strict outcome: {:?}", r2.as_ref().err().map(|e| format!("{e}")));
    ...
    println!("file content (first 24 bytes): {:?}",
