...
status: |
  done — SUPERSEDED 2026-05-26 by Phase 5 COMPLETE (5.8 megaudit PASS-WITH-FINDINGS) +
  Phase 6 decision-lock + 6.1A. This V3.6.1 backlog mini-phase was a Phase 5.7 sub-phase;
  its V3.6.1.1 (B10 delta consumer) + V3.6.1.2 (shared delta cache) SHIPPED. Remaining
  backlog items (B9 removedCells, CellValueJson retype, coverage gaps, D7 #REF!, incremental
  DOM) are all v1.5-deferred / non-blocking and tracked in memory/current_work.md +
  docs/phase5/megaudit-5-8/closures.md §5. The active plan is now the Phase 6 6.1 plan.
...
- **CellValueJson type/runtime drift cleanup** — flagged in the V3.6 phase-termination recommended-next list.  **Scoped 2026-05-26 (read-only):** `types.ts` `CellValueJson` is documented as a discriminated union but is actually a loose bag (`{ kind: 'number'|'boolean'|'text'|'error'|'pending'; number?; boolean?; text?; error? }`), so the compiler can't enforce kind↔payload; `extractSheetSnapshot` (cellGridLogic.ts) defensively runtime-validates each payload with `[bad_argument]` throws.  Cleanup = convert to a real TS discriminated union.  SMALL + LOW-RISK scope (only 2 src refs: the def + `extractSheetSnapshot`; 5 test refs already build valid union members; napi runtime shape unaffected — flat object still structurally satisfies a union variant).  Keep the defensive throws as binding-drift guards.  It IS a code cycle → fresh session.
