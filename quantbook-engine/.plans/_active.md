...
status: in-progress (V3.5.0.1 lock `48be1878db5`; V3.5.0.2 WorkbookSnapshot napi engine `bbd129ed629` + IDE `299fed5eda9`; V3.5.0.3a renameSheet engine `74aa5039e04` + IDE `5ad934d087f`; V3.5.0.3b deleteSheet + Op::RemoveSheet (tombstone) engine `9be3672ca01` + IDE `91a6b3870bd`; V3.5.0.3c moveSheet + Op::MoveSheet (display-order overlay) engine `7ec7877a018` + IDE `e3642ef3458` (+ deep-audit closures engine `fa7541f1e8d`); V3.5.0.4a IDE sheet management commands engine `1be56176fd2` + IDE `8fecc25c03f`; V3.5.0.4b cell-grid render() migration engine `75963628c54` + IDE `0d4659e62d6`; **V3.5.0.5 CellState format extension (D1) shipped** at engine (THIS commit) + IDE (paired) +12 ql-collab (81 -> 93) +7 IDE mocha (330 -> 337).  V3.5.0.5 ships per-cell SetCellFormat cache integration (engine CellState gains format field + new CacheEffect variant + napi FormatIdJson struct + CellSnapshotJson.format passthrough); session-wide RegisterFormat FormatTable cache + format-aware buildHtml rendering DEFERRED to V3.6+.  V3.5.0.4c sheet-tabs UI scaffold REMAINS PENDING (may defer to V3.5.1+).  Next: V3.5.0.6 partial-invalidate undo (D2) -- engine-only; cell-keyed undo dispatches to new invalidate_cell, non-cell-keyed falls back to rebuild_snapshot_cache.)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.5 -- CellState.format extension + CacheEffect::SetCellFormat handler + napi FormatIdJson struct + CellSnapshotJson.format passthrough + 12 ql-collab tests.  Per-cell SetCellFormat cache integration ships; session-wide RegisterFormat FormatTable cache deferred to V3.6+.  Rule 4 arc terminus HELD at 6 (FormatId composition of primitives all positive Send+Sync via Builtin(u32) | Custom(PeerId(u64), u32) walk; FormatIdJson napi struct positive walk).  Predecessor `75963628c54` (V3.5.0.4b plan).
current_ide_head: (this commit pair) V3.5.0.5 IDE -- types.ts FormatIdJson interface + CellSnapshotJson.format?: FormatIdJson optional field + 7 mocha tests covering type-shape + extractSheetSnapshot-drops-format + workbookSnapshot passthrough.  V3.5.0.5 scope is PASSTHROUGH only -- extractSheetSnapshot deliberately drops format (V3.2.a QuantbookCellSnapshot has no format; buildHtml not format-aware until V3.6+).  Predecessor `0d4659e62d6` (V3.5.0.4b IDE).
current_mocha_count: 337 / 337 (V3.5.0.5 adds +7 over 330; V3.5 cumulative +83 over V3.4.0.X 254 baseline)
current_ql_collab_tests: 93 / 93 (V3.5.0.5 adds +12 over 81 V3.4.0.X exit baseline: set_cell_format_writes_state_format / preserves_value_and_formula_per_field_lww / put_value_after_set_cell_format_preserves_format / lww_overwrites_previous_format / clear_cell_format_clears_state_format / format_only_clear_removes_cache_key / clear_format_on_never_set_cell_is_noop / cross_peer_set_cell_format_converges_via_merge_bytes / rebuild_snapshot_cache_round_trip_preserves_format / batch_commit_with_set_cell_format_recurses_into_cache / set_cell_format_custom_variant_via_cache / undo_preserves_format_lww_after_full_rebuild)
