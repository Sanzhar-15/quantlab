...
status: in-progress (V3.5.0.1 lock `48be1878db5`; V3.5.0.2 WorkbookSnapshot napi engine `bbd129ed629` + IDE `299fed5eda9`; V3.5.0.3a renameSheet napi engine `74aa5039e04` + IDE `5ad934d087f`; **V3.5.0.3b deleteSheet napi + Op::RemoveSheet shipped** at engine (THIS commit) + IDE (paired) +11 mocha (273 -> 284) with CRDT semantic locked to tombstone-preserving-id-slot.  V3.5.0.3c moveSheet + new Op::MoveSheet is the next sub-step (PENDING; hardest of the three due to reorder-vs-sheet-id-stability tension).)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.3b deleteSheet napi + Op::RemoveSheet + Workbook tombstone (HashSet<SheetId> removed_sheets field + remove_sheet/is_sheet_removed accessors + apply_op handler + cell-keyed apply_op silent-no-op for tombstoned sheets + workbook_snapshot tombstone filter).  Predecessor `74aa5039e04` (V3.5.0.3a renameSheet).
current_ide_head: (this commit pair) V3.5.0.3b IDE -- types.ts deleteSheet interface + session.ts wrapper + 11 mocha tests.  Predecessor `5ad934d087f` (V3.5.0.3a IDE).
current_mocha_count: 284 / 284 (V3.5.0.3b adds +11 over 273)
current_ql_collab_tests: 81 / 81 (V3.4.0.X exit baseline; V3.5.0.3b napi method + Op::RemoveSheet round-trip tests live in IDE mocha not ql-collab; engine-side replay correctness is covered indirectly via IDE mocha round-trips since the napi path exercises replay)
