...
status: COMPLETE (V3.4.0.1 lock `a87c31eed4f` -> V3.4.0.2 cache `1ae59a1a1aa` -> V3.4.0.3 undo/redo engine `7d63ff53132` + IDE `673792af05f` -> V3.4.0.5a presence napi engine `5ce55f65739` + IDE `8ff2d81f1e4` -> V3.4.0.5b IDE presence engine `cd26a37b513` + IDE `1744cee11af` -> V3.4.0.4a engine napi engine `bef220d7d3c` + IDE `ec9b59db8a5` -> V3.4.0.4b IDE Save As/Open + UUID PeerId engine `a1e2c673381` + IDE `fa7ec198cee` -> drift fixes `b2b3775fcc8` + `30ae2d30b44` + `6cfaaffb01c` -> V3.4.0.7 docs `fe10a245d07` -> MASTER-PLAN sweep `b2052797f9a` -> **V3.4.0.X cumulative megaudit + closures (THIS commit pair)**.  Both lanes returned PASS-WITH-FINDINGS; 3 HIGH + 3 MED + 3 LOW closed in-cycle; 3 MED doc-deferred; 5 LOW V3.4.1+ backlog.  Tests: ql-collab 79 -> **81**; IDE mocha 228 -> **254**.  Rule 4 arc terminus HELD at 6.  R-V3.4-3 RECLASSIFIED to KNOWN-GAP via Opus M2.  V3.4 phase COMPLETE; next session entry = V3.5 (rebuild_workbook IDE-side consumption + full Op enum cache + partial-invalidate undo if profiling justifies).)
date: 2026-05-23
...
current_engine_head: (this commit) V3.4.0.X closures -- engine fixes (CacheEffect hoisted + collect_cache_effects recursive walker for BatchCommit + apply_cache_effect shared mutator with formula-only-key removal + addSheet chunkRows validation + fromQbook D5 docstring rewrite + rebuild_snapshot_cache LWW invariant doc) + +2 ql-collab tests + ide-consumer-contract.md § 4.1.z4 audit-closures sub-section (~120 lines).  Predecessors: b2052797f9a (MASTER-PLAN V3.4.0.7 sweep) + fe10a245d07 (V3.4.0.7 docs § 4.1.z4 V3.4 surface spec) + 6cfaaffb01c (V3.4 plan 2nd-pass drift closure) + 30ae2d30b44 (ASCII-arrow normalize) + b2b3775fcc8 (V3.4 deep-audit drift fixes) + 6f7eb20cf44 (Cargo.lock) + a1e2c673381 (V3.4.0.4b plan + D5 deviation) + bef220d7d3c (V3.4.0.4a engine napi).
current_ide_head: (this commit pair) V3.4.0.X IDE closures -- HIGH-3 addSheet seeding in 3 commands + MEDIUM-2 typing:false commit broadcast in cellGridHtml + MEDIUM-3 validatePresenceNumeric helper in cellGridLogic + dispatcher numeric-validation arm + LOW-1+2 docstring rewrites (sessionFromQbook + generateUuidPeerId) + +26 mocha tests.  Predecessor: fa7ec198cee (V3.4.0.4b IDE Save As/Open + UUID PeerId + 4 mocha tests).
current_mocha_count: 254 / 254 (V3.4.0.X adds +26 over 228; V3.4 cumulative +73 over V3.3.0 181 baseline).
current_ql_collab_tests: 81 / 81 (V3.4.0.X adds +2 over 79: cell_state_formula_only_clear_removes_cache_key + cell_state_batch_commit_recurses_into_cache).
...
**Decision: B.** Implementation (ORIGINAL LOCK -- **DEVIATED at V3.4.0.4b**; see R-V3.4-4 + sub-step 4b for the actual shipped contract):
- ~~Session creation (`createSession(peerId)`): if caller passes explicit `peerId`, use it; otherwise generate `BigInt(uuid_truncated_to_u64)`.~~ ✅ SHIPPED at V3.4.0.4b as IDE-side `generateUuidPeerId()` helper (engine layer stays peer-id-agnostic).
- ~~`.qbook` envelope v3 adds `peer_id_for_this_session: u64` field~~ **DEVIATED**: persisted-per-workbook PeerId had unsolvable two-windows-same-workspace collision (both windows would read same stashed PeerId).  Resolution: fresh-UUID-per-session, NOT persisted.  Envelope stays at v2 (per D3 minimal-ship; no schema bump needed).
- ~~`from_qbook(path, peer_id_override?)`: if `peer_id_override` is passed, use it; otherwise reuse the envelope's `peer_id_for_this_session`~~ **DEVIATED**: `from_qbook(path, peer_id_override: BigInt)` REQUIRES override (not Option); IDE caller always generates fresh UUID via `generateUuidPeerId()`.
- ~~IDE side: `quantlab.quantbookOpen` passes `undefined` for first-open / fresh UUID for "Join as New Peer"~~ **DEVIATED**: every Open generates fresh UUID; no first-open vs rejoin distinction.  Cross-restart op-attribution continuity DEFERRED to V3.4.1+ IF a user-facing "show my contributions" feature surfaces; for V3.4.0 scope the trade-off is accepted (CRDT-correct; closes R-V3.3-5 fully).

## V3.4.0 sub-steps

1. **V3.4.0.1 -- decision lock** ✅ SHIPPED at engine `a87c31eed4f` (docs-only). Decisions D1-D5 above (D5 subsequently DEVIATED at V3.4.0.4b -- see sub-step 4b below + R-V3.4-4 + D5 lock annotation).
...
4. **V3.4.0.4 -- engine + IDE `.qbook` persistence (D3 + D5).** ✅ SHIPPED (a + b).  REORDERED to ship AFTER V3.4.0.5 + SPLIT a/b like V3.4.0.5.
...
5. **V3.4.0.5 -- presence integration with cell-grid (D4).** ✅ SHIPPED (a + b).  REORDERED (2026-05-23): engine napi shipped FIRST; IDE integration followed in a separate sub-step.
...
6. **V3.4.0.6 -- mocha tests** ✅ EFFECTIVELY COMPLETE via incremental per-sub-step coverage (target was "~15 new"; actual shipped is **47 V3.4-specific mocha tests** across 0.2 +3 / 0.3 +13 / 0.4a +5 / 0.4b +4 / 0.5a +9 / 0.5b +16, plus 3 ql-collab unit tests for cache invariants).  Covered: cache-after-undo via PutFormula (0.2); undo-redo round-trip via cache (0.3); .qbook round-trip preserves cache + cells + listSheets + sheet boundaries (0.4a); UUID PeerId uniqueness + non-zero + u64-range + engine round-trip (0.4b); presence updatePresence/peerPresence/clearPresence/sweepPresence + cross-peer mergeBytes enumeration + V1-persistence-known-limitation pin (0.5a); presence data block + decoration + presenceUpdate envelope validation + V3.2.a-compat read-only gate (0.5b).  Gaps NOT covered + DEFERRED: multi-window collab via .qbook (needs live VS Code; user-action gap); presence + scroll race guard (R-V3.4-3 DEFERRED + V3.4.0.5b explicit decision -- existing activeInput guard + full render cover the surface; no race-guard test to write).  V3.4.0.7 docs will inventory these gaps; V3.4.0.X audit may add edge-case mocha if cumulative findings surface them.
7. **V3.4.0.7 -- docs** ✅ SHIPPED at engine (THIS commit).  Inserted `ide-consumer-contract.md § 4.1.z4` (~460 lines; new section between § 4.1.z3 and the existing `---` separator).  Section structure follows V3.3.0.6 § 4.1.z3 template: status line + engine HEAD trail (a87c31eed4f -> 6cfaaffb01c); new engine napi surface (10 methods + PresenceStateJson struct + 6-code error surface table); V3.4.0.2 CellState hybrid cache (Rule 4 walk + per-field LWW + ghost-entry-avoidance + 7 op-mutation paths table + napi shape preservation); V3.4.0.3 undo/redo invalidation contract + Cmd-Z webview wiring with mid-edit guard + dispatcher envelope arms; V3.4.0.4a `.qbook` persistence (envelope v2 + 4-code persistence_error_to_napi + addSheet REAL SEMANTIC GAP closure + R-V3.4-6 CLOSED rationale + IDE Save As/Open commands lifecycle); V3.4.0.4b D5 DEVIATION (full crypto.randomUUID code + two-windows-same-workspace collision avoidance + entropy/non-zero/8-attempt-cap rationale + trade-off + V3.4.1+ revisit triggers); V3.4.0.5a/b presence integration (PresenceStateJson + V1 sweep contract + buildPresenceSnapshotJson + HTML cell-grid-presence data block + outline CSS rule + edit-mode broadcast + dispatcher arm + panel render/dispose lifecycle); 6 drift hazards for V3.x maintainers (CellState evolution / addSheet pre-PutValue / rebuild_workbook engine-only / D5 V3.4.1+ pitfall / presence data-block parallel-structure / UUID retry cap / presenceUpdate no-onCommit); V3.4 risk register R-V3.4-1..6 post-implementation reality (R-V3.4-7 dropped as VOIDED; R-V3.4-4 CLOSED via D5; R-V3.4-3 DEFERRED; R-V3.4-6 CLOSED via toQbook-saves-op_log-directly rationale); 12 out-of-scope items; live smoke procedure (steps 7-11 extending V3.3.0.6 steps 1-6: undo/redo with mid-edit guard pin + .qbook Save As + Open + two-window collab+presence + R-V3.4-5 LOCAL-ONLY undo pin + multi-window same-workbook persistence with D5 fresh-UUID-per-open pin).  Total file size 837 -> 1280 lines.  **Live VS Code smoke remains user-action gap** as designed; procedure now exists for the user to execute at their leisure.  V3.4.0.X parallel megaudit can now run with the full V3.4 spec available as audit-baseline.
8. **V3.4.0.X -- parallel Codex+Opus megaudit + closures** ✅ SHIPPED (THIS commit pair, engine + IDE).  Both lanes returned PASS-WITH-FINDINGS.  **Codex Lane A** (protocol/correctness, transcript `docs/audits/2026-05-24-phase-5-7-v3-4-0-x-codex.md`): 3 HIGH + 3 MEDIUM + 3 LOW.  **Opus Lane B** (adversarial design + Rule 4 walks, transcript `docs/audits/2026-05-24-phase-5-7-v3-4-0-x-opus.md`, ~1350 lines): 1 HIGH + 6 MEDIUM (M6 self-demoted on re-verify) + 5 LOW.  **Cross-lane convergent: 1 HIGH** (Codex H3 + Opus H1 -- `quantlab.quantbookCellGrid` sample-data path lacks `addSheet` -> `quantlab.quantbookSaveAs` fails replay).  **In-cycle closures: 3 HIGH + 3 MED + 3 LOW**: HIGH-1 BatchCommit cache recursion (engine `collect_cache_effects` + `apply_cache_effect` shared helpers); HIGH-2 addSheet chunkRows validation (napi `u32` -> `f64` + `validate_u32_index` + `>= 1` check); HIGH-3 IDE addSheet seeding in `quantlab.quantbookCellGrid` (+ carry to collab variant + multiWindowDemo); MEDIUM-1 ClearFormula formula-only ghost-key removal; MEDIUM-2 typing:false commit broadcast (moved before if(commit) branch in `endEdit`); MEDIUM-3 presenceUpdate numeric validator (new `validatePresenceNumeric` helper); LOW-1 D5 docstring fixes (`fromQbook` napi + `sessionFromQbook` IDE wrapper); LOW-2 UUID entropy doc correction (~60 bits / ~2^30 collision, not ~64 / ~2^32); Opus M5 LWW iteration-order invariant docstring in `rebuild_snapshot_cache`.  **Doc-deferred: 3 MED**: Opus M1 cross-WINDOW collision class expansion (V3.4.1+ if per-workbook stash feature surfaces); **Opus M2 R-V3.4-3 RECLASSIFIED from DEFERRED to KNOWN-GAP** (host-driven `webview.html` rebuild during mid-edit destroys `<input>` -- V3.4.0.5c follow-up MUST add host-level mid-edit-render guard); Opus M4 console.warn dispose-handler policy (already documented inline; § 4.1.z4 entry).  **Backlog: 5 LOW** (Opus L1-L5 polish: PresenceStateJson `#[non_exhaustive]` / hardcoded "quantbook" workbook name / UUID BigInt allocation / dispose try/catch / presenceUpdate range-check (already in M3 closure)).  +2 ql-collab tests (79 -> 81); +26 IDE mocha tests (228 -> 254).  **Rule 4 arc terminus HELD at 6** -- no new Rust types introduced (only private `CacheEffect` DTO).  V3.4 phase COMPLETE; V3.5 entry-readiness highlights documented in Opus transcript § F.
...
- **R-V3.4-3 Presence + 2-window collab race** -- DEFERRED at V3.4.0.5b implementation.  Discovery: render() rebuilds the full HTML on every pollRemote merged tick (V3.2.c pollRemote integration + V3.3.0.3 cache invalidation), so presence decoration regenerates naturally with the rest of the panel.  The V3.3.0.4 `activeInput` mid-edit guard already prevents tbody clobber from the scroll handler.  No separate `presenceRepaintInFlight` flag was needed at V3.4.0.5b scope.  V3.4.0.5c (optional polish) may revisit IF live smoke surfaces races (e.g., presence-update repaint mid-scroll causing flicker); for V3.4.0 scope it's clean.
- **R-V3.4-4 PeerId reuse across .qbook save/load** -- CLOSED at V3.4.0.4b via D5 DEVIATION (fresh-UUID-per-session via crypto.randomUUID truncated to 64-bit BigInt, NOT persisted-per-workbook as originally locked). Implementation discovery: persisted-per-workbook PeerId has unsolvable two-windows-same-workspace collision (both windows would read the same stashed PeerId). Fresh-UUID-per-session is CRDT-correct (each session = distinct Loro peer; future ops get unique attribution) + closes R-V3.3-5 carryforward fully (UUID 2^64 keyspace vs PID ~2^15 effective). Trade-off accepted: no cross-restart "this peer is User A" op-attribution continuity; past ops keep original PeerId; future ops attributed to new session UUID. V3.4.1+ may revisit if a user-facing feature ("show my contributions") surfaces.
...
- **R-V3.4-6 .qbook save during pending-flush** -- CLOSED at V3.4.0.4a implementation.  `toQbook` saves `inner.op_log()` directly which contains ALL local ops (Loro stores ops on append; transport "flush" is the cross-peer sync, orthogonal to disk persistence).  The session's local op log IS the canonical state from this peer's perspective regardless of whether peers have observed it via transport.  No pending-flush hazard at V3.4 scope.  V3.4.1+ may add explicit synchronize-then-save IF user-facing "wait for peer confirmation before saving" feature surfaces (currently no use case justifies it).
- **R-V3.4-7 Presence sweep frequency** -- VOIDED at V3.4.0.5b implementation.  Pre-implementation premise (Phase 5.6 V2 `sweep_presence(threshold_secs)`) was WRONG: the engine `sweepPresence()` actually shipped without a threshold parameter and sweeps ALL presence entries unconditionally.  Periodic auto-sweep on a tick cadence would therefore clobber live remote peers' cursor decorations on every Nth tick.  Correct behaviour: sweep only on lifecycle events (panel.onDidDispose calls clearPresence per V3.4.0.5b panel lifecycle).  Threshold-based variant deferred to V3.4.1+ when engine ships `sweep_presence_with_threshold(secs)`.  Per-workbook stale-entry cleanup: caller-opt-in via `fromSnapshot` + `sweepPresence()` (rejoin-clean-slate pattern; documented in engine presence.rs V1 known limitation).
