...
current_engine_head: (this commit -- V3.6.0.X audit-of-D2 closures: CONVERGENT-HIGH-1 first-write-wins + CONVERGENT-MED-1 sort formats + CONVERGENT-MED-2 drop Builtin in walker + 5 regression tests + docstring/ide-consumer-contract sweeps) <- 0c3cbef8cea (V3.6.0.3 docs sweep) <- ffa598a1c15 (V3.6.0.3 D2) <- 08964962cf8 (V3.6.0.X audit-closure of V3.6.0.2) <- 89f5a5f9e0b (V3.6.0.2 D1) <- 4fd816ded43 (V3.6.0.1 lock)
current_ide_head: b159c899359 (V3.6.0.3 D2 IDE; unchanged by V3.6.0.X audit-of-D2 closure -- napi shape unchanged, only sort order + cache walker semantics changed engine-side)
current_mocha_count: 350 / 350 (V3.6.0.3 D2 ship +4; V3.6.0.X audit-of-D2 closure +0 since napi shape unchanged + sort order is deterministic-stable across 4 existing shape tests)
current_ql_collab_tests: 124 / 124 (V3.5 cumulative +34 + V3.6 cumulative +9: V3.6.0.2 +0 (2 V3.5 tests rewritten) + V3.6.0.X audit-closure-of-V3.6.0.2 +2 + V3.6.0.3 D2 +2 + V3.6.0.X audit-of-D2 closure +5)
...
4. **V3.6.0.4 -- Per-cell op-index for invalidate_cell (D3)**. New `cell_op_index` field + 6-mutation-site sync invariant + debug-asserts.  V3.6.0.X audit-of-D2 Opus LOW-1 also recommends bundling all cache buckets into a `CacheBuckets<'a>` struct at this ship to prevent forgetfulness.  ql-collab regression tests + perf comparison vs V3.5.0.6 invalidate_cell. Estimated 3 sessions.
