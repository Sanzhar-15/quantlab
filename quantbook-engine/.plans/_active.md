...
status: in-progress (V3.6.0.1 decision lock SHIPPED at engine `4fd816ded43`; V3.6.0.2 D1 Loro UndoManager on_push wiring SHIPPED at engine `(THIS commit)` -- removes `pure_local_frontier` field + dispatch gate; new `pending_undo_cells: Arc<Mutex<Option<Vec<(u16, u32, u32)>>>>` field + on_push callback that encodes cells as `LoroValue::List<List<I64>>` into `UndoItemMeta::value`; `undo()` / `redo()` read via `top_undo_meta()` / `top_redo_meta()` BEFORE Loro call + pre-stage cells for the synthetic inverse op's on_push; both V3.5.0.X regression tests REWRITTEN to verify partial-invalidate FIRES correctly post-V3.6.0.2 (not full-rebuild); +0 net ql-collab tests (115/115, 2 rewrites + 0 new); +0 IDE mocha (346/346 unchanged); +0 engine workspace tests (all 30 suites still pass); Rule 4 arc terminus HELD at 6 (positive Send+Sync per-field walk on Arc<Mutex<Option<Vec<(u16, u32, u32)>>>> documented in field docstring; one field swapped for another; 0 new triggers).  V3.6.0.3+ pending fresh session per CLAUDE.md "Never run >2 plan-implement-audit cycles in one session"; V3.6.0.2 audit cycle also fresh session.)
date: 2026-05-24
