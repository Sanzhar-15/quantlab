...
status: in-progress (V3.5.0.1 lock `48be1878db5`; V3.5.0.2 WorkbookSnapshot napi engine `bbd129ed629` + IDE `299fed5eda9`; V3.5.0.3a renameSheet engine `74aa5039e04` + IDE `5ad934d087f`; V3.5.0.3b deleteSheet + Op::RemoveSheet (tombstone) engine `9be3672ca01` + IDE `91a6b3870bd`; V3.5.0.3c moveSheet + Op::MoveSheet (display-order overlay) engine `7ec7877a018` + IDE `e3642ef3458` (+ deep-audit closures at engine `fa7541f1e8d`); **V3.5.0.4a IDE sheet management commands shipped** at engine (THIS commit; plan only) + IDE (paired) +16 mocha (297 -> 313).  V3.5.0.4 SPLIT into 3 sub-sub-steps per scope discovery: 0.4a sheet management commands (rename/delete/move/add using V3.5.0.3 engine napi); 0.4b cell-grid rendering migration to workbookSnapshot(); 0.4c sheet-tabs UI scaffold.  V3.5.0.4b is the next sub-step.)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.4a plan-only -- documents the V3.5.0.4 split (a/b/c sub-sub-steps) + marks 0.4a SHIPPED at IDE.  No engine code changes (V3.5.0.4a is IDE-only: it consumes the V3.5.0.3 engine napi already shipped).  Predecessor `fa7541f1e8d` (V3.5 deep-audit closures).
current_ide_head: (this commit pair) V3.5.0.4a IDE -- 4 new sheet management commands (quantbookSheetAdd / SheetRename / SheetDelete / SheetMove) wiring V3.5.0.3 engine napi (addSheet/renameSheet/deleteSheet/moveSheet) into vscode QuickPick/InputBox/WarningMessage flows; 2 new pure helpers (buildSheetManagementQuickPickItems + buildSheetMovePositionItems) for the picker UIs; package.json + package.nls.json registrations.  +16 mocha tests covering both helpers.  Predecessor `e3642ef3458` (V3.5.0.3c IDE).
current_mocha_count: 313 / 313 (V3.5.0.4a adds +16 over 297; V3.5 cumulative +59 over V3.4.0.X 254 baseline)
current_ql_collab_tests: 81 / 81 (V3.4.0.X exit baseline; V3.5.0.4a is IDE-only -- no ql-collab additions)
...
4. **V3.5.0.4 -- IDE WorkbookSnapshot consumption + sheet management UI commands.** SPLIT into 3 sub-sub-steps per scope discovery (~2-3 sessions total): 0.4a sheet management commands (uses V3.5.0.3 engine napi directly; self-contained); 0.4b cell-grid rendering migration to workbookSnapshot()-driven (touches existing render path); 0.4c sheet-tabs UI scaffold (optional polish; may defer to V3.5.1+).
   ...
   - **V3.5.0.4b (cell-grid rendering migration to workbookSnapshot)** ⏭ PENDING (next session+).  Migrate `cellGridPanel.ts::render()` to call `workbookSnapshot()` instead of `exportSnapshot(sheet)`.  Naturally feeds multi-sheet display-order from V3.5.0.3c; reflects sheet rename/delete/move immediately (no pollRemote tick wait).  Risks: existing render path is the V3.4.0.X-era code; touching it may surface latent bugs.  Estimated 1-2 days.
