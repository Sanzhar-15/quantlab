...
status: in-progress (V3.5.0.1 lock `48be1878db5`; V3.5.0.2 WorkbookSnapshot napi engine `bbd129ed629` + IDE `299fed5eda9`; V3.5.0.3a renameSheet engine `74aa5039e04` + IDE `5ad934d087f`; V3.5.0.3b deleteSheet + Op::RemoveSheet (tombstone) engine `9be3672ca01` + IDE `91a6b3870bd`; V3.5.0.3c moveSheet + Op::MoveSheet (display-order overlay) engine `7ec7877a018` + IDE `e3642ef3458` (+ deep-audit closures engine `fa7541f1e8d`); V3.5.0.4a IDE sheet management commands engine `1be56176fd2` + IDE `8fecc25c03f`; **V3.5.0.4b cell-grid render() migrated to workbookSnapshot() + reactive panel title shipped** at engine (THIS commit; plan only) + IDE (paired) +17 mocha (313 -> 330).  V3.5.0.4 SPLIT: 0.4a SHIPPED (sheet management commands); 0.4b SHIPPED (this -- render() + panel title migrated; V3.5.0.3 sheet ops reflect immediately + addSheet'd-but-empty sheets count in title); 0.4c sheet-tabs UI scaffold PENDING (may defer to V3.5.1+).  Next: V3.5.0.4c sheet-tabs OR V3.5.0.5 (CellState format extension; D1).)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.4b plan-only -- marks 0.4b SHIPPED at IDE.  No engine code changes (V3.5.0.4b is IDE-only: cellGridPanel render() migration to workbookSnapshot()).  Predecessor `1be56176fd2` (V3.5.0.4a plan).
current_ide_head: (this commit pair) V3.5.0.4b IDE -- cellGridPanel.ts::render() migrated from exportCellSnapshot(this.session, this.sheet) to workbookSnapshot(this.session) + extractSheetSnapshot transformer; reactive panel title using workbookSnapshot.sheets.length (was point-in-time at show() per V3.3.0.X reactive-panel-title backlog -- backlog CLEARED).  New `extractSheetSnapshot(snapshot, sheetId)` pure helper in cellGridLogic.ts bridges V3.5.0.2 WorkbookSnapshotJson shape -> V3.2.a QuantbookCellSnapshot shape (preserves buildHtml contract; option (i) minimal-change scope per V3.5.0.4b entry decision).  Skip formula-only cells (V3.4.0.X MEDIUM-1 carry).  Sheet-not-found (tombstone race after V3.5.0.4a delete) -> render empty + console.warn (No-Fallbacks tension acknowledged in docstring).  +17 mocha tests covering both pure transformer (11: empty / found+empty / not-in-snapshot / 4 kind conversions / formula-only skip / unknown-kind throw / missing-payload throw / multi-sheet extract / row+col ordering preservation) and round-trip integration via real session (6: rename reflects immediately / delete -> null / move id-stability / empty-addSheet'd reachable / V3.5.0.4b matches V3.4.0.X exportCellSnapshot per-cell shape / panel title counts addSheet-only sheets).  Predecessor `8fecc25c03f` (V3.5.0.4a IDE).
current_mocha_count: 330 / 330 (V3.5.0.4b adds +17 over 313; V3.5 cumulative +76 over V3.4.0.X 254 baseline)
current_ql_collab_tests: 81 / 81 (V3.4.0.X exit baseline; V3.5.0.4b is IDE-only -- no ql-collab additions)
