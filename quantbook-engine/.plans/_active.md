...
status: in-progress (V3.4.0.1 lock at engine `a87c31eed4f`; V3.4.0.2 cache at `1ae59a1a1aa`; V3.4.0.3 undo/redo at engine `7d63ff53132` + IDE `673792af05f` (+13 mocha 181 -> 194); V3.4.0.5a presence engine napi shipped at engine (THIS commit) + IDE (also this commit pair) with +9 mocha (194 -> 203). V3.4.0.4 .qbook persistence reordered AFTER V3.4.0.5 per scope discovery (4 new crate deps + rebuild_workbook routing). V3.4.0.5b IDE cell-grid presence integration is the next sub-step.)
date: 2026-05-23
...
current_engine_head: (this commit) V3.4.0.5a engine -- PresenceStateJson napi struct + 5 wrappers + plan reorder
current_ide_head: (this commit) V3.4.0.5a IDE -- types + 5 typed wrappers + 9 mocha tests
current_mocha_count: 203 / 203 (V3.4.0.5a adds +9 over 194)
current_ql_collab_tests: 79 / 79 (unchanged; engine presence methods were covered at Phase 5.6 V1/V2)
...
4. **V3.4.0.4 -- engine + IDE `.qbook` persistence (D3 + D5).** REORDERED (2026-05-23): now ships AFTER V3.4.0.5 engine napi per scope discovery -- the existing `ql_io::qbook_format::save_workbook_with_oplog` REQUIRES a fully-rebuilt `Workbook`, so the napi `to_qbook` impl needs to call `inner.rebuild_workbook(&default_registry())` internally + adds 4 new deps on the napi crate (`ql-io` + `ql-functions` + `ql-storage` + `uuid`).  Engine-side rebuild_workbook usage is permitted at V3.4 (IDE-side Workbook consumption stays V3.5+).  Envelope v2->v3 bump for D5 peerId persistence.  IDE commands `quantlab.quantbookSaveAs` + `quantlab.quantbookOpen` with showSaveDialog/showOpenDialog.  ~1-2 days; multi-session.
5. **V3.4.0.5 -- presence integration with cell-grid (D4).** REORDERED (2026-05-23): engine napi shipped FIRST (this commit pair); IDE integration follows in a separate sub-step.
   ...
   - **V3.4.0.5b (IDE cell-grid integration)** PENDING (next session).  Extend `CellGridPanel` to include presence data block (parallel to V3.3.0.4 `cell-grid-data` block); webview script decorates `<td>` cells with `data-peer-${peerId}` attributes + colored borders; presence updates posted as `{type:'presenceUpdate', deltas}` envelope; webview applies deltas to decorations.  Mid-presence guard (V3.4.0.1 D4 closure): `presenceRepaintInFlight` flag extending V3.3.0.4 `activeInput` boolean-flag pattern.  Sweep cadence decision (V3.4.0.1 R-V3.4-7): every 10th pollRemote tick (~10s at the V3.3.0.1 D6 1s polling cadence).
