...
status: in-progress (V3.5.0.1 lock `48be1878db5`; V3.5.0.2 WorkbookSnapshot napi engine `bbd129ed629` + IDE `299fed5eda9`; V3.5.0.3a renameSheet engine `74aa5039e04` + IDE `5ad934d087f`; V3.5.0.3b deleteSheet + Op::RemoveSheet (tombstone) engine `9be3672ca01` + IDE `91a6b3870bd`; **V3.5.0.3c moveSheet + Op::MoveSheet (display-order overlay) shipped** at engine (THIS commit) + IDE (paired) +13 mocha (284 -> 297) -- V3.5.0.3 D4 SHEET-OPS SUITE COMPLETE.  Next sub-step: V3.5.0.4 (IDE UI commands consuming the new sheet ops + WorkbookSnapshot-driven cell-grid rendering).)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.3c moveSheet napi + Op::MoveSheet + Workbook.sheet_display_order overlay (Vec<SheetId> field + move_sheet/sheet_display_order accessors + apply_op handler + workbook_snapshot iterates display order with defensive fallback).  Closes V3.5.0.3 D4.  Predecessor `9be3672ca01` (V3.5.0.3b deleteSheet).
current_ide_head: (this commit pair) V3.5.0.3c IDE -- types.ts moveSheet interface + session.ts wrapper + 13 mocha tests.  Predecessor `91a6b3870bd` (V3.5.0.3b IDE).
current_mocha_count: 297 / 297 (V3.5.0.3c adds +13 over 284; V3.5.0.3 D4 suite total = +32: 0.3a +8 + 0.3b +11 + 0.3c +13)
current_ql_collab_tests: 81 / 81 (V3.4.0.X exit baseline; V3.5.0.3 napi methods + Op round-trip tests live in IDE mocha not ql-collab; engine-side replay correctness is covered indirectly via IDE mocha round-trips since the napi path exercises replay)
