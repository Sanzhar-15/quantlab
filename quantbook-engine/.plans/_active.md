...
status: in-progress (V3.4.0.1 decision lock at engine `a87c31eed4f`; V3.4.0.2 hybrid CellState cache shape at engine `1ae59a1a1aa` (+3 ql-collab tests 76 -> 79); V3.4.0.3 undo/redo napi + Cmd-Z webview wiring shipped at engine (THIS commit) + IDE (also this commit pair) with +13 IDE mocha tests (181 -> 194). V3.4.0.4 .qbook persistence is the next sub-step.)
date: 2026-05-23
...
current_engine_head: (this commit) V3.4.0.3 engine -- napi undo/redo + plan update
current_ide_head: (this commit) V3.4.0.3 IDE -- types + wrappers + dispatcher arms + keydown wiring + 13 tests
current_mocha_count: 194 / 194 (V3.4.0.3 adds +13 over 181 baseline)
current_ql_collab_tests: 79 / 79 (unchanged from V3.4.0.2; engine undo/redo logic was already covered by V3.3.0.X HIGH-1 tests)
