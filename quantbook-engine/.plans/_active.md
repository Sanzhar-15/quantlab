...
status: in-progress (V3.5.0.1 lock at engine `48be1878db5`; **V3.5.0.2 engine WorkbookSnapshot napi shipped** at engine (THIS commit) + IDE (paired) +11 mocha (254 -> 265): 4 new #[napi(object)] structs (CellValueJson + CellSnapshotJson + SheetSnapshotJson + WorkbookSnapshotJson) + workbook_snapshot napi method routing through rebuild_workbook + snapshot_cells + IDE types.ts interfaces + session.ts typed wrapper.  V3.5.0.3 engine sheet rename/delete/move napi (D4) is the next sub-step.)
date: 2026-05-24
...
current_engine_head: (this commit) V3.5.0.2 -- WorkbookSnapshot napi (4 #[napi(object)] structs + workbook_snapshot method).  Predecessor `48be1878db5` (V3.5.0.1 lock).
current_ide_head: (this commit pair) V3.5.0.2 IDE -- types.ts interfaces + session.ts wrapper + 11 mocha tests.  Predecessor `b6a3f48572a` (V3.4.0.X IDE closures).
current_mocha_count: 265 / 265 (V3.5.0.2 adds +11 over 254)
current_ql_collab_tests: 81 / 81 (V3.4.0.X exit baseline; V3.5.0.2 napi method tests live in IDE mocha not ql-collab)
