...
current_engine_head: (this commit -- V3.6.0.X audit-of-D4 closures: Op::SetDateSystem + DateSystemWire + replay handler + from_qbook seed + Pending skip + locale passthrough + per-snapshot parse cache + 2 regression tests + doc sweep) <- 5b3017c70d2 (V3.6.0.5 D4 engine) <- f6ef41ad732 (V3.6.0.X audit-of-D3 closures) <- 84cb5c9d169 (V3.6.0.4 D3 MASTER-PLAN sweep) <- 119df9b3826 (V3.6.0.4 D3 ship) <- prior chain
current_ide_head: (this commit -- V3.6.0.X audit-of-D4 IDE: data-raw-value attribute + beginEdit reads it first + dateSystem TS docstring fix) <- d7cb01557d4 (V3.6.0.5 D4 IDE) <- 21fe6a73ec6 (V3.6.0.X audit-of-D2 IDE typedoc sweep)
