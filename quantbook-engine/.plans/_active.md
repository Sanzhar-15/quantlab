...
current_engine_head: (this commit, V3.6.0.6 D5 engine: appendPutFormula napi thin wrapper + plan sweep) <- 6700b588335 (docs-audit closures) <- e7931a74d0d (V3.6.0.X audit-of-D4 closures: Op::SetDateSystem + DateSystemWire + replay handler + from_qbook seed + Pending skip + locale passthrough + per-snapshot parse cache + 2 regression tests + doc sweep) <- 5b3017c70d2 (V3.6.0.5 D4 engine) <- f6ef41ad732 (V3.6.0.X audit-of-D3 closures) <- 84cb5c9d169 (V3.6.0.4 D3 MASTER-PLAN sweep) <- 119df9b3826 (V3.6.0.4 D3 ship) <- prior chain
current_ide_head: (this commit, V3.6.0.6 D5 IDE: types + wrapper + extractSheetSnapshot + data-raw-formula + beginEdit precedence + 19 mocha tests) <- 19c7e35873a (docs-audit closures IDE) <- b76b8266beb (V3.6.0.X audit-of-D4 IDE: data-raw-value attribute + beginEdit reads it first + dateSystem TS docstring fix) <- d7cb01557d4 (V3.6.0.5 D4 IDE) <- 21fe6a73ec6 (V3.6.0.X audit-of-D2 IDE typedoc sweep)
current_mocha_count: 373 / 373 (V3.6.0.6 D5 +19 over V3.6.0.X audit-of-D4 354 baseline: 6 appendPutFormulaValidated input validation + 4 engine napi contract + 3 workbookSnapshot surface (formula-only / rename-chain repair / round-trip) + 2 extractSheetSnapshot passthrough + 4 buildHtml emit (with/without/escape/precedence))
current_ql_collab_tests: 136 / 136 (V3.6.0.6 D5 +0 engine collab-side; thin napi wrapper has no new collab-side surface)
