...
current_engine_head: 30ae2d30b44 (ASCII-arrow normalize on MASTER-PLAN Phase 5 status header), predecessors b2b3775fcc8 (V3.4 deep-audit drift fixes -- MASTER-PLAN + plan + risk register) + 6f7eb20cf44 (Cargo.lock for V3.4.0.4a deps) + a1e2c673381 (V3.4.0.4b plan + D5 deviation) + bef220d7d3c (V3.4.0.4a engine napi).  This commit pair (next): plan internal-consistency drift fixes -- 7 items including D5 lock deviation annotation, R-V3.4-6 closure note, V3.4.0.X scope envelope-v3 removal, parent sub-step 0.4/0.5 ✅ markers, frontmatter HEADs.
current_ide_head: fa7ec198cee (V3.4.0.4b IDE -- generateUuidPeerId + quantbookSaveAs/Open commands + 4 mocha tests; unchanged from V3.4.0.4b ship since the deep-audit fixes are engine/plan-only).
current_mocha_count: 228 / 228 (V3.4.0.4b adds +4 over 224; V3.4 cumulative +47 over V3.3.0 181 baseline)
...
7. **V3.4.0.7 -- docs** ⏭ PENDING.  Extend `ide-consumer-contract.md § 4.1.z4` (new section) covering: new engine napi surface (undo + redo + toQbook + fromQbook + addSheet + presence 5-pack); V3.4.0.2 CellState hybrid cache shape + 7 op-mutation paths + Rule 4 walk; V3.4.0.3 undo invalidation contract + Cmd-Z webview wiring + mid-edit guard; V3.4.0.4a `.qbook` persistence at envelope v2 (no v3 bump; rebuild_workbook routing internal-only) + persistence_error_to_napi 4 codes; **V3.4.0.4b D5 DEVIATION**: fresh-UUID-per-session via crypto.randomUUID (NOT persisted-per-workbook as originally locked); rationale + cross-restart op-attribution-deferred trade-off; V3.4.0.5a/b presence integration (PresenceStateJson + 5 napi wrappers + cell-grid-presence data block + .cell-peer-presence decoration + presenceUpdate envelope + lifecycle: render-time fetch + dispose-time clearPresence); **V3.4 risk register R-V3.4-1..6** (drop R-V3.4-7 as VOIDED; restate R-V3.4-4 as CLOSED via D5 deviation; R-V3.4-3 DEFERRED); drift hazards for V3.x maintainers (UUID retry cap; addSheet pre-PutValue requirement; rebuild_workbook engine-side-only constraint; presence data-block parallel-structure to cell-grid-data; D5 deviation pitfall for V3.4.1+ if attribution feature surfaces); out-of-scope items (auto-save; multi-sheet Open picker; per-peer color hashing; threshold-based sweepPresence; per-workbook PeerId stash); **live smoke procedure** (extends V3.3.0.6 § 4.1.z3 procedure with: Cmd-Z/Cmd-Shift-Z workbook undo/redo; Quantbook: Save As -> showSaveDialog -> .qbook file written; Quantbook: Open -> showOpenDialog -> CellGridPanel restores state; two-window collab with presence decoration visible cross-window).
