...
status: in-progress (V3.4.0.1 decision lock shipped at engine `a87c31eed4f`; V3.4.0.2 hybrid CellState cache shape shipped at engine (THIS commit) with +3 ql-collab tests (76 -> 79) and napi JSON shape preserved (IDE mocha 181/181 holds). V3.4.0.3 engine + IDE undo/redo napi bindings is the next sub-step.)
date: 2026-05-23
...
current_engine_head: (this commit) V3.4.0.2 hybrid CellState cache shape + 3 new ql-collab tests
current_ide_head: 6dcc7ef9a20 (unchanged from V3.3.0.X; V3.4.0.2 is engine-only)
current_mocha_count: 181 / 181 (unchanged; V3.4.0.2 preserved JSON shape; V3.4.0.3 onward will add IDE mocha)
current_ql_collab_tests: 79 / 79 (76 pre-V3.4.0.2 + 3 V3.4.0.2: cell_state_formula_roundtrip + cell_state_clear_formula_invariant + cell_state_undo_preserves_invariants_after_full_rebuild)
