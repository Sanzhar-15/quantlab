...
status: in-progress (V3.4 entry plan drafted; V3.4.0.1 decision lock ships in THIS commit -- 5 design decisions (Op-enum cache shape, undo invalidation strategy, persistence schema-versioning gate, presence + virtualization race guard, PeerId derivation under persistence) locked per the V3.3.0.X Opus § V3.4 ENTRY READINESS packet. V3.4.0.2-V3.4.0.X is the implementation sequence; multi-week arc.)
date: 2026-05-23
...
current_engine_head: 6083ba9282a (V3.3.0.X follow-up -- archived V3.3 plan status=done + V3.3.0.X closure content)
current_ide_head: 6dcc7ef9a20 (V3.3.0.X IDE megaudit closures + 5 new mocha tests; final V3.3.0 IDE HEAD)
current_mocha_count: 181 / 181 (V3.3.0 exit baseline)
current_ql_collab_tests: 76 / 76 (74 base + 2 V3.3.0.X: undo_invalidates_snapshot_cache + force_clear_snapshot_cache_test_seam)
...
1. **V3.4.0.1 -- decision lock** (THIS commit, docs-only). Decisions D1-D5 above.
2. **V3.4.0.2 -- engine hybrid cache shape (D1).** Migrate `last_snapshot: HashMap<(u16, u32, u32), CellWireValue>` -> `HashMap<(u16, u32, u32), CellState>` where `CellState { value: Option<CellWireValue>, formula: Option<String> }`. Extend `rebuild_snapshot_cache` to handle `Op::PutFormula` + `Op::ClearFormula` in addition to `Op::PutValue`. PRESERVE napi `export_snapshot` JSON shape (`CellValueJson` discriminated union unchanged). Rule 4 per-field walk REQUIRED on `CellState` (new struct).
3. **V3.4.0.3 -- engine + IDE undo/redo napi bindings.** Engine: add `#[napi(js_name = "undo")] pub fn undo(&mut self) -> Result<bool>` + matching `redo`. Engine already has `CollabSession::undo` / `::redo` (post-V3.3.0.X HIGH-1 closure); napi binding is thin wrapper. IDE: typed wrappers `undo(session): boolean` + `redo(session): boolean` in `session.ts`. Webview cell-grid: bind Cmd-Z / Cmd-Shift-Z (Ctrl-Z / Ctrl-Y on Win/Linux) via keydown listener; post `{type:'undo'}` / `{type:'redo'}` envelope; host dispatcher routes to `session.undo()` / `session.redo()`; on consumed=true full re-render. Mocha tests for undo-then-redo round-trip via cache (not just op log).
4. **V3.4.0.4 -- engine + IDE `.qbook` persistence (D3 + D5).** Engine: bind `to_qbook(path: String) -> Result<()>` + `from_qbook(path: String, peer_id_override: Option<BigInt>) -> Result<CollabSession>` via napi. Engine implementation delegates to existing `ql_io::qbook_format`; envelope bumped to v3 ONLY IF the peerId persistence requires a new field (likely yes; document the v2->v3 migration). IDE: command `quantlab.quantbookSaveAs` (vscode.window.showSaveDialog -> `to_qbook`); command `quantlab.quantbookOpen` (showOpenDialog -> `from_qbook` -> CellGridPanel.show). UUID PeerId generation at session creation if no explicit override.
5. **V3.4.0.5 -- engine + IDE presence integration with cell-grid (D4).** Engine napi: bind `update_presence(state: PresenceStateJson)` + `peer_presence(peer: BigInt) -> Option<PresenceStateJson>` + `clear_presence()` + `sweep_presence(threshold_secs: f64) -> usize`. IDE: extend `CellGridPanel` to include presence data block (parallel to V3.3.0.4 `cell-grid-data` block); webview script decorates `<td>` cells with `data-peer-${peerId}` attributes + colored borders; presence updates posted as `{type:'presenceUpdate', deltas}` envelope; webview applies deltas to decorations. Mid-presence guard (D4 closure): `presenceRepaintInFlight` flag.
