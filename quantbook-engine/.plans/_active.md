...
status: in-progress (V3.4.0.1 lock at engine `a87c31eed4f`; V3.4.0.2 cache at `1ae59a1a1aa`; V3.4.0.3 undo/redo at engine `7d63ff53132` + IDE `673792af05f`; V3.4.0.5a presence napi at engine `5ce55f65739` + IDE `8ff2d81f1e4`; V3.4.0.5b IDE presence integration at engine `cd26a37b513` + IDE `1744cee11af`; **V3.4.0.4a .qbook persistence engine napi shipped** at engine (THIS commit) + IDE (paired) +5 mocha (219 -> 224). V3.4.0.4b IDE commands + UUID PeerId + live smoke is the next sub-step.)
date: 2026-05-23
...
current_engine_head: (this commit) V3.4.0.4a engine -- 3 new deps + 4 napi methods (toQbook/fromQbook/addSheet/persistence_error_to_napi) + plan update
current_ide_head: (this commit pair) V3.4.0.4a IDE -- types + 3 typed wrappers + 4 new QuantbookErrorCode values + 5 mocha tests
current_mocha_count: 224 / 224 (V3.4.0.4a adds +5 over 219)
current_ql_collab_tests: 79 / 79 (unchanged; V3.4.0.4a engine-side adds napi only, no new ql-collab logic)
...
4. **V3.4.0.4 -- engine + IDE `.qbook` persistence (D3 + D5).** REORDERED to ship AFTER V3.4.0.5 + SPLIT a/b like V3.4.0.5.
   ...
   - **V3.4.0.4b (IDE commands + UUID PeerId)** PENDING (next session).  `quantlab.quantbookSaveAs` + `quantlab.quantbookOpen` commands with `vscode.window.showSaveDialog`/`showOpenDialog`; UUID-derived PeerId via `crypto.randomUUID()` truncated to 64-bit BigInt for cross-restart collision-resistance (closes R-V3.3-5); per-workbook PeerId stash (vscode workspaceState OR `.qbook`-sibling sidecar -- decision deferred to V3.4.0.4b implementation).  Live smoke test in VS Code window.
