...
export type OhlcBarSeriesRenderInput = {
...
export const renderOhlcBarSeries = (input: OhlcBarSeriesRenderInput): void => {
  const {
    ctx,
    surface,
    plotRect,
  ...
  const tickWidth = Math.max(lineWidth, barWidth * 0.6);
  const halfTick = tickWidth * 0.5;

  // Expand plotRect to account for OHLC bar tick width (half tick width on each side)
  // This prevents leftmost/rightmost bars from being clipped by axis margins
  const expandedPlotRect: Rect = {
    x: plotRect.x - halfTick,
    y: plotRect.y,
    width: plotRect.width + tickWidth, // Add full tick width (half on each side)
  ...
  // The expansion is only for clipping to allow bars to extend beyond edges
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const yOpenSnap = surface.snapY(plotRect.y + yOpen, lineWidth);
    const yCloseSnap = surface.snapY(plotRect.y + yClose, lineWidth);
    ...
    const leftTick = alignToDevicePixel(x - halfTick, safeDpr);
    const rightTick = alignToDevicePixel(x + halfTick, safeDpr);
