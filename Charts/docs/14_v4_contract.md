...
This contract defines V4 scope, compatibility constraints, and required instrumentation for all V4 work.

## Scope + Compatibility

- V1 API is frozen; V2 semantics must not regress (monotonic time, null gaps, worker opt-in).
- V4 changes are additive-only; no breaking changes to public APIs.
...
- Performance gates are enforced through the perf harness (see `docs/15_v4_perf_gates.md`).
...
- `readRenderStats(reset?)` returns:
  ...
  - grid/axis counters and frame cost stats
- `readWorkerStats(reset?)` returns:
  - `lod`: `{ queueDepth, totalMs, completed }`
  - `chunkLod`: `{ queueDepth, totalMs, completed }`
- `getAllocationStats()` returns:
  - `line`: `{ frame, total, pooled }`
  - `chunked`: `{ frame, total, pooled }` (may be zero until pooling is added)
