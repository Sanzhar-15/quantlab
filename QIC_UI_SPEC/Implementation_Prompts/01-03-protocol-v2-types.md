# Prompt 01-03: Protocol V2 Types
...
Define the TypeScript types for the new revision-based message protocol between the VS Code host and the webview. This protocol ensures reliable state synchronization.

---

## Context

The current protocol has issues:
- No revision tracking (can miss updates)
- No gap detection (can get out of sync)
- Mixed message types (hard to maintain)

Protocol V2 introduces:
- Revision numbers on all state-related messages
- Gap detection and recovery
- Clear separation of message categories
- Support for both legacy and new messages during migration
...
### In Scope
- Define `HostToWebviewMessageV2` union type
- Define `WebviewToHostMessageV2` union type
- Define supporting types
- Include amendments for codebase features
- Document message flow

### Out of Scope
- Implementation of message handling
- Message bridge (next prompt)
- Legacy protocol (keep existing)

---

## Pre-Conditions

- [ ] 01-01 complete (state interfaces available)
- [ ] Git branch created: `qic-ui/01-03-protocol-v2`
...
```bash
touch src/vs/workbench/contrib/qic/common/ui/messageProtocolV2.ts
```

### 2. Define Host → Webview Messages
...
### 3. Define Webview → Host Messages
...
- [ ] GAP-05 and GAP-06 fixes included

### Verification Commands
```bash
# Type check
npx tsc --noEmit src/vs/workbench/contrib/qic/common/ui/messageProtocolV2.ts
...
```bash
rm src/vs/workbench/contrib/qic/common/ui/messageProtocolV2.ts
```
...
- Keep existing protocol file for migration period
- V2 messages all have `revision` field (except `ready`)
- Type guards help differentiate V1 vs V2 messages
